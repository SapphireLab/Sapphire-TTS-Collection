...
SemantiCodec~\cite{liu2024semanticodec} quantizes AudioMAE~\cite{huang2022masked} SSL features without relying on acoustic inputs.
...
The GRL technique~\cite{drl} is commonly used for disentanglement. Suppose speaker information needs to be disentangled, and a classifier (or speaker verifier, etc.) $s_\mu(\cdot)$ receives some latent feature $\bm h$ from the acoustic token to perform speaker discriminative tasks.
