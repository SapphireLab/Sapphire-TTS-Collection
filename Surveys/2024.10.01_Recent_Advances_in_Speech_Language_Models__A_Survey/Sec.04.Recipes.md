...
The majority of speech tokenizers produce discrete tokens that better model the \textit{semantic information} within a speech waveform (semantic understanding speech tokenizers, \cref{sec:semanticUnderstandingTokenizer}), such as [W2V-BERT (2021)](../../Models/Speech_Representaion/2021.08.07_W2V-BERT.md); [HuBERT (2021)](../../Models/Speech_Representaion/2021.06.14_HuBERT.md).
...
We refer to them as \textit{semantic tokens} here.

Most SpeechLMs only employ \textit{semantic tokens} to represent speech.
...
A large number of works follow this setting and use HuBERT as the speech tokenizer \cite{textuallypretrainSLM,[SpiRit-LM (2024)](../../Models/Speech_LLM/2024.02.08_SpiRit-LM.md); [SpeechGPT (2023)](../../Models/Speech_LLM/2023.05.18_SpeechGPT.md)}.
...
Although semantic tokens excel at generating semantically meaningful speech because of the modeling of the contextual information within speech waveforms, researchers find out that the speech generated solely upon semantic tokens lacks expressive information such as prosody and different pitches or timbres \cite{expressodataset,[SpiRit-LM (2024)](../../Models/Speech_LLM/2024.02.08_SpiRit-LM.md)}.
To conquer this limitation, \textit{paralinguistic tokens} can be integrated into the modeling process to capture expressive information with speeches.
Specifically, pGSLM \cite{prosodyawareSLM} proposes to use the fundamental frequency (F0) and unit duration as prosody features to complement the HuBERT semantic tokens, and trains a multi-stream transformer language model to predict the semantic tokens, pitch (F0), and unit duration separately.
Similarly, [SpiRit-LM (2024)](../../Models/Speech_LLM/2024.02.08_SpiRit-LM.md) complements the HuBERT semantic tokens with pitch and style tokens \cite{sonarspeechprop}.
...
Another type is \textit{acoustic tokens}, which are tokens aiming to capture the essential acoustic features to reconstruct high-fidelity speech, primarily obtained from neural audio codec models (see \cref{sec:acousticGenerationTokenizer}).
...
VALL-E \cite{codecLM4TTS} utilizes codec tokens to achieve zero-shot TTS.
...
Viola \cite{violaCodecLM} uses codec tokens in a SpeechLM capable of performing ASR, TTS, and Machine Translation (in text).
...
Spectron \cite{spectrogramSLM} performs speech continuation by predicting the spectrograms frame-by-frame.
...
Mini-Omni \cite{Xie2024MiniOmni} extracts intermediate representations from a frozen Whisper encoder as input for the SpeechLM, whereas LauraGPT \cite{Du2023LauraGPT} employs an audio encoder trained alongside the SpeechLM to derive latent representations from input speech.
...
Commonly used datasets include those for ASR \cite{librispeechASR,librilightASR,peoplesdatasetASR,voxpopuliASR}, TTS ([LibriTTS (2019)](../../Datasets/2019.04.05_LibriTTS.md)), ST \cite{cvss-ST,voxpopuliASR}, podcasts \cite{spotifypodcastdataset}, and dialogues \cite{fisherdialoguedataset}.
...
SUTLM \cite{jointmodelingSpeechText} also uses a transformer as the language model.
...
For example, pGSLM \cite{prosodyawareSLM} proposes a multi-stream transformer language model (MS-TLM) that takes multiple streams of input and predicts multiple streams of output to generate speech units, duration, and pitch embeddings simultaneously.
dGSLM \cite{generativedialog2channel} introduced a dialogue transformer language model (DLM) to jointly model the dialogue speech data from the two speakers.
To enable the listening ability of SpeechLMs while speaking, LSLM \cite{LMListenWhileSpeaking} proposes to attach a streaming self-supervised learning (SSL) Encoder to an autoregressive token-based TTS Model.
Viola \cite{violaCodecLM} introduced a multi-task auto-regressive codec language model to autoregressively generate codec tokens instead of speech unit tokens.
...
Research by \cite{textuallypretrainSLM} found that starting with a textually pre-trained language model ([OPT (2022)](../../Models/LLM/2022.05.02_OPT.md) and [LLaMA (2023)](../../Models/LLM/2023.02.27_LLaMA.md)) can enhance the model's convergence rate and significantly improve its speech understanding capabilities.
...
Additionally, [AudioPaLM (2023)](../../Models/Speech_LLM/2023.06.22_AudioPaLM.md) trained the SpeechLM using PaLM and PaLM-2 \cite{Chowdhery2023Palm,palm2}, showing that the SpeechLM benefits from both an increased size of the pre-trained checkpoint and a larger training dataset.

The performance of SpeechLMs can be further enhanced by \textit{aligning} the text and speech modality representations.
...
AudioChatLlama \cite{audiochatllama} aims to ensure that the model produces consistent outputs regardless of whether the input is text or speech.
...
Spectron \cite{spectrogramSLM} solves the text-speech representation alignment problem by jointly supervising multiple objectives.
...
Llama-Omni \cite{llamaomni} also creates instruction-following data by synthesizing text-based datasets, adhering to specific constraints.
...
COSMIC \cite{pan2023cosmicSpeechInstructionTuning} constructed speech QA data by asking GPT-3.5 to generate question-answer pairs based on the transcriptions of English TED talk speeches.
...
The study by dGSLM \cite{generativedialog2channel} introduces a dual-transformer architecture to model two-speaker dialogues, using one transformer to handle speech from each speaker.
A cross-attention transformer layer is included to capture the interactions between the speakers' content.
In contrast, LSLM \cite{LMListenWhileSpeaking} proposes a different approach, utilizing a single decoder-only Transformer to model one speaker's speech in the dialogue.
...
Slience Mode refers to the state in which the SpeechLMs remain inactive or silent during periods of non-interaction.
...
VITA \cite{VITAtencentOmniMLLM} is currently the only work that integrates silence mode.
...
As a result, the model learns to output the \textit{end-of-sequence} token to terminate its response when non-query audio is detected.
