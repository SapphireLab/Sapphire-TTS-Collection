...
- 链接:
  ...
  - [Demo]()
- 文件:
  - [ArXiv](2025.01.25__2501.15177v1__Survey__Audio-Language_Models_for_Audio-Centric_Tasks.pdf)
  - [Publication] #TODO
...
Enabling machines to hear like humans and process audio-centric tasks has long been a significant challenge ([Pengi](../Models/SpeechLM/2023.05.19_Pengi.md)[^001]).
Audio-Language Models (ALMs), which are trained on audio-text data, focus on the processing, understanding, and reasoning of sounds.
...
ALMs are not only applicable to basic audio tasks, such as audio classification ([CLAP](../Models/_Basis/2022.06.09_CLAP_(CNN14+BERT).md)[^002]), but also show great potential for more complicated scenarios.
These include tasks such as audio-text retrieval [^003], audio generation ([AudioLDM](../Models/Diffusion/2023.01.29_AudioLDM.md)[^004]), automatic audio captioning [^005], audio source separation [^006], automatic speech translation ([AudioPaLM](../Models/SpeechLM/2023.06.22_AudioPaLM.md)[^007]), and audio chatbots ([SpeechGPT](../Models/SpokenDialogue/2023.05.18_SpeechGPT.md)[^008]).

In contrast to audio representation learning based on labeled data for specific tasks, ALM can learn from more descriptive textual information, expanding the scope of supervision to include human-annotated captions and readily available titles and descriptions from [web sources [URL]](https://github.com/LAION-AI/audio-dataset/blob/main/README.md)[^009].
...
让机器像人类一样听取和处理以音频为中心的任务长期以来一直是一个显著挑战 [^001].
音频语言模型 (Audio-Language Models, ALMs), 训练在音频-文本数据上, 专注于声音的处理, 理解和推理.
这一领域正在成为音频处理和自然语言处理相交叉的重要研究领域.

ALM 不仅适用于基础的音频任务, 如音频分类, 还在更复杂的场景中展现出巨大潜力.
包括音频文本检索, 音频生成, 自动音频描述, 音频声源分离, 自动语言翻译和音频聊天机器人.
...
To address these challenges, ALMs have been proposed to learn audio concepts through natural language supervision ([CLAP](../Models/_Basis/2022.06.09_CLAP_(CNN14+BERT).md)[^002]).
...
For example, natural language can describe the temporal order of multiple events using words such as `simultaneous`, `before`, and `after` \cite{ghosh2024compa}, better reflecting the complex composition of audio compared to predefined labels and helping models learn their intrinsic relationships [^010].
...
This has led to classic works such as [SpeechGPT](../Models/SpokenDialogue/2023.05.18_SpeechGPT.md)[^008], [Pengi](../Models/SpeechLM/2023.05.19_Pengi.md)[^001], and Qwen-Audio \cite{chu2023qwenaudio}, making Two Heads a unified architecture of Large Audio-Language Models.
...
Note that language pre-training is not the main focus of this paper, for more details please refer to \cite{[^011], minaee2024LLMs}.
...
Typical tasks include automatic speech recognition (ASR) \cite{radford2023whisper, li2024style, bai2024seedasr}, speech-to-text translation (S2TT) \cite{zhang2023VALLEX}, speech-to-speech translation (S2ST) \cite{radford2023whisper}, and automatic audio captioning (AAC) \cite{doh2023lp, mei2021audio, [^005], Kim2024enclap++, Ghosh2024Recap, li2024drcap}.
...
Although pre-trained models already have strong generalization capabilities, supervised fine-tuning on downstream datasets remains an important step to enhance task performance \cite{[CLAP](../Models/_Basis/2022.06.09_CLAP_(CNN14+BERT).md)[^002]; [^022], elizalde2024CLAP2}.
...
They can serve as a format that integrates multiple audio tasks \cite{gong2024ltu} and can also be used to guide models in better following human instructions ([SpeechGPT](../Models/SpokenDialogue/2023.05.18_SpeechGPT.md)[^008]).
...
Task-specific benchmarks like SoundDescs [^015], CompA \cite{ghosh2024compa}, and ADU-Bench \cite{gao2024ADUbench} evaluate models on particular tasks such as ATR and audio dialogue.
...
Literature has experimentally demonstrated that the choice of pre-trained audio and text encoders is crucial for generating robust representations and improving task performance \cite{deshmukh2022wavtext, [^022], mei2024wavcaps}.
