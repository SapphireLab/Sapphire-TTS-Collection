...
To address alignment issues between speech and text modalities, Recent works **SpeechGPT**[^SpeechGPT], [^SpeechGPT-Gen], [^107], **EMOVA**[^EMOVA] utilize a prompting approach called Chain-of-Modality (CoM), which first generates response text autoregressively before producing the corresponding speech.
...
**Text-guided generation.** Some end-to-end methods like **SpeechGPT**[^SpeechGPT], [^SpeechGPT-Gen], [^107], **EMOVA**[^EMOVA] use chain-of-thought reasoning, which allows guiding speech generation with the output of an underlying text LLM.
