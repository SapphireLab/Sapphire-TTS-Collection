...
> These attempts move away from the previous research direction to train models on curated high-quality recording datasets and produce human-like voices on benchmark datasets (Li et al., 2019; [VITS (2021)](2021.06_VITS.md); Tan et al., 2024; [YourTTS (2021)](2021.12_YourTTS.md)). 
...
> Several work (Kharitonov et al., 2023; [SoundStorm (2023)](2023.05_SoundStorm.md)) employ the semantic tokens from self-supervised speech representation learning (Chung et al., 2021) as an intermediary between text and audio tokens. 
...
> **Large-scale TTS**
> [AudioLM (2022)](2022.09_AudioLM.md) is a language model directly trained on audio tokens. 
> In [AudioLM (2022)](2022.09_AudioLM.md), semantic tokens are first generated. 
> These tokens originate from self-supervised discrete speech representation methods ([HuBERT (2021)](2021.06_HuBERT.md), [W2V-BERT (2021)]()) that have previously been utilized for speech resynthesis or generation without text (Lakhotia et al., 2021; Polyak et al., 2021; Nguyen et al., 2023). 
> Following this, the model produces acoustic tokens of neural audio codes given the semantic tokens. 
> Wang et al. propose the first neural codec language model, [Vall-E (2023)](2023.01_VALL-E.md), for text-to-speech that utilizes a pre-trained neural audio codec, EnCodec (D´ efossez et al., 2023). 
> In a different approach, following [AudioLM (2022)](2022.09_AudioLM.md), text-to-speech has been realized by applying language modeling to generate the semantic tokens from text, as demonstrated by [SPEAR-TTS (2023)](2023.02_SPEAR-TTS.md). 
...
> Our approach is similar to recent work that utilizes pre-trained language models, Spectron (Nachmani et al., 2023) and [SoundStorm (2023)](2023.05_SoundStorm.md). 
...
## 3. Preliminaries
> Building upon the approach proposed by [Vall-E (2023)](2023.01_VALL-E.md), we explore zero-shot TTS through the lens of neural codec language modeling task. 
...
> An [RQ-VAE (2022)](2022.03_RQ-VAE.md) is a neural network architecture representing data as discrete codes using residual vector quantization. 
...
> For the text-to-code latent language model, we adopt a transformer-based encoder-decoder LM, especially a pre-trained ByT5-large (Xue et al., 2021a) similar to [SoundStorm (2023)](2023.05_SoundStorm.md). 
...
> (1) [YourTTS (2021)](2021.12_YourTTS.md), a zero-shot TTS built on [VITS (2021)](2021.06_VITS.md) which is flow-based end-to-end TTS (representing **Conventional TTS**), 
> (2) [Vall-E (2023)](2023.01_VALL-E.md) and (3) [SPEAR-TTS (2023)](2023.02_SPEAR-TTS.md) (representing  **Neural Codec LM**), 
> (4) [VoiceBox (2023)](2023.06_VoiceBox.md), a flow-matching-based TTS model trained on large-scale training data (representing **Non-Autoregressive Model with Phoneme Input and Duration**).
...
> We follow the procedure in [VALL-E (2023)](2023.01_VALL-E.md). 
> In English-only Evaluation, we synthesize the transcription by using the <term>automatic speech recognition (ASR)</term> model, the CTC-based [HuBERT (2021)](2021.06_HuBERT.md)-Large model pre-trained on [Libri-Light (2020)]() and then fine-tuned on [LibriSpeech (2015)](). 
...
> 2. Speak Similarity: 
> We assess the speaker similarity of two separate speech audio clips by following the same procedure outlined in [VALL-E](2023.01_VALL-E.md). 
...
> Following the evaluation setting in [VALL-E (2023)](2023.01_VALL-E.md), we employ a subset of the [LibriSpeech]() test-clean dataset. 
> This subset comprises speech clips ranging from 4 to 10 seconds, each with a corresponding transcript. 
> Note that [YourTTS (2021)](2021.12_YourTTS.md) has official checkpoints, [VALL-E (2023)](2023.01_VALL-E.md) has an unofficial checkpoint, and others do not have checkpoints. 
> We use checkpoints of [YourTTS (2021)](2021.12_YourTTS.md) and [VALL-E (2023)](2023.01_VALL-E.md) for evaluations. 
> We compare the other baselines with ours via the performances reported in their papers ([VALL-E (2023)](2023.01_VALL-E.md); [SPEAR-TTS (2023)](2023.02_SPEAR-TTS.md); [VoiceBox (2023)](2023.06_VoiceBox.md)). 
> Since [SPEAR-TTS (2023)](2023.02_SPEAR-TTS.md) and [VoiceBox (2023)](2023.06_VoiceBox.md) also evaluate using the same approach with [VALL-E (2023)](2023.01_VALL-E.md), they can be directly compared with our model as well. 
...
> It is worth noting that [VoiceBox (2023)](2023.06_VoiceBox.md), a phoneme-based duration model, shows better performances than ours. 
...
> Our method is faster than the generation speed of Vall-E reported in [VoiceBox (2023)](2023.06_VoiceBox.md).
> While ours is faster than [VoiceBox (2023)](2023.06_VoiceBox.md) with 64 decoding steps, [VoiceBox (2023)](2023.06_VoiceBox.md) can use fewer iterations of decoding steps. 
> Tab. 
> 3 presents the subjective audio evaluations. 
> CLaM-en significantly outperforms the baseline, [YourTTS (2021)](2021.12_YourTTS.md), in quality and intelligibility, as indicated by QMOS. 
