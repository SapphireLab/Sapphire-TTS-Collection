...
> Though we rule out the individual variance in vocal tract anatomy by using a template space, such missing anatomical configuration is an important determinant of the voice texture, or timbre, of an individual speaker which is a crucial factor in defining the speaker's identity \cite{netzorg2024percetualqaulity}. Note that our definition of the speaker identity does not include information about dialect or accent which is actually aimed to be disentangled from the speaker identity.
