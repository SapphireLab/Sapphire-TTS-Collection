...
Recent music compression techniques based on neural codecs ([SpeechX [11]](../Speech_LLM/2023.08.14_SpeechX.md); [AudioDec [12]](2023.05.26_AudioDec.md); [VALL-E [13]](../Speech_LLM/2023.01.05_VALL-E.md); [FunCodec [14]](2023.09.14_FunCodec.md); [APCodec [15]](2024.02.16_APCodec.md)) attempt to compress music directly into discrete tokens. 
...
Common codecs like [EnCodec [16]](2022.10.24_EnCodec.md) and Generative Adversarial Networks(GAN)-based methods\cite{gan1,gan2,gan3} exhibit limitations in achieving particularly low bitrates.

In recent years, some research and works have focused on using semantic modeling to represent musical characteristics and utilizing [diffusion [20]](../Diffusion/DDPM.md) for reconstruction, such as [SemantiCodec [21]](2024.04.30_SemantiCodec.md) and [SEED-TTS [22]](../Speech_LLM/2024.06.04_Seed-TTS.md). 
...
To address these challenges, we propose a [flow-matching-based [23]](../Flow/2022.10.06_Flow_Matching.md) music codec ***MuCodec***. 
...
- ***MuCodec*** employs ***MuEncoder*** as the feature extractor and [Diffusion Transformer (DiT) [26]](../Diffusion/DiT.md) along with flow-matching-based method for fine-grained music modeling.
...
![](Images/2024.09.20_MuCodec_Fig.01.png)
...
Specifically, we choose pre-trained [HuBERT [30]](../Speech_Representaion/2021.06.14_HuBERT.md) and [MERT [31]](../Speech_Representaion/2023.05.31_MERT.md) models to separately model vocals and music background. 
...
We then calculate the similarity between the generated vocal part and the original vocal part with a pre-trained speaker similarity model\cite{speaker} and use [Whisper [36]](../Speech_LLM/2022.12.06_Whisper.md)-v2 to compute the Word Error Rate (WER) of the generated vocal part as the vocal clarity evaluation metric.

In the ***MuEncoder*** setting, we employ a 13-layer [Conformer [37]](../../Models/_Basis/2020.05.16_Conformer.md) model. 
...
During the generation process, we utilize sampling via [classifier-free guidance [39]](../Diffusion/Classifier-Free_Guidance.md), specifically setting the guidance scale value to 1.5. 
