...
- 链接:
  ...
  - [Demo]()
- 文件:
  - [ArXiv:2409.08583v1](_PDF/2024.09.13_2409.08583v1__LHQ-SVC__Lightweight_and_High_Quality_Singing_Voice_Conversion_Modeling.pdf)
  - [ArXiv:2409.08583v2](_PDF/2025.01.17_2409.08583v2__LHQ-SVC__Lightweight_and_High_Quality_Singing_Voice_Conversion_Modeling.pdf)
  - [Publication] #TODO
...
Nose et al. [^nose2015] proposed an HMM-based singing synthesis technique that allows users to intuitively and continuously control the singing style and intensity of synthesized singing voices.
Kobayashi et al. [^kobayashi2014] introduced a statistical conversion process based on GMM to estimate spectral features directly, addressing the degradation in voice quality during conversion.
Sisman et al. [^sisman2019] were the first to attempt using GANs for SVC and proposed a framework named SINGAN, which achieved high-quality singing voice conversion without relying on automatic speech recognition systems.
...
In comparison to traditional statistical methods, deep learning has demonstrated superior performance by learning complex nonlinear mappings from large datasets, significantly improving the quality of converted voices and speaker similarity [^sisman2020].
Sun et al. [^sun2015] proposed a sequence conversion method based on DBLSTM-RNNs, utilizing LSTM to improve the naturalness and continuity of speech output in VC tasks through parallel training data.
Xie et al. [^xie2016] introduced a DNN-based non-parallel training approach to VC, achieving effective conversion without parallel data.
...
Chen et al. [^chen2024] introduced a method named LCM-SVC, which accelerates inference in latent diffusion models (LDM) for SVC through Latent Consistency Distillation (LCD), while retaining high sound quality and timbre similarity.
...
In recent years, VC has seen remarkable advancements with applications spanning across areas such as speaker identity conversion, speech enhancement, and cross-lingual voice translation [^sisman2020].
...
However, these models often struggled with issues related to the loss of audio quality and the need for parallel data, limiting their generalization capabilities [^kobayashi2014].
...
Diffusion models, a class of generative models, operate by first adding noise to data in the forward process, and then reconstructing the data structure during the reverse process to generate samples [^yang2023].
These models have recently been introduced in the field of Singing Voice Conversion (SVC) to address the limitations of traditional methods when dealing with non-parallel data [^xue2024singvisio].
...
Further advancing controllability in SVC, Wang et al. [^wang2024prompt] introduced Prompt-Singer, a method that allows users to control singer attributes such as gender, vocal range, and volume through natural language prompts.
Although this method improves style control, the model's large size limits its scalability.
Chen et al. [^chen2024ldmsvc] developed LDM-SVC, a zero-shot any-to-any SVC method based on latent diffusion models.
LDM-SVC achieves faster training speed, but its inference quality is unstable.

Over and above these work, Lu et al. [^lu2024comosvc] proposed CoMoSVC, a consistency model-based SVC method designed to balance high-quality generation with fast sampling.
...
### Model structure optimization and dynamic adjustment
...
### Sampling mechanism optimization and efficient reasoning.
...
By distilling the teacher model’s multi-step output into a single-step output for the student model, as shown in Figure ~[frame1](#frame1), we effectively streamline the transformation process.

![](x6.png)

<a id="frame1">Progressive distillation diagram for diffusion modelling</a>
...
The datasets used in this study include the SVC-2023 Dataset [^huang2023singingvoiceconversionchallenge] and the DAMP (Data for Analysis of Musical Performances) Dataset [^8461660].
...
**Vocal Separation**
To extract clean vocal tracks from the songs, we employed the *Ultimate Vocal Remover* project.
This involved two main steps: first, we used MDX-Net [^mdxnet2021] to remove the instrumental accompaniment from the audio files.
Next, we applied the 6\_HP-Karaoke-UVR model [^uvr2021] to remove background harmonics from the vocal-separated files.
...
This step was automated using the open-source *Audio Slicer* tool [^audioslicer2021], which detects silent audio segments and slices the audio based on pitch, ensuring that the resulting audio clips primarily contained vocal content.
...
We used a pre-trained vocoder [^vocoder2020] fine-tuned for singing voice synthesis and calculated Mel-spectrograms with a 512-point Fast Fourier Transform (FFT), a 512-point window size, and a 128-point hop size, producing 80 frequency bins.
...
The following methods are considered: RVC [^RVC2025], SoVITS 4.0 [^svc_develop_team_so_vits_svc_2025], SoVITS 5.0 [^ouor_sovits_svc_5_0], Learn2Sing 2.0, [^xue2022learn2sing], CoMoSVC [^lu2024comosvc].
...
The performance of LHQ-SVC, along with several baseline models, is summarized in Table~[tab1](#tab1).
...
<a id="re1">Comparison of GPU and CPU inference time on computer and cell phones</a>
...
[^nose2015]: Nose, T., Kanemoto, M., Koriyama, T., \& Kobayashi, T. (2015). HMM-based expressive singing voice synthesis with singing style control and robust pitch modeling. *Computer Speech \& Language*, 34(1), 308-322.
[^sisman2020]: Sisman, B., Yamagishi, J., King, S., \& Li, H. (2020). An overview of voice conversion and its challenges: From statistical modeling to deep learning. *IEEE/ACM Transactions on Audio, Speech, and Language Processing*, 29, 132-157.
[^sun2015]: Sun, L., Kang, S., Li, K., \& Meng, H. (2015). Voice conversion using deep Bidirectional Long Short-Term Memory based Recurrent Neural Networks. In *2015 IEEE International Conference on Acoustics, Speech and Signal Processing (ICASSP)* (pp. 4869-4873). IEEE.
[^xie2016]: Xie, F. L., Soong, F. K., \& Li, H. (2016, September). A KL divergence and DNN-based approach to voice conversion without parallel training sentences. In *Interspeech* (pp. 287-291).
[^chen2024]: Chen, S., Gu, Y., Cui, J., Zhang, J., Chen, R., \& Dai, L. (2024). LCM-SVC: Latent Diffusion Model Based Singing Voice Conversion with Inference Acceleration via Latent Consistency Distillation. *arXiv preprint arXiv:2408.12354*.
[^yang2023]: Yang, L., Zhang, Z., Song, Y., Hong, S., Xu, R., Zhao, Y., ... \& Yang, M. H. (2023). Diffusion models: A comprehensive survey of methods and applications. *ACM Computing Surveys*, 56(4), 1-39.
[^xue2024singvisio]: Xue, L., Wang, C., Wang, M., Zhang, X., Han, J., \& Wu, Z. (2024). SingVisio: Visual Analytics of Diffusion Model for Singing Voice Conversion. *arXiv preprint arXiv:2402.12660*.
[^xue2022learn2sing]: Xue, H., Wang, X., Zhang, Y., Xie, L., Zhu, P., \& Bi, M. (2022). Learn2sing 2.0: Diffusion and mutual information-based target speaker svs by learning from singing teacher. *arXiv preprint arXiv:2203.16408*.
[^lei2023unisyn]: Lei, Y., Yang, S., Wang, X., Xie, Q., Yao, J., Xie, L., \& Su, D. (2023, June). UniSyn: an end-to-end unified model for text-to-speech and singing voice synthesis.  In *Proceedings of the AAAI Conference on Artificial Intelligence* (Vol. 37, No. 11, pp. 13025-13033).
[^chen2024ldmsvc]: Chen, S., Gu, Y., Zhang, J., Li, N., Chen, R., Chen, L., \& Dai, L. (2024). LDM-SVC: Latent Diffusion Model Based Zero-Shot Any-to-Any Singing Voice Conversion with Singer Guidance. *arXiv preprint arXiv:2406.05325*.
[^kobayashi2014]: Kobayashi, K., Toda, T., Neubig, G., Sakti, S., \& Nakamura, S. (2014). Statistical singing voice conversion with direct waveform modification based on the spectrum differential.  In *Fifteenth Annual Conference of the International Speech Communication Association*.
[^sisman2019]: Sisman, B., Vijayan, K., Dong, M., \& Li, H. (2019, November). SINGAN: Singing voice conversion with generative adversarial networks.  In *2019 Asia-Pacific Signal and Information Processing Association Annual Summit and Conference (APSIPA ASC)* (pp. 112-118). IEEE.
[^wang2024prompt]: Wang, Y., Hu, R., Huang, R., Hong, Z., Li, R., Liu, W., ... \& Zhao, Z. (2024). Prompt-Singer: Controllable Singing-Voice-Synthesis with Natural Language Prompt. *arXiv preprint arXiv:2403.11780*.
[^lu2024comosvc]: Lu, Y., Ye, Z., Xue, W., Tan, X., Liu, Q., \& Guo, Y. (2024). CoMoSVC: Consistency Model-based Singing Voice Conversion. *arXiv preprint arXiv:2401.01792*.
[^RVC2025]: RVC-Project, \emph{Retrieval-based Voice Conversion WebUI}, GitHub repository, 2023, \url{https://github.com/RVC-Project/Retrieval-based-Voice-Conversion-WebUI},
[^svc_develop_team_so_vits_svc_2025]: svc-develop-team, *SoftVC VITS Singing Voice Conversion*, GitHub repository, \url{https://github.com/svc-develop-team/so-vits-svc}, 2021.
[^ouor_sovits_svc_5_0]: ouor, \emph{Core Engine of Singing Voice Conversion \& Singing Voice Clone}, GitHub repository, \url{https://github.com/ouor/so-vits-svc-5.0}, 2023.
[^huang2023singingvoiceconversionchallenge]: Huang, Wen - Chin and Violeta, Lester Phillip and Liu, Songxiang and Shi, Jiatong and Toda, Tomoki. {The Singing Voice Conversion Challenge 2023}. arXiv:2306.14422 [cs.SD], 2023. Available at: \url{https://arxiv.org/abs/2306.14422}
[^8461660]: Wang, Cheng - i and Tzanetakis, George. {Singing Style Investigation by Residual Siamese Convolutional Neural Networks}. In 2018 IEEE International Conference on Acoustics, Speech and Signal Processing (ICASSP), pp. 116 - 120, 2018. DOI: 10.1109/ICASSP.2018.8461660.
[^mdxnet2021]: Défossez, A., Usunier, N., Bottou, L., \& Bach, F. (2021). Music Source Separation in the Waveform Domain. *Proceedings of the International Society for Music Information Retrieval Conference (ISMIR 2021)*.
[^uvr2021]: UVR Development Team. (2021). Ultimate Vocal Remover (UVR) Model - 6\_HP-Karaoke. Available at: \url{https://ultimatevocalremover.com}.
[^audioslicer2021]: Audio Slicer Development Team. (2021). Audio Slicer: Open-Source Tool for Audio Segmentation. Available at: \url{https://github.com/maxrmz/Audio-Slicer}.
[^vocoder2020]: Kumar, K., Gulati, A., Jain, A., Wang, Y., Zhang, J., \& Kolczynski, A. (2020). MelGAN: Generative Adversarial Networks for Conditional Waveform Synthesis. *Proceedings of NeurIPS 2020*.
