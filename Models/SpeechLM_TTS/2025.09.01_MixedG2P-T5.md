...
-->
本研究提出了一种新的声音合成方法, 该方法通过使用基于深度学习的模型直接从语音生成离散 Token, 来替代传统的音素转换 (Grapheme-to-Phoneme, G2P) 过程.
利用预训练的声音自监督模型, 我们训练了一个 T5 编码器, 从混合脚本文本 (例如, 包含汉字和假名) 中生成伪语言标签.
这种方法消除了对手动音素转写的需要, 降低了成本并增强了可扩展性, 尤其适用于大型未转录音频数据集.
我们的模型在性能上与传统的基于 G2P 的文本转语音系统相当, 并且能够合成出保留自然语言以及副语言特征 (如口音和语调) 的语音.
...
With the advancement of deep learning, speech synthesis models have demonstrated performance that significantly surpasses traditional methods (****Survey20220301**[^Borgholt2022Brief]).
These models typically convert input text into acoustic feature vectors through an encoder, and subsequently generate Mel-spectrograms using techniques such as attention mechanisms or variational inference, which are then transformed into speech by a vocoder (**Tacotron2**[^Shen2017Tacotron2], **VITS**[^Kim2021VITS]).
...
A prominent framework that leverages this representation for speech synthesis is the **Generative Spoken Language Model (GSLM)**[^Lakhotia2021GSLM].
...
Encoders such as **Wav2Vec2.0**[^Baevski2020Wav2Vec2.0] and **HuBERT**[^Hsu2021HuBERT] are commonly employed, offering robust and generalizable speech representations.
...
The decoder module, typically a neural vocoder or sequence-to-sequence model such as **Tacotron2**[^Shen2017Tacotron2], learns to reconstruct the speech waveform from these symbolic representations.
...
The pseudo-language label predictor is built on top of the **T5** architecture[^Raffel2019T5], a powerful Transformer-based sequence-to-sequence model known for its success in a wide range of natural language processing tasks.
...
While prior studies have explored the use of T5 for grapheme-to-phoneme conversion tasks, they remain constrained to alphabetic scripts and conventional G2P paradigms (**T5G2P**[^Rezackova2021T5G2P], **SpeechT5**[^Ao2021SpeechT5]).
...
Also, multilingual G2P systems have also been proposed, but they still operate within a text-based inference thus does not have additional prosodic features essential for natural-sounding speech synthesis, such as accents and durations, that can be obtained from SSL models ([^Sokolov2019Neural], [^Vesik2020One], **CharsiuG2P**[^Zhu2022CharsiuG2P]).
...
> | --- | --- | --- | --- |
> | Japanese | [OpenJTalk](https://open-jtalk.sp.nitech.ac.jp/) + [Mecab](http://taku910.github.io/mecab/) + **MARINE** [^Park2022MARINE] | **ReazonSpeech**[^Yin2023ReazonSpeech] (**T5**[^Raffel2019T5] Pre-training)<br>**JSUT**[^Sonobe2017JSUT] **JVS**[^Takamichi2019JVS] **J-KAC**[^Nakata2021J-KAC] **J-MAC**[^Takamichi2022J-MAC] **JSSS**[^Takamichi2020JSSS] (Fine-tuning for K-means/T5)<br> [tohoku-BERTv3-tokenizer](https://huggingface.co/tohoku-nlp/bert-base-japanese-v3) (T5 Tokenizer) | **JSUT** [^Sonobe2017JSUT] |
>
...
The spectral predictor used to generate synthesized speech from the common input representations is **FastSpeech2**[^Ren2020FastSpeech2].
...
For the synthesized speech, we used the **Whisper**-base[^Radford2022Whisper] ASR model and compared its output with the reference transcription, calculating the Character Error Rate (CER) in Japanese.
...
For this purpose, we employed **WARP-Q**[^Jassim2022WARP-Q], which is robust to common codec-related distortions and compensates internally for temporal misalignments between the ground truth and resynthesized signals—issues often encountered with metrics like **PESQ**[^Rix2001PESQ], which can be sensitive to speaker identity and speech type.
...
From an evaluation perspective, it is also necessary to individually assess the contributions of each input factor (e.g., duration, pitch) fed into **FastSpeech2**.
...
One possible solution is to convert raw text into language-aware discrete tokens using tokenizers such as **mT5**[^Xue2020mT5] or **ByT5**[^Xue2021ByT5], which tokenize text based on Unicode character strings.
...
[^Borgholt2022Brief]: [**Survey20220301**: A Brief Overview of Unsupervised Neural Speech Representation Learning](../../Surveys/2022.03.01_Borgholt2022Brief.md). ArXiv:2203.01829/AAAI(SAS).
...
[^Lakhotia2021GSLM]: [**GSLM**: On Generative Spoken Language Modeling from Raw Audio](../SpeechLM/PureSpeechLM/2021.02.01_GSLM.md). ArXiv:2102.01192/TACL2021.
...
[^Park2022MARINE]: [**MARINE**: A Unified Accent Estimation Method Based on Multi-Task Learning for Japanese Text-to-Speech](../_Basis/2022.09.18_MARINE.md). InterSpeech2022.
[^Yin2023ReazonSpeech]: [**ReazonSpeech**: A Free and Massive Corpus for Japanese ASR](../../Datasets/2023.03.00_ReazonSpeech.md). Japanese ANLP2023.
...
[^Takamichi2019JVS]: [**JVS Corpus**: Free Japanese Multi-Speaker Voice Corpus](../../Datasets/2019.08.17_JVS.md). ArXiv:1908.06248.
[^Sonobe2017JSUT]: [**JSUT Corpus**: Free Large-Scale Japanese Speech Corpus for End-to-End Speech Synthesis](../../Datasets/2017.10.28_JSUT.md). ArXiv:1711.00354.
[^Nakata2021J-KAC]: [**J-KAC**: Audiobook Speech Synthesis Conditioned by Cross-Sentence Context-Aware Word Embeddings](../../Datasets/2021.08.26_J-KAC.md). ISCA SSW2021.
[^Takamichi2022J-MAC]: [J-MAC: Japanese Multi-Speaker Audiobook Corpus for Speech Synthesis](../../Datasets/2022.01.26_J-MAC.md). ArXiv:2201.10896/InterSpeech2022.
[^Takamichi2020JSSS]: [**JSSS**: Free Japanese Speech Corpus for Summarization and Simplification](../../Datasets/2020.10.05_JSSS.md). ArXiv:2010.01793.
[^Rezackova2021T5G2P]: [**T5G2P**: Using Text-to-Text Transfer Transformer for Grapheme-to-Phoneme Conversion](../G2P/2021.08.30_T5G2P.md). InterSpeech2021.
...
[^Sokolov2019Neural]: [Neural Machine Translation for Multilingual Grapheme-to-Phoneme Conversion](../G2P/2020.06.25_Sokolov2020Neural.md). ArXiv:2006.14194/InterSpeech2019.
[^Vesik2020One]: [One Model to Pronounce Them All: Multilingual Grapheme-to-Phoneme Conversion With a Transformer Ensemble](../G2P/2020.06.23_Vesik2020One.md). ArXiv:2006.13343/SIGMORPHON2020.
[^Zhu2022CharsiuG2P]: [**CharsiuG2P**: ByT5 Model for Massively Multilingual Grapheme-to-Phoneme Conversion](../G2P/2022.04.06_CharsiuG2P.md). ArXiv:2204.03067/InterSpeech2022.
[^Qian2022ContentVec]: [**ContentVec**: An Improved Self-Supervised Speech Representation by Disentangling Speakers](../Tokenizers/2022.04.20_ContentVec.md). ArXiv:2204.09224.
...
[^Jassim2022WARP-Q]: [**WARP-Q**: Quality Prediction for Generative Neural Speech Codecs](../../Evaluations/2021.02.20_WARP-Q.md). ArXiv:2102.10449/ICASSP2021.
...
[^Xue2020mT5]: [**mT5**: A Massively Multilingual Pre-Trained Text-to-Text  Transformer](../TextLM/2020.10.22_mT5.md). ArXiv:2010.11934/NAACL2021.
[^Xue2021ByT5]: [**ByT5**: Towards a Token-Free Future with Pre-Trained Byte-to-Byte Models](../TextLM/2021.05.28_ByT5.md). ArXiv:2105.13626/TACL2022.