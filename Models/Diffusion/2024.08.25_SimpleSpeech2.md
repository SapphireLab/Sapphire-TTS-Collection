...
At the high level, previous large-scale TTS models can be categorized into either Auto-regressive (AR) based (e.g., [VALL-E](../SpeechLM/2023.01.05_VALL-E.md)) or Non-auto-regressive (NAR) based models (e.g., [NaturalSpeech2](2023.04.18_NaturalSpeech2.md); [NaturalSpeech3](2024.03.05_NaturalSpeech3.md)).
...
先前的大规模文本转语音模型从高层次上可以分为基于自回归 (Auto-Regressive, AR) 的模型 (如 [VALL-E](../SpeechLM/2023.01.05_VALL-E.md) 和基于非自回归 (Non-Auto-Regressive, NAR) 的模型 (如 [NaturalSpeech2](2023.04.18_NaturalSpeech2.md); [NaturalSpeech3](2024.03.05_NaturalSpeech3.md)) 两类.
...
Text-to-Speech synthesis (TTS) endeavors to synthesize speech that is both intelligible and natural at a human level, a field that has witnessed significant advancements over the past few years due to the proliferation of deep learning technologies ([Tacotron2 [1]](../Acoustic/2017.12.16_Tacotron2.md); [FastSpeech2 [2]](../Acoustic/2020.06.08_FastSpeech2.md); [VITS [3]](../E2E/2021.06.11_VITS.md); [Glow-TTS [4]](../Acoustic/2020.05.22_Glow-TTS.md); [Grad-TTS [5]](../Acoustic/2021.05.13_Grad-TTS.md); [TransformerTTS [6]](../Acoustic/2018.09.19_TransformerTTS.md); [NaturalSpeech [7]](../E2E/2022.05.09_NaturalSpeech.md)).
...
Generally, NAR-based models outperform AR-based models in terms of generation speed and robustness ([FastSpeech2 [2]](../Acoustic/2020.06.08_FastSpeech2.md)).
However, AR-based models exhibit superior diversity, prosody, expressiveness, and flexibility, attributable to their implicit duration modeling and AR sampling strategy ([Tacotron2 [1]](../Acoustic/2017.12.16_Tacotron2.md)).
Furthermore, many NAR-based models ([FastSpeech2 [2]](../Acoustic/2020.06.08_FastSpeech2.md)) depend on fine-grained alignment information (phoneme-level duration), which complicates data pre-processing.
Although numerous studies ([Grad-TTS [5]](../Acoustic/2021.05.13_Grad-TTS.md); [Glow-TTS [4]](../Acoustic/2020.05.22_Glow-TTS.md); [VITS [3]](../E2E/2021.06.11_VITS.md)) have suggested employing the Monotonic Alignment Search (MAS) strategy to learn alignment information, this approach not only increases training complexity but also results in alignments that may not always be suitable.
Most critically, the rigid boundary between phoneme and speech representation can lead to unnatural prosody ([Seed-TTS [8]](../SpeechLM/2024.06.04_Seed-TTS.md)).
...
To achieve these objectives, they propose utilizing large-scale datasets to train Text-to-Speech (TTS) models ([AudioLM [9]](../SpeechLM/2022.09.07_AudioLM.md); \cite{valle,speartts,uniaudio,make-a-voice}.
...
Additionally, the persistent issue of hard boundary alignment continues to limit the natural prosody in NAR-based models ([Seed-TTS [8]](../SpeechLM/2024.06.04_Seed-TTS.md)).
...
We notice that some concurrent works, [Seed-TTS [8]](../SpeechLM/2024.06.04_Seed-TTS.md) and DiTTo-TTS \cite{ditto-tts} also adopt a similar idea to use the sentence duration to control the generated speech length in an NAR framework.
...
In this study, we follow SimpleSpeech \cite{simplespeech} to use scalar quantization \cite{balle2016end,mentzer2023finite} to replace the residual vector quantization in audio codec models, which can be trained with the reconstruction loss and adversarial loss without any training tricks.
...
Following previous work \cite{gao2024lumina}, we will refer to it as flow matching in the following discussion.
flow matching uses a simple strategy to corrupt data: it linearly interpolates between noise and data in a straight line.
...
In other words, classifier-free guidance conducts this by decreasing the unconditional likelihood with a negative score term.
During the training stage, previous works try to mask the condition information of some samples (\textit{e.g.} set the training sample's text as empty with 10\% probability), so that these samples can be used to optimize unconditional distribution $p(\boldsymbol{x})$.
...
Previous NAR TTS models ([FastSpeech2 [2]](../Acoustic/2020.06.08_FastSpeech2.md))\cite{ns2} has a phoneme duration predictor module, we can easily obtain the duration of each phoneme, and then we can sum them to get the final sentence duration.
We use the phoneme duration predictor from open sourced [FastSpeech2](../Acoustic/2020.06.08_FastSpeech2.md) model in [Github](https://github.com/ming024/FastSpeech2).
...
However, previous work ([Seed-TTS [8]](../SpeechLM/2024.06.04_Seed-TTS.md)) suggests that WER may not serve as a definitive criterion; a lower WER often indicates that the model produces more ‘standardized’ speech, which is easier for ASR systems to recognize.
...
The concurrent work, [Seed-TTS [8]](../SpeechLM/2024.06.04_Seed-TTS.md), also uses a similar strategy to us, they show that the system can be more robust after training with large-scale data.
...
Although increasing the number of demonstration samples in the prompt and averaging multiple runs can mitigate this issue, it also significantly raises inference costs. (3) ByT5-based predictors and the duration predictor of [FastSpeech2 [2]](../Acoustic/2020.06.08_FastSpeech2.md) demonstrate greater stability than AR-based phoneme duration predictors.
Considering both cost and simplicity, we recommend using the ByT5-based sentence duration predictor as default choice.
