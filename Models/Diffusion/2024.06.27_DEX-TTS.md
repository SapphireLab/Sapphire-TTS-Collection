...
- 作者:
  - 01 Hyun Joon Park - Korea University
  - 02 Jin Sob Kim - Korea University
  - 03 Wooseok Shin - Korea University
  - 04 Sung Won Han - Korea University
- 链接:
...
- 文件:
  - [ArXiv](_PDF/2406.19135v1__DEX-TTS__Diffusion-Based_EXpressive_Text-to-Speech_with_Style_Modeling_on_Time_Variability.pdf)
  - [Publication]
...
Text-to-Speech (TTS) ([WaveNet](../Vocoder/2016.09.12_WaveNet.md); [Tacotron](../Acoustic/2017.03.29_Tacotron.md); [Tacotron2](../Acoustic/2017.12.16_Tacotron2.md); [FastSpeech](../../Models/Acoustic/2019.05.22_FastSpeech.md)) is the task of synthesizing natural speech from a given text, which is applied to various applications such as voice assistant services.
To generate diverse and high-fidelity speech, researchers have studied Transformer \cite{li2019neural}-, GAN \cite{[Parallel WaveGAN](../Vocoder/2019.10.25_Parallel_WaveGAN.md), yang2021ganspeech}-, and normalizing flow ([Flow-TTS](../Acoustic/2020.04.09_Flow-TTS.md); [Glow-TTS](../Acoustic/2020.05.22_Glow-TTS.md deep generative models.
Recently, with the success of diffusion models in various generative tasks \cite{ho2020denoising, ho2022imagen, schneider2023archisound}, researchers have shifted their focus to diffusion-based TTS ([DiffWave](../Vocoder/2020.09.21_DiffWave.md); [Diff-TTS](../Acoustic/2021.04.03_Diff-TTS.md); [Grad-TTS](../Acoustic/2021.05.13_Grad-TTS.md), liu2022diffgan) and proved the outstanding performance of diffusion models in TTS as well.
...
A few studies ([Diff-TTS](../Acoustic/2021.04.03_Diff-TTS.md); [Grad-TTS](../Acoustic/2021.05.13_Grad-TTS.md); [CoMoSpeech](../../Models/Diffusion/2023.05.11_CoMoSpeech.md)) on diffusion-based TTS have improved TTS performance by adapting diffusion formulations to suit TTS.
However, the network of these studies was confined to simple U-Net, leading to limited latent representations.
Although [U-DiT TTS](../../Models/Diffusion/2023.05.22_U-DiT_TTS.md) used [DiT](2022.12.19_DiT.md) for the diffusion network, DiT is yet to be effectively leveraged for TTS.
...
[Diff-TTS](../Acoustic/2021.04.03_Diff-TTS.md), [Grad-TTS](../Acoustic/2021.05.13_Grad-TTS.md), and [CoMoSpeech](../../Models/Diffusion/2023.05.11_CoMoSpeech.md) properly utilized diffusion methods for TTS.
...
[U-DiT TTS](../../Models/Diffusion/2023.05.22_U-DiT_TTS.md) improved the design of diffusion networks in TTS by adopting [DiT](2022.12.19_DiT.md) blocks while retaining some aspects of U-Net down- and up-sampling.
[DiT](2022.12.19_DiT.md) can extract detailed representations using attention operations between small patches.
...
[Diff-TTS](../Acoustic/2021.04.03_Diff-TTS.md), [Grad-TTS](../Acoustic/2021.05.13_Grad-TTS.md), [CoMoSpeech](../../Models/Diffusion/2023.05.11_CoMoSpeech.md) 等模型恰当地利用了扩散方法进行文本转语音.
尽管它们有效地应用了扩散公式进行文本转语音, 但先前研究中的扩散网络主要局限于 U-Net 架构.
这局限了隐表示, 表明扩散网络设计有待改进的必要.
[U-DiT TTS](../../Models/Diffusion/2023.05.22_U-DiT_TTS.md) 通过采用 [DiT](2022.12.19_DiT.md) 模块, 同时保留 U-Net 下采样和上采样的部分, 改进了文本转语音中扩散网络的设计.
[DiT](2022.12.19_DiT.md) 可以利用小块之间的注意力操作提取详细表示.
...
We utilize [Adaptive Layer Normalization (AdaLN)](../Acoustic/2021.03.01_AdaSpeech.md) after each MHSA and FFN to inject $h^{e}_{v}$, extracted by the T-V encoder given a reference input, into $h_{text}$.
...
We adopt a convolution-based Duration Predictor (DP) ([Glow-TTS](../Acoustic/2020.05.22_Glow-TTS.md{text}$ extracted by the text encoder is used.
...
我们采用 [Glow-TTS](../Acoustic/2020.05.22_Glow-TTS.md其中 $h_{text}$ 由文本编码器提取得到.
...
>
The diffusion decoder comprises convolution blocks, adapters, and [DiT](2022.12.19_DiT.md) blocks.
To leverage powerful denoising in the latent space, we utilize up and down convolution blocks, as in [Grad-TTS](../Acoustic/2021.05.13_Grad-TTS.md), to decrease and increase the resolution of $h_{diff}$.
...
To train ***DEX-TTS***, we follow the loss formulation of previous diffusion-based TTS studies ([Grad-TTS](../Acoustic/2021.05.13_Grad-TTS.md); [CoMoSpeech](../../Models/Diffusion/2023.05.11_CoMoSpeech.md)), in which duration loss $\mathcal{L}_{dur}$, prior loss $\mathcal{L}_{prior}$, and diffusion loss $\mathcal{L}_{diff}$ are used. $\mathcal{L}_{dur}$ is utilized to train DP that predicts the duration mapping $h_{text}$ to mel frames, and it is defined as $||log(d)-log(\hat{d})||_{2}^{2}$.
...
Except for YourTTS (end-to-end TTS), the generated mel-spectrograms are transformed into waveforms by the pre-trained [HiFi-GAN](../Vocoder/2020.10.12_HiFi-GAN.md).
...
For comparison, we select previous diffusion-based TTS models and train models on a single-speaker dataset, [LJSpeech](../../Datasets/2017.07.05_LJSpeech.md), following the set split of [Grad-TTS](../Acoustic/2021.05.13_Grad-TTS.md).
