...
In recent years, neural codec language models[^01], [^02], [^03], [^04] and large-scale diffusion models [^05], [^06], [^07], [^08], [^09], [^10], [^11] have brought considerable advancements to the field of speech synthesis.
Unlike traditional text-to-speech (TTS) systems[^12], [^13], [^14], [^15], [^16], [^17], [^18], these models are trained on large-scale, multi-domain speech corpora, which contributes to notable improvements in the naturalness and expressiveness of synthesized audio.
Given only seconds of speech prompt, they can synthesize identity-preserving speech in a zero-shot manner.

To generate high-quality speech with clear and expressive pronunciation, a TTS model must establish an alignment mapping from text to speech signals[^15], [^19].
However, from the perspective of speech-text alignment, current solutions suffer from the following issues:

- **Models with implicit speech-text alignment** achieve the soft alignment paths through attention mechanisms[^01], [^20], [^21].
...
The lengthy discrete speech codes, which typically require a bit rate of 1.5 kbps[^22], [^23], impose a significant burden on these autoregressive language models;
(2) diffusion-based models without explicit duration modeling[^07], [^08], [^24], [^25], [^26], [^10], [^11], which significantly speeds up the speech generation process.
...
During training, alignment paths[^27], [^15] are directly introduced into their models[^06], [^05], [^09] to reduce the complexity of text-to-speech generation, which achieves higher intelligibility.
Nevertheless, they suffer from the following limitations:
(1) predefined alignments constrain the model's search space to produce more natural-sounding speech[^28], [^20];
...
Zero-shot TTS[^30], [^01], [^02], [^05], [^06], [^31], [^32], [^07], [^33], [^34], [^09], [^35], [^36] aims to synthesize unseen voices with speech prompts.
Among them, neural codec language models[^20] are the first that can autoregressively synthesize speech that rivals human recordings in naturalness and expressiveness.
...
To address these issues, some subsequent works explore solutions based on continuous vectors and non-autoregressive diffusion models[^05], [^06], [^07], [^08], [^10], [^11], [^36].
These works can be categorized into two main types:
The first type directly models speech-text alignments using attention mechanisms without explicit duration modeling[^07], [^08].
...
The second category[^05], [^06] utilizes predefined alignments to simplify alignment learning.
...
The encoder $E$ is similar to the one used in[^45], and the decoder $D$ is based on[^46].
We also adopt the multi-period discriminator (MPD), multi-scale discriminator (MSD), and multi-resolution discriminator (MRD) [^46], [^47] to model the high-frequency details in waveforms, which ensure perceptually high-quality reconstructions.
...
We use the standard transformer block from LLAMA[^51] as the basic structure for ***MegaTTS3*** and adopt the Rotary Position Embedding (RoPE)[^52] as the positional embedding.
...
(1) the librispeech[^29] test-clean set following NaturalSpeech 3[^09] for zero-shot TTS evaluation;
...
(1) For zero-shot TTS, we evaluate speech intelligibility using the word error rate (WER) and speaker similarity using SIM-O[^09].
...
As SIM-R[^06] is not comparable across baselines using different acoustic tokenizers, we recommend focusing on SIM-O in our experiments.
...
(1) w/FA, which replaces the sparse alignment in ***MegaTTS3*** with forced alignment used in [^06], [^05];
...
- **The WaveVAE model** consists of a VAE encoder, a wave decoder, and discriminators; The VAE encoder follows the architecture used in[^45].
...
- **NaturalSpeech 3**[^09] designs a neural codec with factorized vector quantization (FVQ) to disentangle speech waveform into subspaces of content, prosody, timbre, and acoustic details and propose a factorized diffusion model to generate attributes in each subspace following its corresponding prompt.
We obtained the samples by contacting the authors.
...
In large-scale zero-shot TTS, VoiceBox[^06] and NaturalSpeech 2[^05] achieve CFG mechanism by dropping the text and prompt speech features.
...
We follow[^09] and use strong-intensity samples to validate the model’s ability to handle emotional variance;
...
(3) ``Ours w/ Standard AR Duration'', we replace the duration from F-LM with the duration from standard AR duration predictor following SimpleSpeech 2[^11].
...
[^03]: song2024ella
[^04]: xin2024rall
[^05]: shen2023naturalspeech
[^06]: le2023Voicebox
[^07]: lee2024ditto
[^08]: eskimez2024e2
[^09]: ju2024naturalspeech
[^10]: yang2024simplespeech
[^11]: yang2024simplespeech2
[^12]: shen2018natural
[^13]: jia2018transfer
[^14]: li2019neural
[^15]: kim2020glow
[^16]: ren2019fastspeech
[^17]: kim2021conditional
[^18]: kim2022guided
[^19]: tan2021survey
[^20]: chen2024vall
[^21]: du2024cosyvoice
[^22]: kumar2024high
[^23]: wu2024towards
[^24]: lovelace2023simple
[^25]: gao2023e3
[^26]: cambara2024mapache
[^27]: ren2020fastspeech
[^28]: anastassiou2024seed
[^29]: panayotov2015librispeech
[^30]: casanova2022yourtts
[^31]: jiang2024mega
[^32]: liu2024autoregressive
[^33]: li2024styletts
[^34]: lee2023hierspeech++
[^35]: meng2024autoregressive
[^36]: chen2024f5
...
[^61]: wang2024maskgct
[^62]: yang2024instructtts
[^63]: defossez2022high
[^64]: ye2025llasa
[^65]: loizou2011speech
[^66]: lee2024voiceldm
[^67]: yang2024dualspeech
[^68]: zhang2022wenetspeech
[^69]: chen2021gigaspeech
[^70]: ardila2019common
[^71]: livingstone2018ryerson
[^72]: zen2019libritts
[^73]: yang2024qwen2
[^74]: peng2024voicecraft
