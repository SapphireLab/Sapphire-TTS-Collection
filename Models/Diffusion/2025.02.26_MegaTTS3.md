...
We also visualize the attention matrices across various layers of ***MegaTTS3*** and obtain insightful findings in Appendix F.
...
本节介绍 ***MegaTTS3***.
首先, 我们介绍 ***MegaTTS3*** 的架构设计.
然后, 我们提供稀疏对齐机制, 分段 Rectified Flow 加速技术和多条件无分类器引导策略的详细解释.
...
As shown in Figure.01 (a), given a speech waveform $s$, the VAE encoder $E$ encodes $s$ into a latent vector $z$, and the wave decoder $D$ reconstructs the waveform $x = D(z) = D(E(s))$.
...
The training loss of the speech compression model can be formulated as
$$
\mathcal{L} = \mathcal{L}_{\mathrm{rec}} + \mathcal{L}_{\mathrm{KL}} + \mathcal{L}_{\mathrm{Adv}}
$$

where $\mathcal{L}_{\mathrm{rec}}=\|s-\hat{s}\|^2$ is the spectrogram reconstruction loss, $\mathcal{L}_{\mathrm{KL}}$ is the slight KL-penalty loss[^48], and $\mathcal{L}_{\mathrm{Adv}}$ is the LSGAN-styled adversarial loss[^49].
After training, a one-second speech clip can be encoded into 25 vector frames.
For more details, please refer to [Appendix A.1](#Appenidx_A1) and [A.5](#Appendix_A5).

</td><td>

**WaveVAE**

如图 01 (a) 所示, 给定语音波形 $s$, VAE 编码器 $E$ 将 $s$ 编码为隐变量 $z$, 波形解码器 $D$ 重构波形 $x = D(z) = D(E(s))$.
为了减少模型的计算负担和简化语音-文本对齐学习, 编码器 $E$ 按比例 $d$ 下采样语音波形.
编码器 $E$ 类似 **WavTokenizer**[^45] 的编码器, 解码器 $D$ 基于 **HiFi-GAN**[^46].
我们还采用多周期判别器 MPD, 多尺度判别器 MSD, 多分辨率判别器 MRD (**HiFi-GAN**[^46], **UnivNet**[^47]) 来建模波形中的高频细节, 确保感知上高质量的重构.
...
- $\mathcal{L}_{\mathrm{rec}}=\|s-\hat{s}\|^2$ 是频谱图重构损失,
- $\mathcal{L}_{\mathrm{KL}}$ 是轻微的 KL 惩罚损失[^48],
- $\mathcal{L}_{\mathrm{Adv}}$ 是 LSGAN 型的对抗损失[^49].

在训练后, 一秒语音片段可以编码器 25 个向量帧.
更多细节, 请阅读[附录 A.1](#Appenidx_A1) 和 [A.5](#Appendix_A5).
...
Equation.01 converts $Z_{0}$ from $\pi_{0}$ to $Z_{1}$ from $\pi_{1}$.
...
潜在扩散 Transformer 用于预测语音, 需要匹配给定说话人的风格和提供的文本内容.
给定从标准高斯分布 $\pi_{0}$ 采样的随机变量 $Z_0$ 和由语音压缩模型给定的潜在空间中采样的 $Z_1$, 其数据密度为 $\pi_{1}$, 我们采用 Rectified Flow[^50] 来隐式学习传输映射 $T$, 即 $Z_1 := T(Z_0)$.

Rectified Flow 通过构造如下常微分方程 (ODE) 来学习 $T$:
$$
\mathrm{d}Z_t = v(Z_t, t)\,\mathrm{d}t,
$$
...
上述方程将来自 $\pi_{0}$ 的 $Z_{0}$ 转换为来自 $\pi_{1}$ 的 $Z_{1}$.
漂移力 $v$ 驱动流沿着方向 $(Z_{1}-Z_{0})$ 前进.
...
潜在扩散 Transformer, 由 $\theta$ 参数化, 通过 $v_{\theta}(Z_{t}, t)$ 估计 $v(Z_{t}, t)$ 来训练, 最小化以下关于线方向 $(Z_{1}-Z_{0})$ 的最小二乘损失:
$$
\min_v \int_0^1 \mathbb{E} \left[ \| (Z_1 - Z_0) - v(Z_t, t) \|^2 \right] \, \mathrm{d}t.
$$

我们使用来自 **LLAMA3**[^51] 的标准 Transformer 块作为 ***MegaTTS3*** 的基本结构, 并采用 **RoPE**[^52] 作为位置嵌入.

在训练时, 我们将潜在向量序列随机划分为提示区域 $z_{prompt}$ 和掩码目标区域 $z_{target}$, 其中 $z_{prompt}$ 的比例为 $\gamma \sim U(0.1, 0.9)$.
我们使用 $v_{\theta}$ 来预测掩码目标向量 $\hat{z}_{target}$ 条件于 $z_{prompt}$ 和音素嵌入 $p$, 记为 $v_{\theta}(\hat{z}_{target}|z_{prompt}, p)$.
损失仅计算掩码区域 $z_{target}$.
***MegaTTS3*** 学习来自 $p$ 的平均发音, 以及来自 $z_{prompt}$ 的对应说话人的特点, 如声调, 口音, 语调等.
...
在本小节中, 我们描述了稀疏对齐策略作为 ***MegaTTS3*** 的基础, 随后介绍分段 Rectified Flow 和多条件 CFG 策略, 以进一步增强 ***MegaTTS3*** 的能力.
...
The visualizations of the implicit alignment paths are included in [Appendix F](#Appendix_F).
...
首先我们深入分析不同语音-文本对齐建模方法特性背后的原因.

隐式建模语音-文本对齐是相当具有挑战性的任务, 这导致次优的语音理解能力, 尤其是在困难情形下.
另一方面, 采用预定义的硬对齐路径限制了模型的搜索空间, 难以产生更自然的语音.

这些系统的特性启发我们设计一个方法来结合它们的优势:
我们首先给 ***MegaTTS3*** 提供粗略的对齐, 然后使用 Transformer 块中的注意力机制来构造细粒度的隐式对齐路径.
隐式对齐路径的可视化可在[附录 F](#Appendix_F) 中找到.
具体来说, 记潜在语音向量序列为 $z=[z_1, z_2, \cdots, z_n]$, 音素序列为 $p=[p_1, p_2, \cdots, p_m]$, 音素持续时间序列为 $d=[d_1, d_2, \cdots, d_m]$, 其中 $n$, $m$ 为序列长度.
对应音素 $p_i$ 的语音向量的长度为持续时间 $d_i$.

给定 $d=[2,2,3]$, 硬语音文本对齐路径可以表示为 $a=[p_1, p_1, p_2, p_2, p_3, p_3, p_3]$.
为了构造粗略对齐 $\tilde{a}$, 我们随机保留每个音素的一个锚点: $\tilde{a} = [\underline{M}, p_1, p_2, \underline{M}, \underline{M}, \underline{M}, P_3]$, 其中 $\underline{M}$ 表示掩码标记.
$\tilde{a}$ 被卷积层下采样以与潜在序列 $z$ 长度匹配.
然后, 我们直接将下采样的 $\tilde{a}$ 和 $z$ 按通道维度连接.
$\tilde{a}$ 中的锚点提供了 ***MegaTTS3*** 关于每个音素的近似位置信息, 简化了语音-文本对齐的学习过程.
同时, 粗略对齐信息并不限制 ***MegaTTS3*** 的搜索空间, 还能对每个音素的持续时间进行细粒度控制.
...
We provide details of PeRFlow training for ***MegaTTS3*** in [Appendix C](#Appendix_C).

</td><td>

**分段 Rectified Flow 加速**

我们采用 **分段 Rectified Flow (PeRFlow)**[^53] 来蒸馏预训练的 ***MegaTTS3*** 模型, 转化为更高效的生成器.
尽管我们的 ***MegaTTS3*** 在时间维度上是非自回归的, 但需要多次迭代才能求解 Flow ODE.
迭代次数 (即函数评估次数, NFE) 对推理效率有着巨大的影响, 尤其是在模型规模扩大时.
因此, 我们采用 PeRFlow 技术进一步减少 NFE, 通过将流轨迹分段为多个时间窗口来实现.
在这些缩短的时间间隔内应用 ReFlow 操作, PeRFlow 消除了对训练数据准备的完整 ODE 轨迹的模拟的需要, 允许它在训练过程中与大规模真实数据实时训练.

给定窗口数 $K$, 我们将时间 $t\in[0,1]$ 分为 $K$ 个时间窗口 $\{ (t_{k-1}, t_{k}] \}^{K}_{k=1}$.
然后, 我们随机采样 $k\in\{1,\cdots,K\}$ 均匀分布.
我们使用采样时间窗口的起点 $z_{t_{k-1}} = \sqrt{1 - \sigma^2(t_{k-1})} z_1 + \sigma(t_{k-1}) \epsilon$ 来求解时间窗口的终点 $\hat{z}_{t_k} = \phi_{\theta}(z_{t_{k-1}}, t_{k-1}, t_{k})$, 其中 $\epsilon \sim \mathcal{N}(0, I) $ 是随机噪声, $\sigma(t)$ 是噪声调度, $\phi_{\theta}$ 是教师模型的 ODE 求解器.
因为 $z_{t_{k-1}}$ 和 $\hat{z}_{t_{k}}$ 都是可用的, 学生模型 $\hat{\theta}$ 可以通过以下目标训练:
$$
\ell = \left\lVert v_{\hat{\theta}}(z_t, t) - \frac{\hat{z}_{t_k} - z_{t_{k-1}}}{t_k - t_{k-1}} \right\lVert^2,
$$

- $v_{\hat{\theta}}$ 是估计漂移力的函数, 参数为 $\hat{\theta}$,
- $t$ 是均匀分布在 $(t_{k-1},t_{k}]$ 上的采样时间.

我们在[附录 C](#Appendix_C) 中提供了 PeRFlow 训练 ***MegaTTS3*** 的详细细节.
...
The detailed experimental setup is described in [Appendix K](#Appendix_K).
...
我们采用**无分类器引导方法**[^54] 来引导模型 $g_{\theta}$ 的输出指向条件生成 $g_{\theta}(z_t,c)$, 远离无条件生成 $g_{\theta}(z_t,\varnothing)$:
$$
\hat{g}_{\theta}(z_t, c) = g_{\theta}(z_t, \varnothing) + \alpha \cdot \left[ g_{\theta}(z_t, c) - g_{\theta}(z_t, \varnothing) \right],
$$
...
- $c$ 表示条件状态,
- $\varnothing$ 表示无条件状态,
- $\alpha$ 是基于实验结果选择的引导尺度.

和标准的无分类器引导不同, ***MegaTTS3*** 的条件状态 $c$ 由音素嵌入 $p$ 和说话人提示 $z_{prompt}$ 组成.

在实验中, 随着文本引导尺度增加, 我们观察到发音变换遵循如下模式:
1. 从不正确的发音开始;
2. 然后转移到当前说话人的口音的发音;
3. 最后接近目标语言的标准发音.

实验细节请参见[附录 K](#Appendix_K).

这允许我们使用文本引导尺度 $\alpha_{txt}$ 来控制口音强度.

同时, 说话人引导尺度 $\alpha_{spk}$ 应该是相对较高的值, 以确保说话人相似度高.

因此, 我们采用多条件无分类器引导技术来分别控制 $\alpha_{txt}$ 和 $\alpha_{spk}$:
...
在训练中, 我们按概率 $p_{spk}=0.10$ 随机丢弃说话人提示 $z_{prompt}$.
只有当 $z_{prompt}$ 被丢弃时, 我们才按概率 50\% 随机丢弃条件 $p$.
因此, 我们的模型能够处理由上式描述的三种条件输入.
...
We provide the model configuration and detailed hyper-parameter setting in Appendix A.1..
...
The batch sizes, optimizer settings, and learning rate schedules are described in Appendix A.1..
...
Additionally, we visualize the attention score matrices from different transformer layers in ***MegaTTS3*** in Appendix F, leading to some interesting observations.
...
(1) the architecture and algorithm of the MegaTTS 3 model are described in Section~\ref{method} and and relevant hyperparameters are fully described in Appendix A.1.;
