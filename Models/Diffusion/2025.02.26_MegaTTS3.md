...
- **Models with implicit speech-text alignment** achieve the soft alignment paths through attention mechanisms (**VALL-E**[^01], [^20], [^21]).
...
The lengthy discrete speech codes, which typically require a bit rate of 1.5 kbps (**DAC**[^22], [^23]), impose a significant burden on these autoregressive language models;
(2) diffusion-based models without explicit duration modeling (**DiTTo-TTS**[^07], **E2 TTS**[^08], [^24], [^25], [^26], **SimpleSpeech**[^10], **SimpleSpeech2**[^11]), which significantly speeds up the speech generation process.
...
During training, alignment paths[^27], [^15] are directly introduced into their models **Voicebox**[^06], **NaturalSpeech2**[^05], **NaturalSpeech3**[^09] to reduce the complexity of text-to-speech generation, which achieves higher intelligibility.
...
Zero-shot TTS[^30], **VALL-E**[^01], **SPEAR-TTS**[^02], **NaturalSpeech2**[^05], **Voicebox**[^06], [^31], [^32], **DiTTo-TTS**[^07], [^33], [^34], **NaturalSpeech3**[^09], [^35], [^36] aims to synthesize unseen voices with speech prompts.
...
To address these issues, some subsequent works explore solutions based on continuous vectors and non-autoregressive diffusion models (**NaturalSpeech2**[^05], **Voicebox**[^06], **DiTTo-TTS**[^07], **E2 TTS**[^08], **SimpleSpeech**[^10], **SimpleSpeech2**[^11], [^36].
...
The encoder $E$ is similar to the one used in **WavTokenizer**[^45], and the decoder $D$ is based on[^46].
