...
- 标题: "Title"
- 作者:
  - 01 Keon Lee - KRAFTON.AI
  - 02 Dong Won Kim - KRAFTON.AI
  - 03 Jaehyeon Kim - KRAFTON.AI
  - 04 Jaewoong Cho - KRAFTON.AI
- 链接:
...
- 文件:
  - [ArXiv](_PDF/2406.11427v1__DiTTo-TTS__Efficient_&_Scalable_Zero-Shot_TTS_with_Diffusion_Transformer.pdf)
  - [Publication]
...
Hence, their application to TTS often requires a complex pipeline, incorporating speech domain-specific modeling such as phoneme and duration ([Grad-TTS](../Acoustic/2021.05.13_Grad-TTS.md), [NaturalSpeech2](../../Models/Diffusion/2023.04.18_NaturalSpeech2.md)).
Without these components, generation performance tends to be suboptimal ([Simple-TTS](../_tmp/Simple-TTS.md), [E3-TTS](2023.11.02_E3_TTS.md)), while their inclusion hinders the model efficiency and scalability.
...
Consequently, we adopt the [Diffusion Transformer (DiT)](2022.12.19_DiT.md) to TTS, naming our method ***DiTTo-TTS*** (or simply ***DiTTo***).
...
The base-sized ***DiTTo*** surpasses a state-of-the-art autoregressive model ([CLaM-TTS](../../Models/SpeechLM/2024.04.03_CLaM-TTS.md)), offering an inference speed 4.6 times faster and a model size 3.84 times smaller.
...
Efforts to replicate LLM’s capability in different modalities have shown progress, including vision [22], [23], [24] and audio ([VALL-E](../SpeechLM/2023.01.05_VALL-E.md); [AudioLM](../SpeechLM/2022.09.07_AudioLM.md); [SPEAR-TTS](../SpeechLM/2023.02.07_SPEAR-TTS.md); [Qwen-Audio](../SpeechLM/2023.11.14_Qwen-Audio.md)).
[VALL-E](../SpeechLM/2023.01.05_VALL-E.md) employs [EnCodec](../SpeechCodec/2022.10.24_EnCodec.md) for speech-to-token mapping, posing TTS tasks as AR language modeling tasks, thus enabling zero-shot capabilities in the speech domain.
[CLaM-TTS](../../Models/SpeechLM/2024.04.03_CLaM-TTS.md) introduces Mel-VAE to achieve superior token length compression, and enables a language model to predict multiple tokens simultaneously.
Although this approach removes the need for cascaded modeling ([VALL-E](../SpeechLM/2023.01.05_VALL-E.md)) to manage the number of token streams, their resource-intensive inference processes limit their applications [30].
...
[Simple-TTS](../_tmp/Simple-TTS.md) simplifies the data preparation and training process by removing auxiliary modules and the need for phoneme-level durations.
...
### Neural Audio Codec
Neural audio codecs, which effectively compress various types of audio using neural networks, are used as part of many TTS systems ([NaturalSpeech2](../../Models/Diffusion/2023.04.18_NaturalSpeech2.md); [CLaM-TTS](../../Models/SpeechLM/2024.04.03_CLaM-TTS.md); [VALL-E](../SpeechLM/2023.01.05_VALL-E.md)).
...
With a similar architecture, by focusing the compression specifically on speech rather than general audio signals, Mel-VAE ([CLaM-TTS](../../Models/SpeechLM/2024.04.03_CLaM-TTS.md)) achieves approximately 10.76 Hz latents by compressing the mel-spectrogram.
This reduction significantly lowers the computational cost of the speech generation module.
Another research direction of improving neural audio codecs for TTS systems is injecting semantic information using large language models (LLMs) ([SpeechTokenizer](../../Models/SpeechCodec/2023.08.31_SpeechTokenizer.md)).
...
While diffusion models can operate directly on real-world data, many of them are applied in the latent space ([LDM](2021.12.20_LDM.md); [8], [10], [47].
Thanks to the reduced dimensionality, this approach improves computational efficiency and output quality by allowing diffusion models to focus on the semantic information of the data while the autoencoder handles the high-frequency details that are less perceptible ([LDM](2021.12.20_LDM.md)).
...
where $Loss_{NAC}(\psi)$ indicate the loss function which is used when the neural audio codec is pre-trained ([CLaM-TTS](../../Models/SpeechLM/2024.04.03_CLaM-TTS.md)).
...
We apply classifier-free guidance (CFG) [36] and adopt the diffusion noise schedule from [Simple-TTS](../_tmp/Simple-TTS.md).
...
我们采用分类无监督指导 (CFG) [36], 并采用 [Simple-TTS](../_tmp/Simple-TTS.md) 中的扩散噪声调度.
...
For the latent space, we employ Mel-VAE introduced in ([CLaM-TTS](../../Models/SpeechLM/2024.04.03_CLaM-TTS.md)) which is able to compress audio sequences approximately seven times more than [EnCodec](../SpeechCodec/2022.10.24_EnCodec.md), yet maintaining superior quality.
...
对于隐空间, 我们采用 Mel-VAE ([CLaM-TTS](../../Models/SpeechLM/2024.04.03_CLaM-TTS.md)) 作为替代品, 它能够在音频序列的近似压缩率上比 [EnCodec](../SpeechCodec/2022.10.24_EnCodec.md) 提高了 7 倍, 同时保持优秀的质量.
