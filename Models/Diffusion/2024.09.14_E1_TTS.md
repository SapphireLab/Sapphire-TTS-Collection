...
Non-autoregressive (NAR) text-to-speech (TTS) models [^01] generate speech from text in parallel, synthesizing all speech units simultaneously.
This enables faster inference compared to autoregressive (AR) models, which generate speech one unit at a time.
Most NAR TTS models incorporate duration predictors in their architecture and rely on alignment supervision[^02], [^03], [^04].
...
Several pioneering studies[^05], [^06] have proposed implicit-duration non-autoregressive (ID-NAR) TTS models that eliminate the need for alignment supervision or explicit duration prediction.
...
Recently, several diffusion-based[^07] ID-NAR TTS models[^08], [^09], [^10], [^11], [^12], [^13], [^14] have been proposed, demonstrating state-of-the-art naturalness and speaker similarity in zero-shot text-to-speech[^15].
...
Diffusion distillation techniques[^16] can be employed to reduce the number of network evaluations in sampling from diffusion models.
Most distillation techniques are based on approximating the ODE sampling trajectories of the teacher model.
For example, ProDiff[^17] applied Progressive Distillation[^18], CoMoSpeech[^19] and FlashSpeech[^20] applied Consistency Distillation[^21], and VoiceFlow[^22] and ReFlow-TTS[^23] applied Rectified Flow[^24].
Recently, a different family of distillation methods was discovered[^25], [^26], which directly approximates and minimizes various divergences between the generator's sample distribution and the data distribution.
Compared to ODE trajectory-based methods, the student model can match or even outperform the diffusion teacher model[^26], as the distilled one-step generator does not suffer from error accumulation in diffusion sampling.

In this work, we distill a diffusion-based ID-NAR TTS model into a one-step generator with recently proposed distribution matching distillation[^25], [^26] method.
...
Various formulations of diffusion models exist in the literature[^24], [^07], most of which are equivalent to learning a neural network that approximates the score function $s_p(x_t, t) :=\nabla_{x_t} \log p_{t}(x_t)$ at each time $t$.
...
Consider the following weighted average of KL divergence at all noise scales[^25], [^27]:
$$
...
Although the generator $g_\theta$ can be randomly initialized in theory, initializing $g_\theta$ with $s_\phi$ leads to faster convergence and better performance[^25].
Several studies[^28], [^29] have discovered that pretrained diffusion models already possess latent one-step generation capabilities.
Moreover, it is possible to convert them into one-step generators by tuning only a fraction of the parameters[^28], [^29], such as the normalization layers.

While distribution matching distillation resembles generative adversarial networks (GANs)[^30] in its requirement for alternating optimization, it has been empirically observed[^26] to be significantly more stable, requiring minimal tuning and avoiding the mode collapse issue that often hinders GAN training.
...
Rectified Flow[^24] is capable of constructing a neural ordinary differential equation (ODE):
$$
...
***E1 TTS*** is similar to the acoustic model introduced in[^31] with the modification that all speech tokens are generated simultaneously in the first stage.
Further more, we applied DMD to convert the two diffusion transformers~(DiTs)[^32] to one-step generators, removing all iterative sampling from the inference pipeline.
...
Directly training generative models on low-level speech representations such as Mel spectrograms[^11] and raw waveforms[^08] is resource-consuming due to the long sequence lengths.
...
Please refer to[^31] for further details regarding the training process and model architecture.
...
We adopted rotary positional embedding (RoPE)[^33] in all Transformer blocks in ***E1 TTS***.
...
Similar designs have proven effective in other ID-NAR TTS models[^34], [^05].
...
We trained a duration predictor similar to the one in[^35].
The rough alignment between text and speech tokens is first obtained by training an aligner based on RAD-TTS[^36].
...
We observed that minimizing the L1 difference in total duration[^06], [^05] works better than directly minimizing phoneme-level durations, resulting in a lower total duration error.
...
The inference process for text-based speech editing[^37] with E1~TTS involves several steps.
...
In this case, the total duration of the synthesized speech can be estimated by assuming it is a fixed multiple of the input text length as done in[^34].
Or we can train a total-duration predictor following[^06], [^05], which does not require text unit durations as supervision.
...
**Datasets**: All components of the evaluated ***E1 TTS*** model were trained on the LibriTTS[^38] dataset, which is a multi-speaker English speech corpus containing 585 hours of speech audio from over 2,300 speakers.
We used an open-source BigVGAN[^39] checkpoint\footnote{\texttt{github.com/NVIDIA/BigVGAN}  \texttt{(bigvgan\_24khz\_100band)}} to generate 24 kHz speech waveforms from Mel spectrograms.
...
Another baseline system is ARDiT-TTS$_\text{B=1, DMD}$ from[^31], which has a similar design and shares the Mel-to-Token encoder and Token-to-Mel decoder with ***E1 TTS***.
...
The EMA rate has a significant impact on sample quality[^40].
For the DMD training of the DiT models, we adopted the two-timescale update rule~(TTUR)[^41].
The generators are updated once per 10 updates of the score model.
We observed that TTUR stabilized DMD training significantly as discovered in [^26].
...
Following the evaluation protocol in [^31], [^42], we evaluate the zero-shot TTS performance of all models on {test set B}, derived from the {test-clean} subset of LibriTTS, containing 500 test cases.
...
We adopt the 100 hard sentences proposed in [^43] for evaluation.
...
We generated 100 random samples from zero-shot TTS models using the same target text(**VITS**[^44]), "How much variation is there?", and an identical speech prompt\footnote{\texttt{LibriTTS, test-clean, 8555\_284449\_000044\_000000}}.
...
SimpleSpeech[^10] observed poor sample quality when training a similar model on the LibriTTS dataset, indicating that the quantity of training data plays a crucial role.
SESD[^09]~highlighted the significance of the noise schedule[^45] in diffusion training and inference.
...
[^01]: TTSSurvey
[^02]: [DeepVoice](../System/2017.02.25_DeepVoice.md)
[^03]: [FastPitch](../Acoustic/2020.06.11_FastPitch.md)
[^04]: [FastSpeech](../Acoustic/2019.05.22_FastSpeech.md)
[^05]: VARA-TTS
[^06]: [Flow-TTS](../Acoustic/2020.04.09_Flow-TTS.md)
[^07]: ScoreSDE
[^08]: [E3 TTS](2023.11.02_E3_TTS.md)
[^09]: [SESD](2024.09.01_SESD.md)
[^10]: [SimpleSpeech](2024.06.04_SimpleSpeech.md)
[^11]: [E2 TTS](2024.06.26_E2_TTS.md)
[^12]: [Seed-TTS](../SpeechLM/2024.06.04_Seed-TTS.md)
[^13]: [DiTTo-TTS](2024.06.17_DiTTo-TTS.md)
[^14]: Mapache
[^15]: [YourTTS](../E2E/2021.12.04_YourTTS.md)
[^16]: BlogDistillation
[^17]: ProDiff
[^18]: ProgressiveDistillation
[^19]: [CoMoSpeech](2023.05.11_CoMoSpeech.md)
[^20]: [FlashSpeech](2024.04.23_FlashSpeech.md)
[^21]: ConsistencyModel
[^22]: [VoiceFlow](2023.09.10_VoiceFlow.md)
[^23]: [ReFlow-TTS](../Flow/2023.09.29_ReFlow-TTS.md)
[^24]: RectifiedFlow
[^25]: DiffInstruct
[^26]: DMD2
[^27]: VSD
[^28]: LCMLoRA
[^29]: InnateOneStep
[^30]: [GAN](../_Basis/2014.06.10_GAN.md)
[^31]: [ARDiT](2024.06.08_ARDiT.md)
[^32]: [DiT](2022.12.19_DiT.md)
[^33]: [Roformer](../_Transformer/2021.04.20_RoFormer.md)
[^34]: [ParaNet](../Acoustic/2019.05.21_ParaNet.md)
[^35]: [Voicebox](2023.06.23_Voicebox.md)
[^36]: RAD-TTS
[^37]: DiffVoice
[^38]: [LibriTTS](../../Datasets/2019.04.05_LibriTTS.md)
[^39]: [BigVGAN](../Vocoder/2022.06.09_BigVGAN.md)
[^40]: EDM2
[^41]: TTUR
[^42]: [UniCATS](2023.06.13_UniCATS.md)
[^43]: [StyleTTS2](../SpeechLM/2023.06.13_StyleTTS2.md)
[^44]: [CosyVoice](../SpeechLM/ST2S/2024.07.07_CosyVoice.md)
[^45]: [ELLA-V](../SpeechLM/ST2S/2024.01.14_ELLA-V.md)
[^46]: [VITS](../E2E/2021.06.11_VITS.md)
[^47]: NoiseScheduleBlog
