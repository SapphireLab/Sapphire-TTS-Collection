...
- 作者:
  ...
  - [Jaekyoung Bae](../../Authors/Jaekyoung_Bae.md)
- 机构:
  - [Kakao Enterprise](../../Institutions/KOR-Kakao_Enterprise.md)
- 时间:
  - 预印时间: 2020.10.12 ArXiv v1
...
- 链接:
  ...
  - [DOI](https://proceedings.neurips.cc/paper_files/paper/2020/hash/c5d736809766d46260d816d8dbc9eb44-Abstract.html)
  - [Github](https://github.com/jik876/hifi-gan)
- 标签:
  - [声码器](../../Tags/Vocoder.md)
  - [生成对抗网络](../../Tags/Model_GAN.md)
...
1. predicting a low resolution intermediate representation such as mel-spectrograms (Shen et al., 2018, Ping et al., 2017, Li et al., 2019) or linguistic features ([Oord et al.(WaveNet·2016)](../../Models/TTS3_Vocoder/2016.09.12_WaveNet.md)) from text,
2. synthesizing raw waveform audio from the intermediate representation ([Oord et al.(WaveNet·2016)](../../Models/TTS3_Vocoder/2016.09.12_WaveNet.md), Prenger et al., 2019, Kumar et al., 2019).
...
[WaveNet (2016)](../../Models/TTS3_Vocoder/2016.09.12_WaveNet.md) is an autoregressive (AR) convolutional neural network that demonstrates the ability of neural network based methods to surpass conventional methods in quality..
...
[Parallel WaveNet (2019)](2019.10.25_Parallel_WaveGAN.md) is an inverse autoregressive flow (IAF) that is trained to minimize its Kullback-Leibler divergence from a pre-trained WaveNet called a teacher.
Compared to the teacher model, it improves the synthesis speed to 1,000 times or more, without quality degradation.
[WaveGlow (2018)](2018.10.31_WaveGlow.md) eliminates the need for distilling a teacher model, and simplifies the learning process through maximum likelihood estimation by employing efficient bijective flows based on Glow (Kingma and Dhariwal, 2018).
...
Instead of mel-spectrograms, [GAN-TTS (2019)](2019.09.25_GAN-TTS.md) successfully generates high quality raw audio waveforms from linguistic features through multiple discriminators operating on different window sizes.
...
As in previous work (Mathieu et al., 2015, Isola et al., 2017, [MelGAN (2019)](2019.10.08_MelGAN.md)), noise is not given to the generator as an additional input.
...
Additionally, to capture consecutive patterns and long-term dependencies, we use the **multi-scale discriminator (MSD)** proposed in [MelGAN (2019)](2019.10.08_MelGAN.md), which consecutively evaluates audio samples at different levels.
...
The architecture of MSD is drawn from that of [MelGAN (2019)](2019.10.08_MelGAN.md).
...
For previous work using multi-discriminator architecture such as MPD and MSD, [Binkowski et al. (GAN-TTS·2019)](2019.09.25_GAN-TTS.md)’s work can also be referred.
...
The feature matching loss is a learned similarity metric measured by the difference in features of the discriminator between a ground truth sample and a generated sample (Larsen et al., 2016, [MelGAN (2019)](2019.10.08_MelGAN.md)).
As it was successfully adopted to speech synthesis ([MelGAN (2019)](2019.10.08_MelGAN.md)), we use it as an additional loss to train the generator.
...
The synthesis speed was measured on GPU and CPU environments according to the recent research trends regarding the efficiency of neural networks ([MelGAN (2019)](2019.10.08_MelGAN.md), Zhai et al., 2020, Tan et al., 2020).
...
(MelGAN and MoL WaveNet were excluded from comparison for their differences in pre-processing such as frequency clipping. Mismatch in the input representation led to producing low quality audio when combined with Tacotron2.)
