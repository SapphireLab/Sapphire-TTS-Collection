...
- 机构:
  - [Gemelo AI](../../Institutions/GemoloAI.md)
- 时间:
  - 2023.06.01 ArXiv v1
  - 2023.10.03 ArXiv v2
  - 2024.05.29 ArXiv v3
...
> While vocoders evolved significantly (Kawahara et al., 1999; Morise et al., 2016), they tended to oversimplify speech production, generating a distinctive ”buzzy” sound and thus compromising the naturalness of the speech.

> A significant breakthrough in speech synthesis was achieved with the introduction of WaveNet (Oord et al., 2016), a deep generative model for raw audio waveforms.
...
> Autoregressive models, like WaveNet, generate audio samples sequentially, conditioning each new sample on all previously generated ones (Mehri et al., 2016; Kalchbrenner et al., 2018; Valin & Skoglund, 2019).
> On the other hand, non-autoregressive models generate all samples independently, parallelizing the process and making it more computationally efficient (Oord et al., 2018; Prenger et al., 2019; Donahue et al., 2018).
...
语音合成领域的一个重要突破是WaveNet (Oord et al., 2016) 的引入, 这是一种用于原始音频波形的深度生成模型.
...
自回归模型, 如WaveNet, 按顺序生成音频样本, 每个新样本都依赖于所有先前生成的样本 (Mehri et al., 2016; Kalchbrenner et al., 2018; Valin & Skoglund, 2019) .
...
> Progress was made with the introduction of advanced critics, such as the multi-scale discriminator (MSD) (Kumar et al., 2019) and the multi-period discriminator (MPD) (Kong et al., 2020).
...
随着先进批评器的引入, 如多尺度判别器 (MSD)  (Kumar et al., 2019) 和多周期判别器 (MPD)  (Kong et al., 2020), 取得了进展.
...
> This design, known as an isotropic architecture, has been found to work well in various settings, including Transformer (Vaswani et al., 2017).
...
这种设计, 称为各向同性架构, 已在包括Transformer (Vaswani et al., 2017) 在内的各种设置中被发现效果良好.
这种方法对音频合成也可能特别有益.
...
> #### Data 
> The models are trained on the [LibriTTS dataset](../../Datasets/LibriTTS.md), from which we use the entire training subset (both train-clean and train-other).
...
> ##### Baseline Methods 
> Our proposed model, Vocos, is compared to: iSTFTNet (Kaneko et al., 2022), BigVGAN (Lee et al., 2022), and HiFi-GAN (Kong et al., 2020).
...
#### 数据
模型在[LibriTTS数据集](../../Datasets/LibriTTS.md)上进行训练, 我们使用了整个训练子集 (包括train-clean和train-other) .
...
#### 基线方法
我们提出的模型 Vocos 与以下模型进行了比较：iSTFTNet (Kaneko et al., 2022), BigVGAN (Lee et al., 2022), 和 HiFi-GAN (Kong et al., 2020) .
...
> In this section, we draw a comparison with EnCodec (D´efossez et al., 2022), an open-source neural audio codec, which follows a typical time-domain GAN vocoder architecture and uses Residual Vector Quantization (RVQ) (Zeghidour et al., 2021) to compress the latent space.
...
> In EnCodec, dedicated discriminators are trained for each bandwidth.
...
> We utilize the open-source model checkpoint of EnCodec operating at 24 kHz.
> To align with EnCodec, we scale down Vocos to match its parameter count (7.9M) and train it on clean speech segments sourced from the DNS Challenge (Dubey et al., 2022).
> Our evaluation, conducted on the DAPS dataset (Mysore, 2014) and detailed in Table 4, reveals that despite EnCodec’s reconstruction artifacts not significantly impacting PESQ and Periodicity scores, they are considerably reflected in the perceptual score, as denoted by UTMOS.
> In this regard, Vocos notably outperforms EnCodec.
...
> Bark (Suno AI, 2023), a widely recognized open-source model, leverages a GPT-style, decoder-only architecture, with EnCodec’s 6kbps audio tokens serving as its vocabulary.
> Vocos trained to reconstruct EnCodec tokens can effectively serve as a drop-in replacement vocoder for Bark.
...
尽管传统上, 神经声码器从梅尔尺度频谱图重建音频波形——这种方法在许多语音合成管道中被广泛采用——但最近的研究已经开始利用学习到的特征 (Siuzdak et al., 2022), 通常以量化形式 (Borsos et al., 2022) .

在本节中, 我们将与 EnCodec (D'efossez et al., 2022) 进行比较, EnCodec 是一个开源的神经音频编解码器, 它遵循典型的时域GAN声码器架构, 并使用残差向量量化 (RVQ)  (Zeghidour et al., 2021) 来压缩潜在空间.
RVQ通过多层向量量化级联, 迭代量化前一阶段的残差, 形成多级结构, 从而支持多个带宽目标.
在 EnCodec 中, 为每个带宽训练专门的判别器.
相比之下, 我们已经将 Vocos 适配为具有投影判别器 (Miyato & Koyama, 2018) 的条件 GAN, 并在生成器中集成了自适应层归一化 (Huang & Belongie, 2017) .

#### 音频重建
我们使用运行在 24 kHz 的 EnCodec 的开源模型检查点.
为了与 EnCodec 对齐, 我们将 Vocos 缩减到与其参数计数 (7.9M) 匹配, 并使用来自 DNS 挑战 (Dubey et al., 2022) 的干净语音段对其进行训练.
我们的评估在 DAPS 数据集 (Mysore, 2014) 上进行, 并在表 04 中详细说明, 揭示尽管 EnCodec 的重建伪影对 PESQ 和周期性分数的影响不大, 但它们在感知分数上得到了显著反映, 如 UTMOS 所示.

![](Images/2023.03.01.Vocos.Tab.04.png)

在这方面, Vocos 明显优于 EnCodec.
...
Bark (Suno AI, 2023) 是一个广泛认可的开源模型, 它利用了GPT风格的仅解码器架构, 其中 EnCodec 的 6 kbps 音频令牌作为其词汇.
训练用于重建 EnCodec 令牌的 Vocos 可以有效地作为 Bark 的即插即用替代声码器.
