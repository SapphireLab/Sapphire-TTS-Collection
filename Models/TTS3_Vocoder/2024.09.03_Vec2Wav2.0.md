...
> Discretizing speech into `tokens` has prevailed in speech generative tasks, such as text-to-speech (TTS) (VQTTS; [VALL-E [2]](../Speech_LLM/2023.01.05_VALL-E.md); [UniCATS [3]](../Diffusion/UniCATS.md); [FACodec [4]](../Speech_Neural_Codec/2024.03.05_FACodec.md)), in the era of large language models (LLMs). 
...
> A typical instance is the idea of `prompted vocoders` proposed by CTX-vec2wav ([UniCATS [3]](../Diffusion/UniCATS.md)) which is later verified in VC~\cite{li2024sef}.
...
> The overall architecture inherits the frontend-generator framework of CTX-vec2wav ([UniCATS [3]](../Diffusion/UniCATS.md)), where the input discrete speech tokens are first fed to a Conformer-based frontend module to soften the discreteness, before a vocoder generator that finally outputs the realistic waveforms.
...
> In the frontend module, the prompt embeddings are utilized by the position-agnostic cross-attention mechanism ([UniCATS [3]](../Diffusion/UniCATS.md); li2024sef), which does not apply positional encoding to the query sequence.
...
> Instead of using mel-spectrogram to provide timbre information from the reference prompt, we use a pretrained [WavLM [11]](../Speech_Representaion/2021.10.26_WavLM.md) model as a timbre feature extractor owing to its widely-verified advantage on speaker verification~\cite{jung2024espnet,superb}. 
> We freeze the [WavLM](../Speech_Representaion/2021.10.26_WavLM.md) model in training and only use the output feature at a certain location of its Transformer blocks.
...
> We inherit the multi-scale discriminators (MSD) and multi-period discriminators (MPD) from [HiFi-GAN [38]](2020.10.12_HiFi-GAN.md). 
...
> This loss is added with a certain coefficient, and following vec2wav~\cite{VQTTS} and CTX-vec2wav ([UniCATS [3]](../Diffusion/UniCATS.md)), we cancel it after warming up.
...
> Other hyper-parameters follow CTX-vec2wav ([UniCATS [3]](../Diffusion/UniCATS.md)).
...
> Note that the training data in all baselines either includes LibriTTS or is magnitudes larger (e.g. [FACodec](../Speech_Neural_Codec/2024.03.05_FACodec.md)), so the comparisons are fair enough.
