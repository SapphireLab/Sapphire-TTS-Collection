...
Secondly, the scarcity of speech data compared to text leads to highly imbalanced speech-text training data and in turn causes catastrophic forgetting ([Freeze-Omni](../SpokenDialogue/2024.11.01_Freeze-Omni.md)).
...
This results in intermediate outputs that still contain text, as seen in models such as [LLaMA-Omni](2024.09.10_LLaMA-Omni.md) and [Freeze-Omni](../SpokenDialogue/2024.11.01_Freeze-Omni.md).
...
[SALMONN](../SpokenDialogue/2023.10.20_SALMONN.md) is another speech-text LLM for generic audio understanding, by integrating separate speech and audio encoders with a pre-trained text LLM through Q-Former and adopting LoRA for modality alignment.
...
[LLaMA-Omni](2024.09.10_LLaMA-Omni.md) and [Mini-Omni](../SpokenDialogue/2024.08.27_Mini-Omni.md) are two recent end-to-end frameworks that have not been trained for full-duplex communication.
...
Among collaborative systems, [VITA](../SpokenDialogue/2024.08.09_VITA.md) runs two models at the same time, namely, the generation model and the monitoring model, to support full-duplex communication.
...
Among end-to-end full-duplex models, the early work of [dGSLM](../SpokenDialogue/2022.03.30_dGSLM.md) proposes a Siamese architecture to jointly process both audio token streams of user speech and system speech.
...
Different from these native multimodal models, our ***MinMo*** is in the category of **aligned multimodal models**, which also include Llama-Omni, [Mini-Omni2](2024.10.15_Mini-Omni2.md), and [Freeze-Omni](../SpokenDialogue/2024.11.01_Freeze-Omni.md).
...
[Freeze-Omni](../SpokenDialogue/2024.11.01_Freeze-Omni.md) is a speech-to-speech model that freezes the pre-trained text LLM to reserve the LLM's capabilities.
...
The **Voice Encoder** is initialized with the pretrained SenseVoice-large encoder module ([FunAudioLLM](2024.07.04_FunAudioLLM.md)), which provides robust voice understanding capabilities and supports multilingual speech recognition, emotion recognition, and audio event detection.
...
We utilize the streaming audio generation mechanism of [CosyVoice 2](2024.12.13_CosyVoice2.md), due to its low latency and competitive speech synthesis performance.
...
For reconstructing waveforms from the speech tokens, we utilize an off-the-shelf streaming token2wav synthesizer, as described by [CosyVoice 2](2024.12.13_CosyVoice2.md).
...
- For text chat data primarily sourced from [Alpaca](../../Datasets/Alpaca.md) and [ShareGPT [URL]](https://sharegpt.com/), we utilize the zero-shot in-context generation method from [CosyVoice](2024.07.07_CosyVoice.md) to convert user text into user speech.
...
Post-processing is based on a modified [Whisper](2022.12.06_Whisper.md) normalizer, with modifications primarily for improved number normalization.
...
As shown in Table \ref{tab:multilingual_asr_performance}, ***MinMo*** achieves superior ASR performance on most test sets across various languages, compared to [Whisper Large v3](2022.12.06_Whisper.md) and [Qwen2-Audio](2024.07.15_Qwen2-Audio.md).
...
For the Chinese cases, we utilized the Paraformer-zh model \citep{Paraformer}, while the English cases were processed using [Whisper-large V3](2022.12.06_Whisper.md).
...
We observed that ***MinMo***'s voice decoder has slightly reduced content consistency and speech quality on the Chinese test sets compared to the TTS baseline, [CosyVoice 2.0-SFT](2024.12.13_CosyVoice2.md).
...
Additionally, since the original format of these three datasets is text-based, during testing, we use [CosyVoice 2](2024.12.13_CosyVoice2.md) to perform TTS synthesis on the input question texts.
