...
Recent advances in ASR have led to several large-scale models, such as **Whisper**[^1], **Qwen-Audio**[^2],  **Qwen2-Audio**[^3], **SenseVoice**[^4], and **Seed-ASR**[^5], showing a paradigm shift from end-to-end models with millions of parameters (**Survey** by Li et al. (2022)[^6]; **Survey** by Prabhavalkar et al. (2023)[^7]) to larger-scale models (**Whisper**[^1]; **FunAudioLLM**[^4]; **Google USM**[^8]; **TouchASP**[^9]) and the integration of pre-trained text LLMs (**Qwen-Audio**[^2]; **Qwen2-Audio**[^3]; **Seed-ASR**[^5]; **Speech-LLaMA**[^10]; **AudioPaLM**[^11]; [Second-Pass Reranking & Deep LLM-Fusion](2023.06.28_Prompting_LLMs_for_Zero-Shot_Domain_Adaptation_in_Speech_Recognition.md)[^12]; **SLM**[^13]; **COSMIC**[^14]; [^15], **SALM**[^16]; [^17], [^18], **SLAM-ASR**[^19]).
...
***FireRedASR-LLM*** utilizes an innovative Encoder-Adapter-LLM framework (**Seed-ASR**[^5]; **Speech-LLaMA**[^10]; [^18]; **SLAM-ASR**[^19]), comprising 8.3B parameters to push the boundary of recognition accuracy.
...
It employs an Attention-based Encoder-Decoder (AED) architecture ([^22], **LAS**[^23]) with up to 1.1B parameters.
...
Furthermore, we incorporated a simple yet effective adapter design inspired by recent works (**Qwen-Audio**[^2]; **Qwen2-Audio**[^3]; **Seed-ASR**[^5]; **Speech-LLaMA**[^10]; **AudioPaLM**[^11]; [Second-Pass Reranking & Deep LLM-Fusion](2023.06.28_Prompting_LLMs_for_Zero-Shot_Domain_Adaptation_in_Speech_Recognition.md)[^12]; **SLM**[^13]; **COSMIC**[^14]; [^15]; **SALM**[^16]; [^17]; [^18]; **SLAM-ASR**[^19]), facilitating efficient model adaptation and research iteration.
</td><td>
