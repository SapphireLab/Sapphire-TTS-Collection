...
Some recent VC methods [^baade2024neural], [^li2024sef] have attempted to integrate semantic information into their frameworks, but their effectiveness remains limited.
...
Diffusion-based VC models, such as Diff-VC [^popov2021diffvc] and StableVC [^yao2024stablevc], generate high-quality speech by directly modeling audio signals, which allows for high-fidelity conversion while benefiting from recent optimizations that enable faster inference.
These models now generate speech in a single step, significantly improving efficiency.
On the other hand, LLM-driven approaches, including OpenVoice V2 [^openvoice], LM-VC [^wang2023lm], and DualVC3 [^ning2024dualvc3], operate on acoustic tokens rather than raw audio, which may lead to some information loss but provides advantages in handling long-form speech.
...
Furthermore, conversation-oriented models such as Moshi [^moshi], Mini-Omni [^omni], and Llama-Omni [^fang2024llama] have demonstrated the potential of integrating text generation with speech transformation.
...
Inspired by this, we propose \textbf{StarVC} (\textbf{S}peech-\textbf{T}ext \textbf{A}uto-\textbf{R}egressive \textbf{V}oice \textbf{C}onversion), a novel VC framework that unifies speech conversion and text generation within one auto-regressive structure.
...
As shown in Figure \ref{fig:speech_production}, the system consists of four main components: Semantic Feature Extractor, Speaker Feature Extractor, Language Model and Acoustic Decoder.
...
Whisper [^whisper] is a widely used ASR model trained on diverse speech data, making it well-suited for capturing linguistic content in voice conversion.
...
ERes2Net [^ERes2Net] is designed for speaker embedding extraction, incorporating multi-scale feature fusion for improved robustness.
As shown in Figure \ref{fig:speech_production_2}, we extract FBank features $B_t \in \mathbb{R}^{T_f \times F_f}$, where $T_f$ is the number of frames and $F_f$ is the number of FBank bins.
...
We apply delay method adopted in MusicGen [^music_gen], which effectively conditions acoustic tokens on prior text tokens.
...
In addition, we also employed the teacher forcing strategy [^williams1989teacher] and the RoPE [^rope] encoding strategy to optimize the training effect of the model.
...
While Mimi allows flexible control over the number of output code layers, we follow the same setting as Moshi [^moshi] and select eight layers.
...
Compared to other codecs such as Encodec [^defossez2022encodec], SNAC [^siuzdak2024snac], DAC [^dac] and SemantiCodec [^liu2024semanticodec], Mimi demonstrates superior performance in both speech quality and speaker similarity, making it more effective for voice conversion.
...
- CosyVoice [^du2024cosyvoice], [^du2024cosyvoice2]: A diffusion-based speech generation approach from Alibaba, featuring a VC variant.
- OpenVoice V2 [^openvoice]: A large-scale, zero-shot VC model built upon YourTTS [^casanova2022yourtts].
- TriAAN-VC [^park2023triaan]: A deep-learning-based framework for any-to-any VC, focusing on disentangling linguistic content and target speaker characteristics.
\end{itemize}
...
- \emph{WER (Word Error Rate)}: Assesses how accurately the linguistic content is preserved by transcribing the converted audio (using Whisper-large [^whisper]) and comparing it to the ground-truth text.
...
[^stylianou1998continuous]:
[^bargum2023reimagining]:
[^sisman2020overview]:
[^chen2022wavlm]:
[^hsu2021hubert]:
