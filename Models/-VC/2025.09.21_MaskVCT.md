...
Recent VC models [^resynthesis], [^nansy], [^nansypp], [^freevc], [^diffhiervc], [^maskgct] used pre-trained self-supervised models' representations as linguistic features, however, these features contains rich information to reconstruct the mel-spectrogram itself while they mentioned "disentangled" [^nansy].

Some models are conditioned on the target's pitch.
Those *pitch-conditioned* models [^nansy], [^nansypp], [^diffhiervc] have high controllability with frame-wise pitch, but a robust pitch predictor is needed.
Meanwhile, *pitch-unconditioned* models [^freevc], [^maskgct], [^genvc] generate speech without the need for an explicit pitch input.
One noticeable point is that pitch-unconditioned models can generate speech from tonal languages, such as Chinese [^maskgct], due to the entangled pitch information in pre-trained linguistic features.
...
Many models mainly focus on converting the vocal timbre [^nansypp], [^maskgct] of the source to that of the target.

Recently, *zero-shot* models have emerged, allowing conversion to previously unseen speaker voices without additional fine-tuning [^nansy], [^freevc], [^diffhiervc], [^maskgct].

Another trend is leveraging codec-based language models (CodecLMs).
Targeting discretized tokens from neural audio codecs [^dac], [^Defossez2022EnCodec], [^naturalspeech3], CodecLMs cast generation as a Transformer-friendly classification task [^transformer] and reconstruct via autoregressive prediction [^valle] or masked modeling [^maskgct], typically conditioned on a short 3-second speaker prompt.
...
By using syllabic representations from SylBoost [^syllablelm], ***MaskVCT*** gains flexibility in phoneme selection and accent variation within each segment.
By leveraging multiple classifier-free guidances (CFGs) [^cfg], we allow for a dynamic balance between conditions at inference time, effectively creating a highly controllable system.
...
Self-supervised speech representations provide time-aligned and fast-varying phonetic features [^ss_phonetic] so VC models built on them tend to reproduce source-aligned speech.
Table [tab:results_main](#tab:results_main) shows that even pitch-unconditioned, linguistics-only-conditioned models [^freevc], [^maskgct] retain high pitch correlation with the source, indicating pitch leakage.
This motivates a coarser and pitch-stripped representation for source-pitch-agnostic conversion.

Recent works propose syllabic representations [^syllablelm], [^sylber] that discretize speech into slow-varying units by quantizing both within-vector and across adjacent frames.
...
We use the discrete tokens from the pre-trained syllabic model SylBoost and its quantizer [^syllablelm], and train learnable embeddings for each token.

Continuous and discrete linguistic representations offer complementary strengths: continuous features improve intelligibility and phoneme alignment, whereas discrete syllable tokens better preserve target timbre/speaker similarity [^sylber] and mitigate source speaker traits leakage; conversely, continuous features can weaken speaker fidelity [^vevo].
...
We map continuous pitch to a vector using sinusoidal embedding [^transformer], making the system agnostic to pitch-extractor resolution and requiring no fine-tuning when changing extractors.
...
We extract all pitch contours with a rate of 50 Hz using Praat [^praat].

### Speaker Conditioning

To perform zero-shot VC, we implemented a speaker prompt mechanism similar to Vall-E [^valle].
...
Inspired by masked language modeling [^bert],
MaskGIT [^maskgit] frames image synthesis as discrete token unmasking.
Extending this idea, similar to CodecLMs [^valle], [^musicgen], non-autoregressive codec-based models adopt masked modeling to speech [^maskgct], [^soundstorm],
typically on residual vector quantization (RVQ) codecs [^dac], [^Defossez2022EnCodec].
Recently, MaskGCT [^maskgct] achieved a state-of-the-art zero-shot text-to-speech with a masked model, and its S2A module can be utilized as a VC model.
...
During training, we randomly sample a masking time step $u\sim\mathcal{U}[0,1]$, a target codebook layer $c\sim p(c) = 1 -\frac{2(c+1)}{C(C+1)}$ [^maskgct], and cosine scheduled mask $\mathbf{M}_{u,c}\sim\mathrm{Bernoulli}\left(\cos\left(\pi u/2\right)\right)$ [^maskgit].
...
CFG [^cfg] is a commonly used technique to improve conditional generation at inference time and is also used in CodecLMs that require fine time-aligned conditioning for accurate acoustic reconstruction [^valle], [^maskgct].
There are several studies that applied dual CFG [^voiceldm], [^dualspeech],
notably, DualSpeech [^dualspeech] uses dual CFG to jointly steer intelligibility and speaker fidelity via phoneme and speaker-aware phoneme embeddings.
...
Within masked generative modeling framework [^maskgct], [^maskgit], [^soundstorm], we define speech synthesis as a conditional probability with three factors as speaker prompt $\mathbf{A}_p$, linguistic content $\mathbf{L}$, and pitch $\mathbf{P}$ as $\log p_{\theta}(\mathbf{A}_{n}|\mathbf{A}_{n+1},
...
Following DualSpeech [^dualspeech], the speaker condition couples the prompt with linguistic as $\log p_\theta(\mathbf{A}_n\mid \mathbf{A}_{n+1},\mathbf{A}_p,\mathbf{L},\varnothing)$.
...
We utilize a simple Transformer encoder-based architecture [^transformer], especially PreLN with rotary positional embedding.
...
For our baseline, we utilized the official implementation and checkpoint of Diff-HierVC\footnote{\url{https://github.com/hayeong0/Diff-HierVC}} [^diffhiervc], FACodec\footnote{\url{https://hf.co/amphion/naturalspeech3_facodec}} [^naturalspeech3], [^amphion], MaskGCT\footnote{\url{https://hf.co/amphion/MaskGCT}} [^maskgct],  FreeVC\footnote{\url{https://github.com/OlaWod/FreeVC}} [^freevc], GenVC\footnote{\url{https://github.com/caizexin/GenVC}}[^genvc].
For accent conversion, we only tested the models that achieved the best objective metrics in the main evaluation.
Table [tab:baselines](#tab:baselines) shows the number of parameters and training dataset hours for each models.
...
We employ the official checkpoint of the DAC 16 kHz [^dac] as a codec, from which we extract 9 codebook indices to encode each audio frame.
For syllabic representations, we use SylBoost 8.33 Hz [^syllablelm], which has a minimum of tokens.
To improve robustness to phase-related inconsistency [^inconsistency], we apply PhaseAug [^phaseaug] to all inputs before DAC encoder.
Although SylBoost yields consistent features, self-reconstruction can leak information [^nansy]; thus, following NANSY [^nansy], we pitch-shift clean speech to create perturbed variants and train with a 50\% property for each case.
...
We apply SpecAugment [^specaugment] to combined embeddings by randomly masking 10\% of the channel dimension of the input vectors.
...
Specifically, we utilized the train-clean subsets of LibriTTS‑R [^librittsr], the train subset of MLS‑en [^mls], VCTK [^vctk], LibriHeavy-Large [^librilight], [^libriheavy], the clean subset of HiFi‑TTS [^hifitts], LJSpeech [^ljspeech], and the speech subset of RAVDESS [^ravdess].

We utilized the test-clean subset of LibriTTS-R [^librittsr] to test the general performance of VC models.
...
To assess accent conversion performance, we utilize the non-native English speech corpus L2-ARCTIC [^l2arctic].
Two conversion directions are considered:
{Libri$\rightarrow$L2}: converting from near-native LibriTTS-R speech to accented speech, and {L2$\rightarrow$L2}: conversion between distinct accents.
...
<a id="tab:results_main">Evaluation results for MaskVCT and the baseline methods on the LibriTTS-R test-clean dataset.</a>

<a id="tab:accent">Accent Conversion Results, the audios are converted from **Libri**TTS-R or **L2**-ARCTIC to L2-ARCTIC.</a>

### Inference Setup

During every iteration, we select the top probability positions by Gumbel-Softmax [^gumbel] following the number of cosine mask schedules.
...
We assess intelligibility with WER/CER from Whisper large-v3 [^whisper], speaker similarity (S-SIM) as the cosine between WavLM speaker embeddings of the converted utterance and the target prompt [^wavlm], pitch tracking via the F0 Pearson correlation (FPC), and automatic quality via UTMOS v2 [^utmosv2].
...
For the accent conversion, we measure accent similarity (A-SIM) by CommonAccent [^commonaccent].
...
Table [tab:results_main](#tab:results_main) presents results on the LibriTTS-R test-clean subset.
...
Moreover, because the syllabic quantizer is determined by K-means clustering [^syllablelm], [^sylber], misreadings caused by incorrect syllable mappings cannot be easily corrected.
...
[^valle]: valle
[^syllablelm]: syllablelm
[^cfg]: cfg
[^ss_phonetic]: ss_phonetic
[^sylber]: sylber
[^vevo]: vevo
[^praat]: praat
[^bert]: bert
