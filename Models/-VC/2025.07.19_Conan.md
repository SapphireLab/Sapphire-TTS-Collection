...
We randomly use the source and reference speakers in the train subset of the LibriTTS dataset [^Zen2019Libritts], which contains 586 hours of English speech from 2,456 speakers. 
All speech is downsampled to 16 kHz. 
To evaluate Conan’s performance in zero‐shot scenarios, we include the VCTK dataset, which comprises 44 hours of clean speech from 110 English speakers. 
For a fair comparison, we reference the StreamVC [^Yang2024Streamvc] data‐partitioning strategy.

We use the test‐clean subset of LibriTTS as the source speech and then randomly select 6 unseen speakers from the VCTK dataset to form the test set.

**Implementation Details.**

We set the sample rate to 16 kHz, the window size to 1024, the hop size to 320, and the number of mel bins to 80 to derive mel-spectrograms from raw waveforms.

We set the chunk size to 80 ms and employ a 6-layer Emformer with 2 right context chunks as the **full setting**.

For the **fast setting**, the chunk size is 20 ms, and a 3-layer Emformer without right context chunks is employed.

The default size of the codebook for CVQ is 128.

**Training Details.**

We train our model using 1 NVIDIA A100 GPU. 
The Adam optimizer is used with $\beta_1 = 0.9$ and $\beta_2 = 0.98$. 
The Stream Content Extractor trains for 80k steps, the main model trains for 160k steps, and the Causal Shuffle Vocoder trains for 600k steps to reach convergence.

**Evaluation Details.**

We objectively and subjectively evaluate our model based on three criteria: **real‐time content accuracy, quality, and speaker similarity**.

Content accuracy is measured by Word Error Rate (WER) and Character Error Rate (CER) using a HuBERT‐Large ASR model \footnote{\url{https://github.com/resemble-ai/Resemblyzer}}. 
Speaker similarity (SIM) is assessed with Resemblyzer \footnote{\url{https://huggingface.co/facebook/hubert-large-ls960-ft}}. 
For subjective metrics, we conduct mean opinion score (MOS) and comparative mean opinion score (CMOS) evaluations.

Both metrics are rated on a 1–5 scale and reported with 95\% confidence intervals.

Specifically, we use MOS‑Q to judge quality (clarity and naturalness) and MOS‑S to assess speaker similarity (timbre and styles) [^Zhang2024Gtsinger].

For CMOS‑Q and CMOS‑S, listeners compare pairs of speech samples produced by different systems.

We randomly selected at least 20 pairs with more than 6 unseen speakers for each subjective evaluation, with at least 15 listeners per pair. 
All participants were fairly compensated and provided informed consent for the use of their responses in scientific research.

### Main Results

<a id="tab: main">Evaluation results of Conan and baseline models, including both objective (WER, CER, SIM) and subjective (MOS-S, MOS-Q) evaluations.

All baselines, except for StreamVC, are evaluated in an offline setting.</a>

We use several popular voice‐conversion models, including VQMIVC [^Wang2021Vqmivc], BNE‐PPG‐VC [^Tang2023Any-to-Any], Diff‐VCTK [^Liu2021Any-to-Many], QuickVC [^Guo2023QuickVC], and StreamVC [^Yang2024Streamvc] as baselines. 
Since StreamVC is not open-sourced, to ensure a fair comparison, we adopt the same dataset split and objective evaluation protocols that they used. 
We rely on the objective metrics reported in their paper and perform subjective assessments based on their demo pages. 

As shown in Table [tab: main](#tab: main), Conan in the full setting delivers the highest speaker similarity (SIM, MOS-S), indicating that the Adaptive Style Encoder and timbre encoder effectively model the speaker identity and therefore achieve superior zero-shot similarity.

Conan also attains the best quality (MOS-Q), demonstrating that the streaming framework enhances synthesis quality.

For content accuracy, Conan records the lowest WER, while its CER is slightly higher than that of StreamVC.

This is mainly because StreamVC reuses the source pitch and energy values, giving it a CER advantage.

Finally, the metrics for Source (Vocoder) and Reference (Vocoder) are both excellent, further highlighting the outstanding online reconstruction capability of the Causal Shuffle Vocoder.

<a id="tab: rtf">Latency and RTF for Conan’s full and fast settings tested on a single A100 80 GB GPU. 
Latency includes the sum of each module’s delay, plus the chunk size, and the right context.</a>

As shown in Table [tab: rtf](#tab: rtf), in the faster setting, Conan uses a 20 ms chunk size to perform strictly causal, chunk‐wise online voice conversion with only 37 ms of latency, yet still achieves high performance metrics. 
Under Conan’s full setting with 80 ms chunk size and 2 right context chunks, we achieve high quality while still meeting real‐time latency constraints.

### Ablation Study

<a id="tab: abl">Evaluation results for the ablation study.

SCE denotes Stream Content Encoder, ASE is Adaptive Style Encoder, and CSV represents Causal Shuffle Vocoder.</a>

As shown in Table [tab: abl](#tab: abl), we conduct ablation studies to demonstrate the effectiveness of various design choices within the full setting.

When the original HuBERT is used instead of the Stream Content Encoder, we observe a substantial deterioration in both synthesis quality and WER, underscoring the incompatibility of HuBERT with streaming scenarios.

Furthermore, when right‐context chunks are omitted, all metrics worsen, indicating that the size of the lookahead can be adjusted as a trade‐off between efficiency and quality.

Furthermore, removing the Adaptive Style Encoder results in a marked decrease in MOS-S, confirming that our approach effectively captures speaker identity. 
Finally, substituting the Causal Shuffle Vocoder with a zero-padding causal HiFi-GAN leads to a decline in MOS-Q, as the zero padding causal HiFi-GAN causes a decline in synthesis quality.

## 5·Conclusion

In this paper, we introduce Conan, a streaming voice conversion method that preserves the linguistic content of a source utterance while adopting the speaker information of a reference utterance. 
At its core, Conan employs the Streaming Content Extractor based on Emformer to encode source audio with minimal latency, ensuring semantic information is captured in real time. 
To transfer the reference speaker identity, Adaptive Style Encoder analyzes the reference speech and derives detailed speaker representations that guide the conversion process. 
Finally, Causal Shuffle Vocoder, built upon a fully causal HiFiGAN architecture enhanced with pixel-shuffle operations, reconstructs high-fidelity audio with low generation delay. 
Experimental results show that Conan surpasses existing baseline approaches, achieving better performance according to both subjective and objective metrics.
...
[^Wang2024Streamvoice]: Streamvoice: Streamable Context-Aware Language Modeling for Real-Time Zero-Shot Voice Conversion. arXiv:2401.11053.
[^Chen2022Controlvc]: Controlvc: Zero-Shot Voice Conversion With Time-Varying Controls on Pitch and Speed. arXiv:2209.11866.
[^Yang2024Streamvc]: Streamvc: Real-Time Low-Latency Voice Conversion. ICASSP 2024-2024 IEEE International Conference on Acoustics, Speech and Signal Processing (ICASSP) 2024.
...
[^Yang2022Streamable]: Streamable Speech Representation Disentanglement and Multi-Level Prosody Modeling for Live One-Shot Voice Conversion.. Interspeech 2022.
[^Zhang2025TCSinger]: TCSinger 2: Customizable Multilingual Zero-Shot Singing Voice Synthesis. arXiv:2505.14910.
[^Zhang2025ISDrama]: ISDrama: Immersive Spatial Drama Generation Through Multimodal Prompting. arXiv:2504.20630.
...
[^Chen2021Adaspeech]: Adaspeech: Adaptive Text to Speech for Custom Voice. arXiv:2103.00993.
[^Quamer2024End-to-End]: End-to-End Streaming Model for Low-Latency Speech Anonymization. 2024 IEEE Spoken Language Technology Workshop (SLT) 2024.
...
[^Ebbers2021Contrastive]: Contrastive Predictive Coding Supported Factorized Variational Autoencoder for Unsupervised Learning of Disentangled Speech Representations. ICASSP 2021-2021 IEEE International Conference on Acoustics, Speech and Signal Processing (ICASSP) 2021.
...
[^Hayashi2022Investigation]: An Investigation of Streaming Non-Autoregressive Sequence-to-Sequence Voice Conversion. ICASSP 2022-2022 IEEE International Conference on Acoustics, Speech and Signal Processing (ICASSP) 2022.
[^Ren2020Fastspeech]: Fastspeech 2: Fast and High-Quality End-to-End Text to Speech. arXiv:2006.04558.
[^Hayashi2021Non-Autoregressive]: Non-Autoregressive Sequence-to-Sequence Voice Conversion. ICASSP 2021-2021 IEEE International Conference on Acoustics, Speech and Signal Processing (ICASSP) 2021.
[^Kameoka2021FastS2S-VC]: FastS2S-VC: Streaming Non-Autoregressive Sequence-to-Sequence Voice Conversion. arXiv:2104.06900.
[^Chen2023Streaming]: Streaming Voice Conversion via Intermediate Bottleneck Features and Non-Streaming Teacher Guidance. ICASSP 2023-2023 IEEE International Conference on Acoustics, Speech and Signal Processing (ICASSP) 2023.
[^Ning2024Dualvc]: Dualvc 2: Dynamic Masked Convolution for Unified Streaming and Non-Streaming Voice Conversion. ICASSP 2024-2024 IEEE International Conference on Acoustics, Speech and Signal Processing (ICASSP) 2024.
[^Wang2023Alo-Vc]: Alo-Vc: Any-to-Any Low-Latency One-Shot Voice Conversion. arXiv:2306.01100.
...
[^Choi2020Attentron]: Attentron: Few-Shot Text-to-Speech Utilizing Attention-Based Variable-Length Embedding. arXiv:2005.08484.
[^Kumar2021Normalization]: Normalization Driven Zero-Shot Multi-Speaker Speech Synthesis.. Interspeech 2021.
[^Za{\i}di2021Daft-Exprt]: Daft-Exprt: Robust Prosody Transfer Across Speakers for Expressive Speech Synthesis. arXiv:2108.02271.
[^Huang2022Generspeech]: Generspeech: Towards Style Transfer for Generalizable Out-of-Domain Text-to-Speech Synthesis. arXiv:2205.07211.
[^Lee2021Styler]: Styler: Style Factor Modeling With Rapidity and Robustness via Speech Decomposition for Expressive and Controllable Neural Text to Speech. arXiv:2103.09474.
[^Jiang2024Mega-TTS]: Mega-TTS 2: Boosting Prompting Mechanisms for Zero-Shot Speech Synthesis. The Twelfth International Conference on Learning Representations 2024.
...
[^{\L}a{\'n}cucki2021Fastpitch]: Fastpitch: Parallel Text-to-Speech With Pitch Prediction. ICASSP 2021-2021 IEEE International Conference on Acoustics, Speech and Signal Processing (ICASSP) 2021.
[^Zhang2024TCSinger]: TCSinger: Zero-Shot Singing Voice Synthesis With Style Transfer and Multi-Level Style Control. Proceedings of the 2024 Conference on Empirical Methods in Natural Language Processing 2024.
[^Zheng2023Online]: Online Clustered Codebook. Proceedings of the IEEE/CVF International Conference on Computer Vision 2023.
[^Zhang2025Versatile]: Versatile Framework for Song Generation With Prompt-Based Control. arXiv:2504.19062.
[^Van2017Neural]: Neural Discrete Representation Learning. Advances in Neural Information Processing Systems 2017.
[^Zhang2024StyleSinger]: StyleSinger: Style Transfer for Out-of-Domain Singing Voice Synthesis. Proceedings of the AAAI Conference on Artificial Intelligence 2024.
...
[^Zhang2024Gtsinger]: Gtsinger: A Global Multi-Technique Singing Corpus With Realistic Music Scores for All Singing Tasks. arXiv:2409.13832.
[^Tang2023Any-to-Any]: Any-to-Any Generation via Composable Diffusion. Advances in Neural Information Processing Systems 2023.
...
[^Guo2023QuickVC]: QuickVC: Any-to-Many Voice Conversion Using Inverse Short-Time Fourier Transform for Faster Conversion. arXiv:2302.08296.