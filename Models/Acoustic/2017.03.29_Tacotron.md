...
Modern text-to-speech (TTS) pipelines are complex (["Text-to-Speech Synthesis (2009) "](../../_Books/2009_Text-to-Speech_Synthesis.md)) .
...
Moreover, unlike end-to-end speech recognition [LAS](../ASR/2015.08.05_LAS.md) or machine translation [GNMT](../../Models/NMT/2016.09.26_GNMT.md), TTS outputs are continuous, and output sequences are usually much longer than those of the input.
...
[WaveNet](../Vocoder/2016.09.12_WaveNet.md) is a powerful generative model of audio.
...
However, [Char2Wav](../../Models/E2E/2017.02.18_Char2Wav.md) still predicts vocoder parameters before using a [SampleRNN](../Vocoder/2016.12.22_SampleRNN.md) neural vocoder, whereas ***Tacotron*** directly predicts raw spectrogram.
Also, their seq2seq and [SampleRNN](../Vocoder/2016.12.22_SampleRNN.md) models need to be separately pre-trained, but our model can be trained from scratch.
...
![](Images/2017.03.29.Tacotron.Fig.02.png)
...
Since we use [Griffin-Lim](../Vocoder/1984.04.00_Griffin-Lim.md) as the synthesizer, the post-processing net learns to predict spectral magnitude sampled on a linear-frequency scale.
...
It could be used to predict alternative targets such as vocoder parameters, or as a WaveNetlike neural vocoder ([WaveNet](../Vocoder/2016.09.12_WaveNet.md), [SampleRNN](../Vocoder/2016.12.22_SampleRNN.md), [Deep Voice](../../Models/TTS0_System/2017.02.25_DeepVoice.md)) that synthesizes waveform samples directly.

We use the [Griffin-Lim](../Vocoder/1984.04.00_Griffin-Lim.md) algorithm to synthesize waveform from the predicted spectrogram.
We found that raising the predicted magnitudes by a power of $1.2$ before feeding to [Griffin-Lim](../Vocoder/1984.04.00_Griffin-Lim.md) reduces artifacts, likely due to its harmonic enhancement effect.
We observed that [Griffin-Lim](../Vocoder/1984.04.00_Griffin-Lim.md) converges after $50$ iterations (in fact, about $30$ iterations seems to be enough), which is reasonably fast.
We implemented [Griffin-Lim](../Vocoder/1984.04.00_Griffin-Lim.md) in [TensorFlow](../_Basis/2016.03.14_TensorFlow.md) hence it’s also part of the model.
While GriffinLim is differentiable (it does not have trainable weights), we do not impose any loss on it in this work.
We emphasize that our choice of [Griffin-Lim](../Vocoder/1984.04.00_Griffin-Lim.md) is for simplicity; while it already yields strong results, developing a fast and high-quality trainable spectrogram to waveform inverter is ongoing work.
...
We use the [Adam](../../Modules/Optimization/2014.12.22_Adam.md) optimizer with learning rate decay, which starts from $0.001$ and is reduced to $0.0005$, $0.0003$, and $0.0001$ after $500K$, $1M$ and $2M$ global steps, respectively.
...
Given the strong baselines and the artifacts introduced by the [Griffin-Lim](../Vocoder/1984.04.00_Griffin-Lim.md) synthesis, this represents a very promising result.
...
Our output layer, attention module, loss function, and [Griffin-Lim](../Vocoder/1984.04.00_Griffin-Lim.md) -based waveform synthesizer are all ripe for improvement.
For example, it’s well known that [Griffin-Lim](../Vocoder/1984.04.00_Griffin-Lim.md) outputs may have audible artifacts.
