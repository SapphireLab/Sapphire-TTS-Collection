...
The growing demands for modern speech interaction systems, highlighted by GPT-4o ([^20],[^21]), underscore the need for open-source, high-performance Mandarin ASR solutions.
...
***FireRedASR-LLM*** utilizes an innovative Encoder-Adapter-LLM framework ([Seed-ASR](2024.07.05_Seed-ASR.md)[^5]; [Speech-LLaMA](2023.07.08_Speech-LLaMA.md)[^10]; [^18]; [^19]), comprising 8.3B parameters to push the boundary of recognition accuracy.
...
It employs an Attention-based Encoder-Decoder (AED) architecture ([^22],[^23]) with up to 1.1B parameters.
...
***FireRedASR-AED*** adopts an end-to-end architecture that combines a Conformer-based Encoder (Enc) with a Transformer-based Decoder (Dec) ([^24], [^25]).
...
According to the recently released report [^36], existing models including Baichuan-omni, Qwen2-Audio-Instruct, and Whisper-Large-v3 (with parameter sizes of 7B+, 7B+, and 1.5B respectively) achieve average CERs of 6.7\%, 9.9\%, and 44\% on KeSpeech.
...
[^16]: chen2024salm
[^17]: lakomkin2024end
[^18]: geng2024unveiling
[^19]: ma2024embarrassingly
[^20]: OpenAI. Hello gpt-4o. https://openai.com/index/hello-gpt-4o/, 2024.
[^21]: Gpt-4o system card
[^22]: bahdanau2016end
[^23]: chan2016listen
[^24]: gulati2020conformer
[^25]: Transformer
[^26]: dai2019transformer
...
[^37]: LibriSpeech
[^38]: SpecAugment
[^39]: Sequence Transduction with Recurrent Neural Networks.
[^40]: Two-Pass End-to-End Speech Recognition.