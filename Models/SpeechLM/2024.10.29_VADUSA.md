...
如图 2 所示, 结果表明, 带有 4 或 6 个草稿头的 ***VADUSA-wt*** 在候选选择范围 {64, 96, 128} 上的性能变化.
与预期相反, 增加候选数量并不能提高加速性能, 这是因为接受更多候选导致的计算成本增加.
容忍性机制在 1 和 4 之间变化, 4 或 6 个头使用 ***VADUSA-wot*** 或 ***VADUSA-wt*** 策略训练.
如图 3 所示, 容忍性机制在一般的 ***VADUSA*** 配置中表现良好, 较大的容忍值获得更好的加速.
