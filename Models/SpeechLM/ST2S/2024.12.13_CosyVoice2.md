...
In recent years, neural text-to-speech (TTS) synthesis models have garnered significant attention for surpassing traditional concatenative and statistical parametric methods ([Tacotron [1]]; [Tacotron2 [2]]; [DeepVoice3 [3]]; [Clarinet [4]](../E2E/2018.07.19_ClariNet.md); [FastSpeech [5]]; [Transformer-TTS [6]]; [FastSpeech2 [7]]).
These models have achieved high fidelity and naturalness on pre-defined specific speakers.
Recent studies show that zero-shot TTS models are able to synthesize speech for any speaker by imitating the timbre, prosody and style of a reference speech ([VALL-E [8]](ST2S/2023.01.05_VALL-E.md)).
...
Codec language models utilize a speech codec model to extract discrete speech representation ([SoundStream [9]](../SpeechCodec/2021.07.07_SoundStream.md); [EnCodec [10]](../SpeechCodec/2022.10.24_EnCodec.md); [FunCodec [11]](../SpeechCodec/2023.09.14_FunCodec.md)) and employ an autoregressive ([VALL-E [8]](ST2S/2023.01.05_VALL-E.md); [SPEAR-TTS [12]](ST2S/2023.02.07_SPEAR-TTS.md); [ELLA-V [13]](ST2S/2024.01.14_ELLA-V.md); [VALL-T [14]](ST2S/2024.01.25_VALL-T.md); [RALL-E [15]](ST2S/2024.04.04_RALL-E.md); [VALL-E 2 [16]](ST2S/2024.06.08_VALL-E_2.md); [VALL-E R [17]](ST2S/2024.06.12_VALL-E_R.md)) or masked ([MaskGCT [18]](ST2S/2024.09.01_MaskGCT.md)) language model to predict the speech tokens, which are then synthesized to waveforms via codec vocoders ([WaveNeXt [19]](../Vocoder/2023.12.16_WaveNeXt.md); [Vocos [20]](../Vocoder/2023.03.01_Vocos.md)).
Continuous speech representations are also explored in [MELLE [21]](ST2S/2024.07.11_MELLE.md).
Language model-based TTS can generate varied and prosody-consistent speech via autoregressive sampling.

Inspired by advances in image generation, denoising diffusion ([DDPM [22]](../Diffusion/2020.06.19_DDPM.md); [Song et al. [23]](../Diffusion/2020.11.26_Score-Based_Generative_Modeling_through_SDE.md)) and [flow matching models [24]](../Diffusion/2022.10.06_Flow_Matching.md) have been introduced into non-autoregressive (NAR) speech synthesis.
Early diffusion-based TTS models required duration prediction for each text (phone) to address the length disparity between text and speech features ([Voicebox [25]](2023.06.23_VoiceBox.md); [NaturalSpeech3 [26]](../Diffusion/2024.03.05_NaturalSpeech3.md); [VoiceFlow [27]](../Diffusion/2023.09.10_VoiceFlow.md); [Matcha-TTS [28]](../Diffusion/2023.09.06_Matcha-TTS.md)).
However, this rigid alignment can affect naturalness, resulting in flat prosody.
To mitigate this issue, cross-attention and Diffusion Transformers (DiT) have been introduced into NAR TTS models ([E3- TS [29]](../Diffusion/2023.11.02_E3_TTS.md); [Ditto-TTS [30]](../Diffusion/2024.06.17_DiTTo-TTS.md)).
Recent research indicates simpler approaches for text-speech alignment in NAR TTS models, such as [E2 TTS [31]](../Diffusion/2024.06.26_E2_TTS.md), [F5-TTS [32]](../Diffusion/2024.10.09_F5-TTS.md) and [Seed-TTS [33]](2024.06.04_Seed-TTS.md).
...
Hybrid systems combine the text-to-codec language model and codec-to-feature diffusion model ([Seed-TTS [33]](2024.06.04_Seed-TTS.md); [CosyVoice [34]](ST2S/2024.07.07_CosyVoice.md); [FireRedTTS [35]](2024.09.05_FireRedTTS.md)).
...
This results in high latency, negatively impacting user experience in applications like voice chat ([GPT-4o [36]](../SpokenDialogue/2024.09.06_GPT-4o.md); [SpeechGPT [37]](../SpokenDialogue/2023.05.18_SpeechGPT.md)).
To address this issue, streaming synthesis has been explored for language model-based zero-shot TTS models ([LiveSpeech [38]](2024.06.05_LiveSpeech.md); [LiveSpeech2 [39]](2024.10.01_LiveSpeech2.md); [BASE TTS [40]](ST2S/2024.02.12_BASE-TTS.md); [LLM2Speech [41]](2023.09.20_LLM2Speech.md)), but diffusion-based TTS models and hybrid systems lack well-established streaming solutions.

Building on the success of [CosyVoice [34]](ST2S/2024.07.07_CosyVoice.md), we introduce ***CosyVoice2***, a streaming zero-shot TTS model with improved prosody naturalness, content consistency, and speaker similarity.
...
***CosyVoice2*** builds on the similar design philosophy of its predecessor ([CosyVoice [34]](ST2S/2024.07.07_CosyVoice.md)) by separating the semantic and acoustic information of speech signals and modeling them independently.
...
As shown in Figure \ref{fig:overall} (a), we insert the [finite scalar quantization (FSQ) module [42]](../../Modules/VQ/FSQ.md)  into the encoder of SenseVoice-Large ASR model ([FunAudioLLM [43]](2024.07.04_FunAudioLLM.md)).
At the training stage, the input speech $X$ goes through the $\text{Encoder}_1$ to obtain the intermediate representations, where $\text{Encoder}_1$ consists of six Transformer blocks with the [rotary positional embedding [44]](../../Modules/PositionEmb/RoPE.md).
Then, the intermediate representations are fed into the FSQ module for quantization, and the quantized representations are passed through the rest of SenseVoice-Large modules, including $\text{Encoder}_2$ and $\text{ASR\ Decoder}$, to predict the posterior probabilities of corresponding text tokens.
...
In ***CosyVoice2***, the pre-trained textual LLM, [Qwen2.5-0.5B [45]](../TextLM/2024.12.19_Qwen2.5.md), is used as the text-speech language model to generate the speech tokens autoregressively with the input text as a prompt.
...
- For the **Non-Streaming** mode, the `start of sequence` `\circled{S}`, all text tokens, `turn of speech` token `\circled{T}`, all speech tokens and the `end of sequence` `\circled{E}` are concatenated sequentially as shown in the bottom of Figure \ref{fig:ULM}.
...
In the non-streaming mode, the prompt and to-synthesize text tokens are concatenated as the whole entity, and the prompt speech tokens are treated as the pre-generated results and are fixed: `\circled{S}, prompt\_text, text, \circled{T}, prompt\_speech`.
The autoregressive generation of LM is started from such sequence until the `End of sequence` token `\circled{E}` is detected.
...
Then, we mix it up with the prompt speech tokens on the ratio of $N$:$M$: `\circled{S}, mixed\_text\_speech, \circled{T}, remaining\_speech`.
...
Thus, the initial sequence is very simple: `\circled{S}, text, \circled{T}`.
Starting from this, the text-speech LM can generate speech tokens autoregressively until `\circled{T}`.
- **SFT, Streaming**: In the streaming mode of SFT, we start the speech generation from the following sequence: `\circled{S}, first\_N\_text`.
...
Besides, we also train the model on both conditional and non-conditional situations to enable the classifier-free guidance (CFG) ([CFG [46]](../Diffusion/2022.07.26_Classifier-Free_Guidance.md); [iDDPM [47]](../Diffusion/2021.02.18_iDDPM.md); [Voicebox [48]](2023.06.23_VoiceBox.md)) at the inference stage:
...
The first-package latency is an important metric for streaming synthesis models, which significantly affects the user experience especially in LLM-based voice chat applications, such as [GPT-4o [36]](../SpokenDialogue/2024.09.06_GPT-4o.md).
...
We use WER and SS to distinguish preferred sample $x^w$ and rejected samples $x^l$ and optimize the TTS system with [direct preference optimization (DPO) [49]](../../Modules/RLHF/2023.05.29_DPO.md) as follow:
...
***CosyVoice2*** shares the same training data as its previous version ([CosyVoice [34]](ST2S/2024.07.07_CosyVoice.md)).
We first collect the speech-only data with internal speech processing tools.
Subsequently, the [Paraformer [50]](../ASR/2022.06.16_Paraformer.md)  and SenseVoice ([FunAudioLLM [43]](2024.07.04_FunAudioLLM.md)) are employed to generate pseudo text labels for Chinese and other languages, respectively.
...
The first one is constructed from the test-clean set of Librispeech corpus ([UniCATS [51]](../Diffusion/2023.06.13_UniCATS.md)), denoted as test-clean.
...
As for the speaker similarity (SS), we employ the [ERes2Net model [52]](../_Basis/2023.05.22_ERes2Net.md) to extract speaker embeddings of prompt and generated utterances, and their raw cosine similarity is treated as the speaker similarity.
NMOS score ([DNSMOS P.385 [53]](../../Evaluations/2021.10.05_DNSMOS_P.385.md)) is used to evaluate the objective quality.

The second evaluation is conducted on the SEED test sets ([Seed-TTS [33]](2024.06.04_Seed-TTS.md)), which is widely used to evaluate recent TTS models, covering various text domains and reference speeches.
...
Subsequently, the [S3prl toolkit [55]](../../Evaluations/2024.04.15_SUPERB.md) is employed to further evaluate the speaker entanglement by performing speaker identification (SID) task.
...
We first evaluated our ***CosyVoice2*** models on a limited English text domain and compared it with several open-source models, such as [ChatTTS [Github] [56]](https://github.com/2noise/ChatTTS), [GPT-SoVITS [Github] [57]](https://github.com/RVC-Boss/GPT-SoVITS), [OpenVoice [58]](../E2E/2023.12.03_OpenVoice.md), [ParlerTTS [59]](2024.02.02_Parler-TTS.md), [EmotiVoice [Github] [60]](https://github.com/netease-youdao/EmotiVoice), and its predecessor [CosyVoice [34]](ST2S/2024.07.07_CosyVoice.md).
