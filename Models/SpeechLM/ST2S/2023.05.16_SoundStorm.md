...
***SoundStorm*** receives as input the semantic tokens of **AudioLM** and relies on bidirectional attention and confidence-based parallel decoding to generate the tokens of a neural audio codec.
Compared to the autoregressive generation approach of **AudioLM**, our model produces audio of the same quality and with higher consistency in voice and acoustic conditions, while being two orders of magnitude faster.
...
Modeling discrete representations of audio produced by neural codecs (**SoundStream**[^01]; **EnCodec**[^02]) makes the task of audio generation amenable to the powerful Transformer-based sequence-to-sequence modeling approaches (**Transformer**[^03]).
Casting unconditional and conditional audio generation as sequence-to-sequence modeling has unlocked rapid progress in speech continuation (**AudioLM**[^04]), text-to-speech (**VALL-E**[^05]; **SPEAR-TTS**[^06]), and general audio and music generation (**AudioGen**[^07]; [MusicLM (../../Music/2023.01.26_MusicLM.mdicLM.md)).
...
3) Custom architectures adapted to the special structure of the tokens produced by neural audio codecs (**AudioGen**[^07]; **VALL-E**[^05]; [RQ-Transformer (2022)]()).
...
Concretely, both **SoundStream**[^01] and **EnCodec**[^02] rely on Residual Vector Quantization (RVQ), where each compressed audio frame is quantized by a series of quantizers, with each quantizer operating on the residual of the previous one, and the number of quantizers control-ling the overall bitrate.
...
具体来说，**SoundStream**[^01] 和 **EnCodec**[^02] 都依赖于Residual Vector Quantization（RVQ），其中每个压缩的音频帧由一系列量化器进行量化，每个量化器在之前的量化器的残差上操作，量化器的数量控制总体比特率。
...
***SoundStorm*** relies on a bidirectional attention-based [Conformer (2020)]() that is trained to predict masked audio tokens produced by **SoundStream**[^01] given a condition-ing signal such as the semantic tokens of **AudioLM**[^04].
On the input side, it sums up the embeddings of the tokens corresponding to the same **SoundStream**[^01] frame, such that the internal sequence length for the self-attention is identical to the number of **SoundStream**[^01] frames, and independent of the number of quantizers in the RVQ.
...
***SoundStorm*** 依赖于一个基于双向注意力的Conformer（2020）, 该 Conformer 经过训练以预测由 **SoundStream**[^01] 生成的掩码音频标记, 给定一个条件信号, 例如 **AudioLM**[^04] 的语义标记.
在输入方面, 它将对应于同一 **SoundStream**[^01] 帧的标记的嵌入相加, 使得自注意力的内部序列长度与 **SoundStream**[^01] 帧的数量相同, 并且与 RVQ 中的量化器数量无关.
...
We demonstrate that ***SoundStorm*** can serve as **AudioLM**[^04]'s acoustic generator, replacing both **AudioLM**[^04]'s stage two (coarse acoustic model) and stage three (fine acoustic model).
***SoundStorm*** produces audio two orders of magnitude faster than **AudioLM**[^04]'s hierarchical autoregressive acoustic generator with matching quality and improved consistency in terms of speaker identity and acoustic conditions.
Furthermore, we show that ***SoundStorm***, coupled with the text-to-semantic modeling stage of **SPEAR-TTS**[^06], can synthesize high-quality, natural dialogues, allowing one to control the spoken content(via transcripts), speaker voices (via short voice prompts)and speaker turns (via transcript annotations).
When synthesizing dialogues of 30 seconds, we measure a runtime of 2 seconds on a single [TPU-v4 (2023)]().

我们展示了 ***SoundStorm*** 可以作为 **AudioLM**[^04] 的声学生成器, 取代 **AudioLM**[^04] 的第二阶段 (粗声学模型) 和第三阶段 (细声学模型)
***SoundStorm*** 生成的音频速度比 **AudioLM**[^04] 的分层自回归声学生成器快两个数量级, 同时保持相同质量和提高的语音和声学条件一致性.
此外, 我们展示了 ***SoundStorm*** 与 **SPEAR-TTS**[^06] 的文本到语义建模阶段相结合, 可以合成高质量自然的对话, 允许通过脚本控制说话内容, 通过短语音提示控制说话者声音和通过脚本注释控制说话者轮次.
...
Neural audio codecs (**SoundStream**[^01]; **EnCodec**[^02]), with their ability of reconstructing high-quality audio at very low bitrates, subsequently allowed for extending discrete modeling to audio signals as diverse as multi-speaker speech and piano ([AutoLM (2022)](); **SPEAR-TTS**[^06]), music ([MusicLM (20../../Music/2023.01.26_MusicLM.mdLM.md)) or sound effects (**AudioGen**[^07]).
In particular, **AudioLM**[^04] introduces a hierarchical sequence-to-sequence approach where high-level semantic tokens are generated as an intermediate representation, which is then used as a conditioning signal for predicting tokens of a **SoundStream**[^01]
While this hierarchical approach has demonstrated remarkable results for speech (**SPEAR-TTS**[^06]) and music modeling ([MusicLM (2023)](../../../Music/2023.01.26_MusicLM.md[SingSong (2023)]()), the computational cost of modeling flattened **SoundStream**[^01] tokens with self-attention scales quadratically with the sequence length and thus the bitrate of the neural codec, preventing these models from generating long-form audio with high quality.
...
特别是 AutoLM (2022) 引入了一种层次序列到序列方法, 高级别语义 token 作为中间表示, 之后作为条件信号用于预测 **SoundStream**[^01] 编解码器的 token.
虽然这一层次方法已经在语音和音乐建模方面获得了显著成果, 但使用自注意力对扁平化 **SoundStream**[^01] token 进行建模的计算成本随着序列长度和神经编解码器的比特率呈二次增长, 从而阻止了这些模型生成高质量的长形式音频.
...
Operating on such sequences, **AudioGen**[^07] proposes a Transformer with $Q$ separate heads for the different RVQ levels, predicting the tokens for an RVQ frame in parallel.
...
在处理这一序列时, **AudioGen**[^07] 提出了一个具有 $Q$ 个分别用于不同 RVQ 级别的独立头的 Transformer, 并并行预测 RVQ 帧的 tokens.
虽然为推理提供了显著的加速, 但作者发现对于文本到音频生成, 与具有相似比特率和重建质量但只有一级量化的神经音频编解码器的 token 序列建模相比, 这种方法性能较差.

**VALL-E**[^05] instead relies on a hybrid approach, where the tokens corresponding to the first RVQ level are predicted autoregressively, and the subsequent levels are produced non-autoregressively.
...
**VALL-E**[^05] 则依赖于混合方法, 其中对应第一个 RVQ 级别的 token 是自回归预测的, 后续级别以非自回归生成.
...
***SoundStorm*** receives as input a sequence of discrete tokens representing the conditioning signal and produces as output a sequence of **SoundStream**[^01] tokens, which can be decoded back to audio waveforms.
We assume that the conditioning signal is time-aligned with the **SoundStream**[^01] frames or can be upsampled to the same rate.
Such a conditioning signal is, for example, the semantic token sequence used in **AudioLM**[^04], **SPEAR-TTS**[^06], or [MusicLM../../Music/2023.01.26_MusicLM.mdmd), which makes our method a drop-in replacement for the acoustic generators of these models.

We leave the extension to other types of conditioning signals via cross-attention or to unconditional sampling for future work, and focus our presentation of ***SoundStorm*** as the acoustic generator within **AudioLM**[^04], replacing both **AudioLM**[^04]'s coarse and fine acoustic modeling stages.
...
At the input side, we interleave the time-aligned conditioning tokens with the **SoundStream**[^01] tokens at the frame level, embed the resulting sequence, sum the embeddings corresponding to the same frame, including the embedding of the conditioning token, and pass the resulting continuous embeddings to a Conformer.
Consequently, the sequence length for bidirectional self-attention in the Conformer is determined by the number of **SoundStream**[^01] frames (typically 50 per second), and thus is independent of the number of RVQ levels $Q$, allowing one to handle audio with length on the order of minutes.
At the output side, we use $Q$ dense layers as heads to produce the target **SoundStream**[^01] tokens.

在输入侧, 我们将时间对齐的条件 token 和 **SoundStream**[^01] token 在帧级别交错, 嵌入结果序列, 对应同一帧的嵌入求和, 包括条件 token 嵌入, 并将结果连续嵌入传递给 Conformer.
因此, Conformer 中使用的双向自注意力的序列长度由 **SoundStream**[^01] 帧数决定, 和 RVG 级别数无关, 从而可以处理长度为分钟级的音频.
在输出侧, 我们使用 $Q$ 个稠密层作为头来产生目标 **SoundStream**[^01] token.
...
Let $Y\in\{1,\cdots, C\}^{T\times Q}$ denote the **SoundStream**[^01] tokens, where $C$ indicates the codebook size used in each RVQ level out of the $Q$ levels.
...
条件 token 不会被掩盖.
记 **SoundStream**[^01] token 为 $Y\in\{1,\cdots, C\}^{T\times Q}$, $C$ 为在每个 RVQ 级别中使用的码本尺寸, 一共有 $Q$ 个级别.
我们的掩码方案如下:
- 采样提示的分隔符时间步 $t$;
- 采样当前 RVQ 级别 $q$;
...
Given a conditioning signal, our decoding scheme starts with all **SoundStream**[^01] tokens masked out except for the ones of the prompt (if provided).
...
给定一个条件信号, 我们的解码方案从除了提示之外的所有 **SoundStream**[^01] token 开始.
...
When compared to the acoustic generator of **AudioLM**[^04], ***SoundStorm*** is two orders of magnitude faster and achieves higher temporal consistency when generating long audio samples.
By combining a text-to-semantic token model similar to **SPEAR-TTS**[^06] with ***SoundStorm***, we can scale text-to-speech synthesis to longer contexts and generate natural dialogues with multiple speaker turns, controlling both the voices of the speakers and the generated content.

本项工作提出了 ***SoundStorm***, 一个可以高效地从离散条件 token 生成高质量音频的模型.
和 **AudioLM**[^04] 的声学生成器相比, ***SoundStorm*** 运行速度快两个数量级, 且在生成长音频样本时获得更高的时序一致性.
通过结合类似于 **SPEAR-TTS**[^06] 的文本到语义 token 模型, 能够将文本转语音合成扩展到更长内容和生成具有多个说话人轮次的自然对话, 同时控制说话人的声音和生成的内容.
...
In this work, we use it as a replacement for the acoustic generation pipeline of **AudioLM**[^04] and **SPEAR-TTS**[^06].
...
我们将其作为 **AudioLM**[^04] 和 **SPEAR-TTS**[^06] 的声学生成管道的替代品.
...
Thus, it is crucial to put in place safeguards against potential misuse: to this end, we have verified that the audio generated by ***SoundStorm*** remains detectable by a dedicated classifier (98.5%　using the same classifier as **AudioLM**[^04]).
Hence, as a component of a larger system, we believe that ***SoundStorm*** would be unlikely to introduce additional risks to those discussed previously by **AudioLM**[^04] and **SPEAR-TTS**[^06].
At the same time, relaxing the memory and computational requirements of **AudioLM**[^04] would make research in the domain of audio generation more accessible to a wider community.
...
因此, 必须采取措施防止潜在的滥用: 为此我们验证了 ***SoundStorm*** 生成的音频仍然可以被专用的分类器检测到 (**AudioLM**[^04]) 相同 的分类器, 检测率为 98.5%).
因此作为更大系统的一部分, 我们认为 ***SoundStorm*** 不太可能引入由 **AudioLM**[^04] 和 **SPEAR-TTS**[^06] 讨论的额外风险.
同时, 放松 **AudioLM**[^04] 的内存和计算要求将使得音频生成领域的研究对更广泛社区更可访问.
未来我们计划探索检测合成语音的其他方法, 例如音频水印, 以便任何潜在的产品使用这种技术严格遵循我们的负责任 AI 原则.

## References: 参考文献

[^01]: [**SoundStream**](../../SpeechCodec/2021.07.07_SoundStream.md)
[^02]: [**EnCodec**](../../SpeechCodec/2022.10.24_EnCodec.md)
[^03]: [**Transformer**](../../_Transformer/2017.06.12_Transformer.md)
[^04]: [**AudioLM**](2022.09.07_AudioLM.md)
[^05]:
[^06]:
[^07]: [**AudioGen**](2022.09.30_AudioGen.md)