...
- 在更近期的端到端全双工模型中, **Moshi**[^02], **GLM-4-Voice**[^03], **SyncLM**[^16], **IntrinsicVoice**[^17], 和 **Omni-Flatten**[^18] 是**原生多模态模型**.
...
和这些原生多模态模型不同, 我们的 ***MinMo*** 属于**对齐多模态模型**, 同类模型有 **LLaMA-Omni**, **Mini-Omni2**[^19], 和 **Freeze-Omni**[^05].
对齐多模态模型在集成声音能力的同时维持现有预训练文本大语言模型的能力.
- Mini-Omni2 引入了基于命令的中断机制以支持全双工对话, 然而, 它只在 ASR 上评估, 和 **Whisper**, VITA, Mini-Omni 进行对比.
- **Freeze-Omni**[^05] 是语音到语音模型, 将预训练文本 LLM 冻结来保留 LLM 的能力.
...
Collaborative systems, such as **ParalinGPT**[^20], **E-Chat**[^21], and **Spoken-LLM**[^22], incorporate paralinguistic features to enhance the understanding of acoustic information such as emotions.
...
Significant progresses have been made in text-style controllable TTS, such as **Salle**[^23], **PromptTTS**[^24], **PromptTTS2**[^25], **InstructTTS**[^26], and **ControlSpeech**[^27].
In contrast to these collaborative systems, **Moshi**[^02] uses a TTS engine with a single actor's voice and recorded monologues in over 70 speaking styles to synthesize training data to support understanding and generation of acoustic information in an end-to-end model.
**GLM-4-Voice**[^03] employs high-quality, multi-turn spoken dialogues tailored to specific speech style requirements, such as speaking rate, emotion, or dialect, to support style-controllable spoken dialogues.
...
Contrary to previous claims that aligned multimodal models such as **LLaMA-Omni** and Freeze-Omni only allow language models to control the content of speech but not the style and prosody (**GLM-4-Voice**[^03]), in this work, we propose a novel streaming voice decoder for the aligned multimodal model ***MinMo*** and find that this decoder enhances instruction-following capabilities and enables ***MinMo*** to generate speech corresponding to user-specified emotions, dialects, speaking rates, as well as mimicking specific voices.

</td><td>

相比基于文本的对话模型, 多模态口语对话模型的独特之处在于它们能够理解和生成文本内容之外的声学信息.
语音模态不仅包含了内容, 好包含了声学信息 (如情感, 方言, 语速等).
一个智能的多模态口语对话系统应该能够全面地理解输入语音中的声学信息 (如情感), 也能够理想地生成带有指定情感, 方言, 语速等的响应, 并模仿特定声音, 这样系统才能在交流中实现更深层次的理解和响应.

**协作系统** (如 **ParalinGPT**[^20], **E-Chat**[^21], 和 **Spoken-LLM**[^22]) 引入了副语言特征 (如情感) 来增强对声学信息的理解.
这些系统可以和风格可控的文本转语音系统结合以生成具有特定情感, 语速, 音量等的响应.
在风格可控的文本转语音方面已经有了显著的进步, 如 **Salle**[^23], **PromptTTS**[^24], **PromptTTS2**[^25], **InstructTTS**[^26], 和 **ControlSpeech**[^27].

和这些协作系统不同, **Moshi**[^02] 使用了具有一个单独的演员超过 70 种说话风格的声音和录制的独白的 TTS 引擎来合成训练数据, 以支持端到端模型的理解和生成声学信息.
**GLM-4-Voice**[^03] 采用高质量的多轮口语对话, 针对特定说话风格需求 (如语速, 情感, 方言) 进行了优化, 以支持风格可控的口语对话.
然而, 到目前为止, 先前的工作都没有表明对齐多模态模型能够支持风格可控的声音生成.

之前的对齐多模态模型 (如 **LLaMA-Omni** 和 Freeze-Omni) 只允许语言模型控制语音内容, 而不控制风格和语调 (**GLM-4-Voice**[^03]), 在本文中, 我们为对齐多模态模型提出了一个新的流式语音解码器并发现这一解码器增强了指令跟随能力, 并使得 ***MinMo*** 能够生成对应于用户指定的情感, 方言, 语速, 以及模仿特定声音的语音.
...
[Figure.03](#Fig.03) illustrates the model architecture of ***MinMo***.
...
The **Voice Encoder** is initialized with the pretrained SenseVoice-large encoder module (**FunAudioLLM**[^28]), which provides robust voice understanding capabilities and supports multilingual speech recognition, emotion recognition, and audio event detection.
...
We use the [pretrained Qwen2.5-7B-instruct model [URL]](https://huggingface.co/Qwen/Qwen2.5-7B-Instruct) (**Qwen2.5**[^29]) as the pre-trained text LLM, due to its outstanding performance on various benchmarks.
We utilize the streaming audio generation mechanism of **CosyVoice2**[^30], due to its low latency and competitive speech synthesis performance.
...
The **Voice Token LM** uses the pretrained **CosyVoice2**[^30] LM module.
...
The **Token2wav Synthesizer** comprises a pretrained flow-matching model, which converts tokens to mel spectrograms, and a pretrained vocoder, which transforms mel spectrograms into waveforms, both sourced from **CosyVoice2**[^30].
...
We compare ***MinMo*** with the baseline model **SeACo-Paraformer**[^42], which is an open-source ASR system with strong Chinese hotword biasing performance.
