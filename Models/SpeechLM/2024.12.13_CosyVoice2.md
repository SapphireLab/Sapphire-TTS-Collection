...
The $\text{Encoder}_1$, low-rank projector of FSQ module, bounded round operation and index calculation form the speech tokenizer for CosyVoice 2. Our speech tokenizer works at a token rate of 25 Hz, i.e., 25 speech tokens per second.

### Unified Text-Speech Language Model: 统一的文本-语音语言模型

In CosyVoice 2, the pre-trained textual LLM, Qwen2.5-0.5B \cite{qwen2.5}, is used as the text-speech language model to generate the speech tokens autoregressively with the input text as a prompt.
...
$$
\begin{aligned}
\nu_t(\phi^{OT}_t(X_0,X_1)|\theta) &= \mathrm{UNet}_\theta\left(\phi^{OT}_t(X_0,X_1),t;\mathbf{v},\{\mu\}_{1:L},\tilde{X_1}\right) \\
\end{aligned}
$$
