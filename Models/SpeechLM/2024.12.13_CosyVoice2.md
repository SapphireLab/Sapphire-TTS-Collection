...
Recent studies show that zero-shot TTS models are able to synthesize speech for any speaker by imitating the timbre, prosody and style of a reference speech ([VALL-E [8]](2023.01.05_VALL-E.md)).
...
Hybrid systems combine the text-to-codec language model and codec-to-feature diffusion model ([Seed-TTS [33]](2024.06.04_Seed-TTS.md); [CosyVoice [34]](2024.07.07_CosyVoice.md); [FireRedTTS [35]](2024.09.05_FireRedTTS.md)).
...
This results in high latency, negatively impacting user experience in applications like voice chat ([GPT-4o [36]](../TextLM/2024.09.06_GPT-4o.md); [SpeechGPT [37]](2023.05.18_SpeechGPT.md)).
To address this issue, streaming synthesis has been explored for language model-based zero-shot TTS models ([LiveSpeech [38]](2024.06.05_LiveSpeech.md); [LiveSpeech2 [39]](2024.10.01_LiveSpeech2.md); [BASE TTS [40]](2024.02.12_BASE-TTS.md); [LLM2Speech [41]](2023.09.20_LLM2Speech.md)), but diffusion-based TTS models and hybrid systems lack well-established streaming solutions.
...
As shown in Figure \ref{fig:overall} (a), we insert the [finite scalar quantization (FSQ) module [42]](../../Modules/VQ/FSQ.md)  into the encoder of SenseVoice-Large ASR model ([FunAudioLLM [43]](2024.07.04_FunAudioLLM.md)). At the training stage, the input speech $X$ goes through the $\text{Encoder}_1$ to obtain the intermediate representations, where $\text{Encoder}_1$ consists of six Transformer blocks with the [rotary positional embedding [44]](../../Modules/PositionEmb/RoPE.md).
...
The first-package latency is an important metric for streaming synthesis models, which significantly affects the user experience especially in LLM-based voice chat applications, such as [GPT-4o [36]](../TextLM/2024.09.06_GPT-4o.md).
...
We use WER and SS to distinguish preferred sample $x^w$ and rejected samples $x^l$ and optimize the TTS system with [direct preference optimization (DPO) [49]](../../Modules/RLHF/DPO.md) as follow:
...
The first one is constructed from the test-clean set of Librispeech corpus ([UniCATS [51]](../Diffusion/2023.06.13_UniCATS.md)), denoted as test-clean. This test set is used to evaluate ***CosyVoice2*** on a limited English domain.
...
For the test-ko, we selected 1,000 speech samples with a WER of less than 5\% and no deletion or insertion errors, utilizing the [Whisper-Large V3 [54]](2022.12.06_Whisper.md) as the ASR model.
