...
Table~[Tab.02](#tab:comparison) compares the speech quality generated by ParaStyleTTS with that of other models.
...
Table~[Tab.03](#tab:para_performance_comparision) presents a comparison of expressive speaking style control across different models.
...
These embeddings are visualized using t-SNE in Figure~[fig:embed-all](#fig:embed-all), providing an external perspective on how well the model encodes paralinguistic styles in the acoustic space.
In Figure~[Fig.02](#fig:embed-age), age-related embeddings form well-separated clusters, with only minor overlap between Teenagers and Young Adults.
...
Figure~[fig:embed-gender](#fig:embed-gender) shows clearly distinct clusters for Male and Female, which confirms that the model captures gender-specific acoustic features.
In Figure~[fig:embed-emotion](#fig:embed-emotion), most emotion classes form coherent and distinguishable clusters.
...
Table~[Tab.04](#tab:resource_comparision) presents a resource-based comparison of TTS models in terms of inference speed, model size, and CUDA memory usage, which are three key metrics that affect real-time responsiveness, storage cost, and hardware requirements.
...
Beyond using LLMs for style fusion, CosyVoice also requires a flow-matching-based vocoder[^Lipman2022Flow] to convert the LLM output into a waveform.
...
Table~[Tab.09](#tab:text_prompt) in the appendix section shows the full details of evaluated prompts.
...
As shown in Figure~[fig:vibe-gender-var](#fig:vibe-gender-var), embeddings of Male and Female speech form two well-separated clusters, indicating that the model maintains stable control regardless of prompt formulation.
...
This inconsistency is reflected in Figure~[Fig.03](#fig:cosyvoice-gender-var), where multiple Male samples are mis-clustered near the Female region.
...
[^Wang2017Tacotron]: Tacotron: Towards End-to-End Speech Synthesis. arXiv:1703.10135.
...
[^Kim2020Glow-Tts]: Glow-Tts: A Generative Flow for Text-to-Speech via Monotonic Alignment Search. Advances in Neural Information Processing Systems 2020.
