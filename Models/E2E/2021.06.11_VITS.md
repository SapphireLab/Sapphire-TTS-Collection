...
- 作者:
  - 01 Jaehyeon Kim (Kakao Enterprise)
  - 02 Jungil Kong (Kakao Enterprise)
  - 03 Juhee Son (Kakao Enterprise, KAIST)
- 链接:
  - [ArXiv](https://arxiv.org/abs/2106.06103)
  - [Publication](https://proceedings.mlr.press/v139/kim21f)
...
- 文件:
  - [ArXiv](_PDF/2106.06103v1__VITS__Conditional_Variational_Autoencoder_with_Adversarial_Learning_for_End-to-End_Text-to-Speech.pdf)
  - [Publication](_PDF/2106.06103p0__VITS__ICML2021.pdf)
...
A subjective human evaluation (**Mean Opinion Score, MOS**) on the **LJ Speech**, a single speaker dataset, shows that our method outperforms the best publicly available TTS systems and achieves a MOS com parable to ground truth.
...
在 **LJ Speech** 数据集的主观评价, 即**平均意见得分 (Mean Opinion Score, MOS)** 中, 我们对比了我们的模型与最先进的公开可用的文本转语音系统, 并达到了可比的 MOS.
...
The first stage is to produce intermediate speech representations such as mel-spectrograms (**Tacotron2**[^01]) or linguistic features (**WaveNet**[^02]) from the preprocessed text (Although there is a text preprocessing step in TTS systems, We herein use preprocessed text interchangeably with the word "text".), and the second stage is to generate raw waveforms conditioned on the intermediate representations (**WaveNet**[^02]; **WaveRNN**[^03]).
...
第一阶段是生成中间语音表示如梅尔频谱图 (**Tacotron2**[^01]) 或来自预处理文本的语言特征 (**WaveNet**[^02]),
第二阶段是根据中间表示生成原始波形 (**WaveNet**[^02]; **WaveRNN**[^03]).
每个阶段的模型都单独建立.

</td></tr>
<tr><td>

Neural network-based autoregressive TTS systems have shown the capability of synthesizing realistic speech (**Tacotron2**[^01]; **TransformerTTS**[^04]), but their sequential generative process makes it difficult to fully utilize modern parallel processors.
To overcome this limitation and improve synthesis speed, several non-autoregressive methods have been proposed.
In the text-to-spectrogram generation step, extracting attention maps from pre-trained autoregressive teacher networks (**FastSpeech**[^05]; **ParaNet**[^06]) is attempted to decrease the difficulty of learning alignments between text and spectrograms.
More recently, likelihood-based methods further eliminate the dependency on external aligners by estimating or learning alignments that maximize the likelihood of target mel-spectrograms (**AlignTTS**[^07]; **Flow-TTS**[^08]; **Glow-TTS**[^09]).
Meanwhile, **Generative Adversarial Networks (GANs)**[^10] have been explored in second stage models.
GAN-based feed-forward networks with multiple discriminators, each distinguishing samples at different scales or periods, achieve high-quality raw waveform synthesis (**MelGAN**[^11]; **GAN-TTS**[^12]; **HiFi-GAN**[^13]).

</td><td>

基于神经网络的文本转语音系统 (**Tacotron2**[^01]; **TransformerTTS**[^04]) 已经展示了合成真实语音的能力, 但它们的顺序生成过程使得它们难以充分利用现代并行处理器.
为了克服这一限制并提升合成速度, 已经有数种非自回归方法被提出.
在文本到频谱图生成步骤中, 从预训练的自回归教师网络中提取注意力特征图 (**FastSpeech**[^05]; **ParaNet**[^06]) 试图减少学习文本和频谱图之间对齐的难度.
近期, 基于似然的方法进一步消除了对外部对齐器的依赖, 通过最大化目标频谱图的似然来估计或学习对齐 (**AlignTTS**[^07]; **Flow-TTS**[^08]; **Glow-TTS**[^09]).
同时, 生成对抗网络被用于第二阶模型.
基于 GAN 的前馈网络具有多个判别器, 每个判别不同尺度或周期的样本, 实现高质量的原始波形合成 (**MelGAN**[^11]; **GAN-TTS**[^12]; **HiFi-GAN**[^13]).

</td></tr>
<tr><td>

Despite the progress of parallel TTS systems, two-stage pipelines remain problematic because they require sequential training or fine-tuning (**Tacotron2**[^01]; **Wave-Tacotron**[^14]) for high-quality production wherein latter stage models are trained with the generated samples of earlier stage models.
...
尽管并行文本转语音系统取得了进展, 但两阶段方案仍然存在问题. 因为它们要求顺序训练或微调以获取高质量结果 (**Tacotron2**[^01]; **Wave-Tacotron**[^14]), 即后一阶段的模型需要在前一阶段模型的生成样本上进行训练.
此外, 他们依赖于预定义的中间特征限制了将学习到的隐藏表示以获得进一步的性能提升.
近期, 有几项工作如 **FastSpeech2s**[^15] 和 **EATS**[^16] 提出了高效的端到端训练方法, 如训练于较短的音频片段而不是完整波形, 利用梅尔频谱图解码器辅助文本表示学习, 并设计专门的频谱图损失以松弛目标和生成语音的长度不匹配.
...
In this work, we present a parallel end-to-end TTS method that generates more natural sounding audio than current two-stage models.
Using a **Variational AutoEncoder (VAE)**[^17], we connect two modules of TTS systems through latent variables to enable efficient end-to-end learning.
...
Our method obtains more natural sounding speech and higher sampling efficiency than the best publicly available TTS system, **Glow-TTS**[^09] with **HiFi-GAN**[^13].
...
我们的方法能获得比最好的开源 TTS 系统 (**Glow-TTS**[^09] + **HiFi-GAN**[^13]) 更自然的语音和更高的采样效率.
[示例页面](https://jaywalnut310.github.io/vits-demo/index.html) 和 [源码](https://github.com/jaywalnut310/vits) 已公开.
...
### End-to-End Text-to-Speech: 端到端文本转语音

<table><tr><td width="50%">

Currently, neural TTS models with a two-stage pipeline can synthesize human-like speech (**WaveNet**[^02]; **DeepVoice3**[^18]; **Tacotron2**[^01]).
...
**FastSpeech2s**[^15] is an extension of **FastSpeech2**[^15] that enables end-to-end parallel generation by adopting adversarial training and an auxiliary mel-spectrogram decoder that helps learn text representations.
However, to resolve the one-to-many problem, **FastSpeech2s**[^15] must extract phoneme duration, pitch, and energy from speech used as input conditions in training.
**EATS**[^16] employs adversarial training as well and a differentiable alignment scheme.
To resolve the length mismatch problem between generated and target speech, **EATS**[^16] adopts soft dynamic time warping loss that is calculated by dynamic programming.
**Wave-Tacotron**[^14] combines normalizing flows with **Tacotron2**[^01] for an end-to-end structure but remains autoregressive.
...
**FastSpeech2s**[^15] 是 **FastSpeech2**[^15] 的扩展, 它通过采用对抗训练和一个辅助的梅尔频谱图解码器来实现端到端并行生成.
然而, 为了解决一对多问题, **FastSpeech2s**[^15] 必须从训练时使用的语音中提取音素时长, 音高和能量.
**EATS**[^16] 也是采用对抗训练和不同的对齐方案.
为了解决生成和目标语音之间的长度不匹配问题, **EATS**[^16] 采用软动态时间窗损失, 它是通过动态规划计算的.
**Wave-Tacotron**[^14] 是将标准化流与 **Tacotron2**[^01] 结合以实现端到端结构, 但仍然是自回归的.
...
**VAEs**[^17] are one of the most widely used likelihood-based deep generative models.
...
In speech synthesis, **GMVAE-Tacotron**[^19] and [^20] combine **Tacotron2**[^01] and VAEs to learn speech style and prosody.
**BVAE-TTS**[^21] generates mel-spectrograms in parallel based on a bidirectional VAE (**IAF**[^22]).
Unlike the previous works that applied VAEs to first stage models, we adopt a VAE to a parallel end-to-end TTS system.
[^23], [^24] and [^25] improve VAE performance by enhancing the expressive power of prior and posterior distribution with normalizing flows.
...
在语音合成中, Hsu 和 Zhang 将 **Tacotron2**[^01] 和 VAEs 结合起来, 以学习语音风格和韵律.
...
Similar to our work, Ma et al. proposed a conditional VAE with normalizing flows in a conditional prior network for non-autoregressive neural machine translation, **FlowSeq**[^26].
However, the fact that our model can explicitly align a latent sequence with the source sequence differs from **FlowSeq**[^26], which needs to learn implicit alignment through attention mechanisms.
...
和我们的工作类似, Ma 等人提出了 **FlowSeq**[^26] 一个带有标准化流的条件变分自编码器, 用于非自回归神经机器翻译.
然而, 由于我们的模型可以显式地对齐隐变量序列和源序列, 与 **FlowSeq**[^26] 不同, 它需要通过注意机制学习隐式对齐.
我们的模型通过 MAS 匹配隐变量序列和时间对齐的源序列, 从而消除了将隐变量序列转换为标准正态随机变量的负担, 这使得标准化流的架构更简单.

</td></tr></table>

### Duration Prediction in Non-Autoregressive Text-to-Speech: 非自回归文本转语音中的时长预测

<table><tr><td width="50%">

Autoregressive TTS models (**VoiceLoop**[^27]; **Tacotron2**[^01]; **Flowtron**[^28]) diverse speech with different rhythms through their autoregressive structure and several tricks including maintaining dropout probability during inference and priming ([^29]).
Parallel TTS models (**FastSpeech**[^05]; **ParaNet**[^06]; **Glow-TTS**[^09]; ****FastSpeech2s**[^15]), on the other hand, have been relied on deterministic duration prediction.
...
Figure.01a and Figure.01b show the training and inference procedures of our method, respectively.
...
In practice, we do not upsample the whole latent variables $z$ but use partial sequences as an input for the decoder, which is the windowed generator training used for efficient end-to-end training (**FastSpeech2s**[^15]; **EATS**[^16]).
...
We, therefore, apply a normalizing flow $f_{\theta}$ ([^23]), which allows an invertible transformation of a simple distribution into a more complex distribution following the rule of change-of-variables, on top of the factorized normal prior distribution:
...
### Alignment Estimation: 对齐估计

#### Monotonic Alignment Search: 单调对齐搜索

<table><tr><td width="50%">

To estimate an alignment $A$ between input text and target speech, we adopt **Monotonic Alignment Search (MAS)**(**Glow-TTS**[^09]), a method to search an alignment that maximizes the likelihood of data parameterized by a normalizing flow $f$:
...
To find the optimal alignment, **Glow-TTS**[^09] use dynamic programming.
...
Although we search the alignment which maximizes the ELBO not the exact log-likelihood of data, we can use the MAS implementation of **Glow-TTS**[^09].
...
附录 A 包含 MAS 的伪代码.
尽管我们搜索 ELBO 最大的对齐, 而不是数据精确的对数似然, 但我们可以使用 **Glow-TTS**[^09] 中的 MAS 实现.
...
#### Duration Prediction from Text·文本中的时长预测
...
The duration could be used to train a deterministic duration predictor, as proposed in previous work (**Glow-TTS**[^09]), but it cannot express the way a person utters at different speaking rates each time.
...
We apply variational dequantization (**Flow++**[^30]) and variational data augmentation (**VFlow**[^31]) to solve these problems.
...
We apply the stop gradient operator (**VQ-VAE**[^32]), which prevents back-propagating the gradient of inputs, to the input conditions so that the training of the duration predictor does not affect that of other modules.
...
时长可以用于训练一个确定性的时长预测器, 如 **Glow-TTS**[^09] 所提出的, 但它不能表达人类在不同语速下说话的方式.
...
我们应用变分去量化 (**Flow++**[^30]) 和变分数据增强 (**VFlow**[^31]) 以解决这些问题.
...
我们应用对输入条件应用停止梯度算子 (**VQ-VAE**[^32] 防止梯度反向传播) 以便训练时长预测器不影响其他模块.
...
In this work, we use two types of loss successfully applied in speech synthesis; the least-squares loss function (**LSGAN**[^33]) for adversarial training, and the additional feature-matching loss ([^34]) for training the generator:
...
Notably, the feature matching loss can be seen as reconstruction loss that is measured in the hidden layers of the discriminator suggested as an alternative to the element-wise reconstruction loss of VAEs ([^34]).
...
In this section, we mainly describe the newly added parts of ***VITS*** as we followed configurations of **Glow-TTS**[^09] and **HiFi-GAN**[^13] for several parts of our model:
we use the same Transformer encoder and WaveNet residual blocks as those of **Glow-TTS**[^09];
our decoder and the multi-period discriminator is the same as the generator and multi-period discriminator of **HiFi-GAN**[^13], respectively, except that we use different input dimension for the decoder and append a sub-discriminator.
...
本节主要描述了所提方法中新增的 ***VITS*** 的部分, 我们遵循 **Glow-TTS**[^09] 和 **HiFi-GAN**[^13] 的配置, 并对模型的不同部分进行了描述:
- 我们使用与 **Glow-TTS**[^09] 相同的 Transformer 编码器和 WaveNet 残差块;
- 我们使用与 **HiFi-GAN**[^13] 相同的生成器和多周期判别器, 除了对解码器使用不同的输入维度并添加了一个子判别器.
...
For the posterior encoder, we use the non-causal WaveNet residual blocks used in **WaveGlow**[^35] and **Glow-TTS**[^09].
...
For the multi-speaker case, we use global conditioning (**WaveNet**[^02]) in residual blocks to add speaker embedding.
...
对于后验编码器, 我们采用 **WaveGlow**[^35] 和 **Glow-TTS**[^09] 中使用的非因果 WaveNet 残差块.
WaveNet 残差块由带有门控激活单元的空洞卷积层和跳跃连接组成.
块上的线性映射层产生正态后验分布的均值和方差.
对于多说话人情形, 我们在残差块中使用全局条件化 (**WaveNet**[^02]) 以添加说话人嵌入.
...
The text encoder is a **Transformer**[^36] encoder that uses **relative positional representation**[^37] instead of absolute positional encodin.
...
The normalizing flow is a stack of affine coupling layers ([^38]) consisting of a stack of WaveNet residual blocks.
...
The decoder is essentially the **HiFi-GAN**[^13] V1 generator.
...
We follow the discriminator architecture of the multi-period discriminator proposed in **HiFi-GAN**[^13].
The multi-period discriminator is a mixture of Markovian window-based sub-discriminators (**MelGAN**[^11]), each of which operates on different periodic patterns of input waveforms.

For the discriminator, **HiFi-GAN**[^13] uses the multi-period discriminator containing five sub-discriminators with periods [2, 3, 5, 7, 11] and the multi-scale discriminator containing three sub-discriminators.
...
我们遵循 **HiFi-GAN**[^13] 提出的多周期判别器的架构.
多周期判别器是一个由马尔可夫窗口子判别器组成的混合模型 (**MelGAN**[^11]), 它们分别操作于输入波形的不同周期模式.

对于判别器, **HiFi-GAN**[^13] 使用包含五个子判别器的多周期判别器和包含三个子判别器的多尺度判别器.
...
We also apply **Neural Spline Flows**[^39], which take the form of invertible nonlinear transformations by using monotonic rational-quadratic splines, to coupling layers.
...
我们还应用了神经样条流[^39], 它是一种使用单调分段二次样条的非线性变换, 用于耦合层.
...
Figure.05a and Figure.05b show the training and inference procedures of the stochastic duration predictor, respectively.
The main building block of the stochastic duration predictor is the dilated and depth-wise separable convolutional (DDSConv) residual block as in Figure.05c.
...
Figure.06a and Figure.6b show the architecture of a condition encoder and a coupling layer used in the stochastic duration predictor.
...
We used the **LJ Speech dataset**[^40] for comparison with other publicly available models and the **VCTK dataset**[^41] to verify whether our model can learn and express diverse speech characteristics.
The **LJ Speech dataset**[^40] consists of 13,100 short audio clips of a single speaker with a total length of approximately 24 hours.
...
The **VCTK dataset**[^41] consists of approximately 44,000 short audio clips uttered by 109 native English speakers with various accents.
...
我们在两个不同的数据集上进行了实验.
我们使用 **LJ Speech 数据集**[^40] 用于和其他开源模型进行对比, 然后用 **VCTK 数据集**[^41] 验证模型是否可以学习和表达多样化的语音特征.

**LJ Speech 数据集**[^40] 由单个说话人的 13,100 短音频片段组成, 总时长约为 24 小时.
音频格式为 16 位 PCM, 采样率为 22 kHz, 我们没有对其进行任何处理.
我们随机将数据集分为训练集 (12,500 个样本), 验证集 (100 个样本), 和测试集 (500 个样本).

**VCTK 数据集**[^41] 由 109 名英语母语发言人发出的约 44,000 短音频片段组成, 总时长约为 44 小时.
...
We convert text sequences to IPA phoneme sequences using open-source software (**Phonemizer**[^42]), and the converted sequences are interspersed with a blank token following the implementation of **Glow-TTS**[^09].
...
我们使用开源软件 **Phonemizer**[^42]将文本序列转换为 IPA 音素序列, 并遵循 **Glow-TTS**[^09] 的实现, 在 IPA 音素序列中插入空白 Token.
...
The networks are trained using the **AdamW optimizer**[^43] with $\beta_1= 0.8$, $\beta_2=0.99$ and weight decay $\lambda = 0.01$.
The learning rate decay is scheduled by a 0.9991/8 factor in every epoch with an initial learning rate of $2\times 10^{−4}$.
Following previous work (**FastSpeech2s**[^15]; **EATS**[^16]), we adopt the windowed generator training, a method of generating only a part of raw waveforms to reduce the training time and memory usage during training.
...
与之前的工作 (**FastSpeech2s**[^15]; **EATS**[^16]) 类似, 我们采用窗口生成器训练, 一种只生成部分原始波形的训练方法以减少训练时间和内存占用.
...
### Experiment Setup for Comparison: 对比实验设置

<table><tr><td width="50%">

We compared our model with the best publicly available models.
We used **Tacotron2**[^01], an autoregressive model, and **Glow-TTS**[^09], a flow-based non-autoregressive model, as first stage models and **HiFi-GAN**[^13] as a second stage model.
We used their public implementations and pre-trained weights.
Since a two-stage TTS system can theoretically achieve higher synthesis quality through sequential training, we included the fine-tuned **HiFi-GAN**[^13] up to 100k steps with the predicted outputs from the first stage models.
We empirically found that fine-tuning **HiFi-GAN**[^13] with the generated mel-spectrograms from **Tacotron2**[^01] under teacher-forcing mode, led to better quality for both **Tacotron2**[^01] and **Glow-TTS**[^09] than fine-tuning with the generated mel-spectrograms from **Glow-TTS**[^09], so we appended the better fine-tuned **HiFi-GAN**[^13] to both **Tacotron2**[^01] and **Glow-TTS**[^09].
...
The probability of dropout in the pre-net of **Tacotron2**[^01] was set to 0.5.
For **Glow-TTS**[^09], the standard deviation of the prior distribution was set to 0.333.
...
我们将模型和最佳的公开模型进行了比较.
我们使用自回归模型 **Tacotron2**[^01], 和基于流的非自回归模型 **Glow-TTS**[^09] 作为一阶段模型, **HiFi-GAN**[^13] 作为二阶段模型.
我们使用它们的公开实现和预训练权重.
由于两阶段 TTS 系统理论上可以通过串行训练实现更高的合成质量, 我们将 **HiFi-GAN**[^13] 使用 **Tacotron2**[^01] **Glow-TTS**[^09] 的预测输出微调到 100k 步.
我们经验性地发现, 使用 **Tacotron2**[^01] 制模式, 微调 **HiFi-GAN**[^13] 能产生更好的质量, 因此我们将更好的微调 **HiFi-GAN**[^13] 追加到 **Tacotron2**[^01] 和 **Glow-TTS**[^09].
由于每个模型在采样时都有一定的随机性, 我们在实验中固定了每个模型的超参数, 以控制随机性.
**Tacotron2**[^01] 的预网络的丢弃概率设置为 0.5.
**Glow-TTS**[^09]
 的先验分布的标准差设置为 0.333.
...
### Speech Synthesis Quality: 语音合成质量
...
The evaluation results are shown in Table.01.
...
Table.01.Comparison of evaluated MOS with 95% confidence intervals on the **LJ Speech dataset**[^40].
...
The ***VITS*** (DDP), which employs the same deterministic duration predictor architecture used in **Glow-TTS**[^09] rather than the stochastic duration predictor, scores the second-highest among TTS systems in the MOS evaluation.
...
***VITS*** (DDP) 使用和 **Glow-TTS**[^09] 相同的确定性时长预测器架构, 而不是随机时长预测器, 在 MOS 评估中排名第二.
这些结果表明:
...
Table.02. MOS comparison in the ablation studies.
...
### Generalization to Multi-Speaker Text-to-Speech:多说话人文本转语音

<table><tr><td width="50%">

To verify that our model can learn and express diverse speech characteristics, we compared our model to **Tacotron2**[^01], **Glow-TTS**[^09] and **HiFi-GAN**[^13], which showed the ability to extend to multi-speaker speech synthesis ([^44]; **Glow-TTS**[^09]; **HiFi-GAN**[^13]).
We trained the models on the **VCTK dataset**[^41].
We added speaker embedding to our model as described in Section 2.5.
For **Tacotron2**[^01], we broadcasted speaker embedding and concatenated it with the encoder output, and for **Glow-TTS**[^09], we applied the global conditioning following the previous work.
The evaluation method is the same as that described in Section 4.1.
As shown in Table.03, our model achieves a higher MOS than the other models.

</td><td>

为了验证我们的模型可以学习和表示多样的语音特性, 我们将模型与 **Tacotron2**[^01] **Glow-TTS**[^09] 和 **HiFi-GAN**[^13] 进行了比较, 它们都具有扩展到多说话人语音合成能力 ([^44]; **Glow-TTS**[^09]; **HiFi-GAN**[^13]).
我们在 **VCTK 数据集**[^41]上训练了模型.
我们在模型中加入了说话人嵌入, 如第 2.5 节所述.
对于 **Tacotron2**[^01] 其与编码器输出连接起来, 对于 **Glow-TTS**[^09], 我们遵循之前的工作, 应用全局条件.
评估方法与第 4.1 节中描述的相同.
...
Table 3.Comparison of evaluated MOS with 95% confidence intervals on the **VCTK dataset**[^41].
...
Similar to **Flowtron**[^28], all samples here were generated from a sentence “How much variation is there?”.
Figure.02a shows histograms of the lengths of 100 generated utterances from each model.
While **Glow-TTS**[^09] generates only fixed-length utterances due to the deterministic duration predictor, samples from our model follow a similar length distribution to that of **Tacotron2**[^01].
Figure.02b shows the lengths of 100 utterances generated with each of five speaker identities from our model in the multi-speaker setting, implying that the model learns the speaker-dependent phoneme duration.
F0 contours of 10 utterances extracted with the **YIN algorithm**[^45] in Figure.03 shows that our model generates speech with diverse pitches and rhythms, and five samples generated with each of different speaker identities in Figure.03d demonstrates our model expresses very different lengths and pitches of speech for each speaker identity.
Note that **Glow-TTS**[^09] could increase the diversity of pitch by increasing the standard deviation of the prior distribution, but on the contrary, it could lower the synthesis quality.

</td><td>

我们验证了随机时长预测器生成的语音长度有多少种, 合成的样本有多少种不同的语音特性.
与 **Flowtron**[^28] 类似, 这里所有的样本都来自于一句话 "How much variation is there?".
图 2a 显示了来自每个模型的 100 个合成句子的长度直方图.
虽然 **Glow-TTS**[^09] 只生成固定长度的句子, 但来自我们的模型的样本与 **Tacotron2**[^01].
图 2b 显示了来自我们的模型的 100 个句子的长度, 这表明模型学习了说话人相关的音素时长.
图 3 显示了使用 **YIN 算法**[^45]提取的 10 个句子的 F0 轮廓, 这表明我们的模型生成了具有不同音高和韵律的语音, 图 3d 显示了来自五个不同说话人标识的 10 个句子, 这表明模型为每个说话人标识生成了不同的语音长度和音高.
注意到 **Glow-TTS**[^09] 可以通过增加先验分布的标准差来增加音高的多样性, 但相反, 它会降低合成质量.

</td></tr></table>

### Synthesis Speed: 合成速度

<table><tr><td width="50%">

We compared the synthesis speed of our model with a parallel two-stage TTS system, **Glow-TTS**[^09] and **HiFi-GAN**[^13].
We measured the synchronized elapsed time over the entire process to generate raw waveforms from phoneme sequences with 100 sentences randomly selected from the test set of the **LJ Speech dataset**[^40].
...
我们比较了我们的模型与 **Glow-TTS**[^09] + **HiFi-GAN**[^13] 的合成速度.
我们测量了从 **LJ Speech 数据集**[^40] 的测试集中随机选择 100 个句子的音素序列生成原始波形所需的时间.
...
[^01]: [**Tacotron2**: Natural TTS Synthesis by Conditioning WaveNet on Mel Spectrogram Predictions.](../Acoustic/2017.12.16_Tacotron2.md) ICASSP2018.
[^02]: [**WaveNet**: A Generative Model for Raw Audio.](../Vocoder/2016.09.12_WaveNet.md) ArXiv2016.
[^03]: [**WaveRNN**: Efficient Neural Audio Synthesis.](../Vocoder/2018.02.23_WaveRNN.md) ICML2018.
[^04]: [**TransformerTTS**: Neural Speech Synthesis With Transformer Network.](../Acoustic/2018.09.19_TransformerTTS.md) AAAI2019.
[^05]: [**FastSpeech**: Fast, Robust and Controllable Text to Speech.](../Acoustic/2019.05.22_FastSpeech.md)
[^06]: [**ParaNet**: Non-Autoregressive Neural Text-to-Speech.](../Acoustic/2019.05.21_ParaNet.md) ICML2020.
[^07]: [**AlignTTS**: Efficient Feed-Forward Text-to-Speech System without Explicit Alignment.](../Acoustic/2020.03.04_AlignTTS.md) ICASSP2020.
[^08]: [**Flow-TTS**: A Non-Autoregressive Network for Text to Speech Based on Flow.](../Acoustic/2020.04.09_Flow-TTS.md) ICASSP2020.
[^09]: [**Glow-TTS**: A Generative Flow for Text-to-Speech via Monotonic Alignment Search.](../Acoustic/2020.05.22_Glow-TTS.md) NeurIPS2020.
[^10]: [**GAN**: Generative Adversarial Nets.](../_Basis/2014.06.10_GAN.md) NeurIPS2014.
[^11]: [**MelGAN**: MelGAN: Generative Adversarial Networks for Conditional Waveform Synthesis.](../Vocoder/2019.10.08_MelGAN.md) NeurIPS2019.
[^12]: [**GAN-TTS**: High Fidelity Speech Synthesis with Adversarial Networks.](../Vocoder/2019.09.25_GAN-TTS.md) ICLR2019.
[^13]: [**HiFi-GAN**: Generative Adversarial Networks for Efficient and High Fidelity Speech Synthesis.](../Vocoder/2020.10.12_HiFi-GAN.md) NeurIPS2020.
[^14]: [**Wave-Tacotron**: Spectrogram-Free End-to-End Text-to-Speech Synthesis.](../_tmp/Wave-Tacotron.md) ArXiv2020.
[^15]: [**FastSpeech2**: Fast and High-Quality End-to-End Text to Speech](../Acoustic/2020.06.08_FastSpeech2.md) ICLR2021.
[^16]: [**EATS**: End-to-end Adversarial Text-to-Speech.](2020.06.05_EATS.md) ICLR2021.
[^17]: [**VAE**: Auto-Encoding Variational Bayes.](../_Basis/VAE.md) ICLR2014.
[^18]: [**DeepVoice3**: 2000-Speaker Neural Text-to-Speech.](../Acoustic/2017.10.20_DeepVoice3.md) ICLR2018.
[^19]: [**GMVAE-Tacotron**: Hierarchical Generative Modeling for Controllable Speech Synthesis.](../Acoustic/2018.10.16_GMVAE-Tacotron.md) ICLR2019.
[^20]: Learning Latent Representations for Style Control and Transfer in End-to-End Speech Synthesis. ICASSP2019.
[^21]: [**BVAE-TTS**: Bidirectional Variational Inference for Non-Autoregressive Text-to-Speech.](../Acoustic/2021.01.13_BVAE-TTS.md) ICLR2021.
[^22]: [**IAF**: Improved Variational Inference with Inverse Autoregressive Flow.](../_Basis/IAF.md) NeurIPS2016.
[^23]: Variational Inference with Normalizing Flows. ICML2015.
[^24]: Variational Lossy Autoencoder. ICLR2017.
[^25]: Latent Normalizing Flows for Discrete Sequences. ICML2019.
[^26]: **FlowSeq**: Non-Autoregressive Conditional Sequence Generation with Generative Flow. EMNLP-IJCNLP2019.
[^27]: [**VoiceLoop**: Voice Fitting and Synthesis via a Phonological Loop.](../Acoustic/2017.07.20_VoiceLoop.md) ICLR2018.
[^28]: [**Flowtron**: An Autoregressive Flow-based Generative Network for Text-to-Speech Synthesis.](../Acoustic/2020.05.12_Flowtron.md) ICLR2021.
[^29]: Generating Sequences with Recurrent Neural Networks. ArXiv2013.
[^30]: [**Flow++**: Improving Flow-Based Generative Models with Variational Dequantization and Architecture Design.](../Flow/2019.02.01_Flow++.md) ICML2019.
[^31]: [**VFlow**: More Expressive Generative Flows with Variational Data Augmentation.](../Flow/VFlow.md) ICML2020.
[^32]: [**VQ-VAE**: Neural Discrete Representation Learning.](../../Modules/VQ/2017.11.02_VQ-VAE.md) NeurIPS2017.
[^33]: [**LSGAN**: Least Squares Generative Adversarial Networks.](../_Basis/2016.11.13_LSGAN.md) ICCV2017.
[^34]: Autoencoding beyond Pixels Using a Learned Similarity Metric. ICML2016.
[^35]: [**WaveGlow**: A Flow-Based Generative Network for Speech Synthesis.](../Vocoder/2018.10.31_WaveGlow.md) ICASSP2019.
[^36]: [**Transformer**: Attention is All You Need.](../_Transformer/2017.06.12_Transformer.md) NeurIPS2017.
[^37]: Self-Attention with Relative Position Representations. ACL2018.
[^38]: Density estimation using Real NVP. ICLR2017.
[^39]: Neural Spline Flows. NeurIPS2019.
[^40]: [**LJSpeech**: The LJ Speech Dataset.](../../Datasets/2017.07.05_LJSpeech.md)
[^41]: [**VCTK**: CSTR VCTK Corpus: English Multi-Speaker Corpus for CSTR Voice Cloning Toolkit.](../../Datasets/2012.08.00_VCTK.md)
[^42]: **Phonemizer** [Github](https://github.com/bootphon/phonemizer) 2021.
[^43]: [**AdamW**: Decoupled Weight Decay Regularization.](../../Modules/Optimization/2017.11.14_AdamW.md) ICLR2019.
[^44]: Transfer Learning from Speaker Verification to Multispeaker Text-To-Speech Synthesis. NeurIPS2018.
[^45]: **YIN**: A Fundamental Frequency Estimator for Speech and Music. 2002.