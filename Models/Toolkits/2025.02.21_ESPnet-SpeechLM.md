...
The advent of large language models (LLMs) has significantly advanced machine intelligence, particularly in the text domain (**GPT-4**[^01], **LLaMA3**[^02]).
As research expands beyond text, LLMs are increasingly applied to multimodal scenarios ([^03], **GPT-4o**[^04], **VITA**[^05]), such as speech ([^06], [^07]) and vision ([^08]), with the aim of achieving higher-level intelligence and enhancing human-computer interactions.
...
SpeechLMs have demonstrated remarkable progress across a variety of speech tasks, including zero-shot generalization (**VALL-E**[^09]), low-resource modeling (**SPEAR-TTS**[^10]), multi-task learning (**VoxtLM**[^11], **UniAudio**[^12]), instruction following ([^13]), real-time interaction (**Moshi**[^14], **Mini-Omni**[^15]), and emergent abilities (**UniAudio1.5**[^16]).
Similarly to text-based LLMs ([^17]), SpeechLMs benefit from scaling data volume, parameter size, and computational resources ([^18]).
...
However, despite these advances, the development of SpeechLMs remains a complex and resource-intensive endeavor (**Moshi**[^14]).
...
We also detail the creation of a 1.7B-parameter multi-task SpeechLM, pre-trained on ASR, TTS, TextLM, and AudioLM tasks, by leveraging 240 billion text tokens or audio frames (Section.\ref{multitask_exp}).
...
Examples of this include DeepSpeed [^19] and FlashAttention [^20].
To preserve text capability, it is common to initialize SpeechLMs from pre-trained text LLMs, which can rely on open-source platforms like [HuggingFace Transformers [URL]](https://github.com/huggingface/transformers).
These tools are integrated into ***ESPnet-SpeechLM***.
We also noticed that current text LLM training frameworks [^21], [^22] provide limited support for speech features.
...
Current research on SpeechLMs and their transparency has been significantly advanced by prior open-source SpeechLM research works ([^23], **Mini-Omni**[^15], **Moshi**[^14], **UniAudio**[^12], **VoxtLM**[^11]).
SpeechLM research also greatly benefits from general speech processing and sequence-to-sequence modeling toolkits [^24], [^25], [^26], [^27], [^28], [^29], as they provide a wide range of components applicable to SpeechLM development.
...
More specifically, it is built upon the existing ESPnet [^24] codebase to better exploit prior community efforts and compare with existing non-SpeechLM works.
...
***ESPnet-SpeechLM*** uniformly frames speech tasks as sequential modeling problems using auto-regressive prediction over discrete token sequences within a decoder-only Transformer [^30].
...
This capability is especially critical for audio codec models [^31], [^32], which encode each audio frame into multiple tokens.
...
Specifically, an index file is provided for each data item, with the format of example-id content in each line.
...
***ESPnet-SpeechLM*** processes these files to generate a unified data.json file for each dataset, which contains tokenized results and metadata.
data.json is the data format in both training and evaluation.
...
***ESPnet-SpeechLM*** provides multiple implementations of multi-stream language models (**VALL-E**[^09], [^33], **UniAudio**[^12]).
...
alternatively, we support any AutoModelForCausalLM from HuggingFace Transformers to leverage pre-trained text LLMs
Also, following **Moshi**[^14], custom weights can be provided during loss computing to balance the tokens from different tokenization methods.
This is usually to guarantee one audio frame has the same loss weight as one non-audio token.
Lastly, in addition to applying the cross-entropy loss, the toolkit also supports reinforcement learning from human feedback (RLHF) for SpeechLMs (see [^34] for details).
...
Within these scripts, we consistently adopt the [VERSA  [URL]](https://github.com/shinjiwlab/versa), a comprehensive collection of >60 speech and audio evaluation metrics [^35].
...
Similar to single-task training, for each task, the task template definition (Section.\ref{template}) and preprocessing (Section.\ref{preprocessing}) are completed separately, which gives multiple tokenized datasets and the corresponding data.json files.

Note: Especially, these preprocessing works are easy to distribute and are suitable for collaborative works.

The data loader accepts a list of data.json and fuses these datasets before training, which allows the users to dynamically change the multitasking data setups.
...
We provide multiple options for these two tokenization methods, with an emphasis on ESPnet-Codec [^35] and XEUS [^36].
...
Besides text and audio, these multi-modal models can leverage information from auxiliary modalities, such as music score [^37], vision token [^38], classification labels (e.g., bool, time-stamp), speaker-identity and the continuous LLM embeddings.
...
Upon the Transfomer, we support 4 distinctive multi-stream language model implementations (**VALL-E**[^09], **UniAudio**[^12], [^33]).
For training efficiency, we leverage DeepSpeed [^19], FlashAttention [^20] and [Liger-Kernel [URL]](https://github.com/linkedin/Liger-Kernel).
These modules enable us to achieve model FLOPs utility (MFU) [^39] as high as 35\% with multi-node training using NVIDIA H100 GPUs.
...
As a highlight of this demo, we present a 1.7B pre-trained SpeechLM that covers 4 tasks similar to **VoxtLM**[^11]: ASR, TTS, text auto-regressive prediction (TextLM), and speech auto-regressive prediction (AudioLM) (Section.\ref{multitask_exp}).
These models are released through [ESPnet HuggingFace Hub [URL]](https://huggingface.co/espnet).
...
We consistently leverage the pre-trained text LLM, [SmolLM2 series [URL]](https://huggingface.co/HuggingFaceTB), for SpeechLM initialization.
We adopt the 360M and 1.7B versions for single-task and multi-task models, respectively.
We adopt delay interleave [^33] as the multi-stream language model architecture.
In terms of tokenization, we adopt the Codec\_SSL method
(Section.\ref{feature}) for speech representation.
To preserve full transparency and self-consistency, [ESPnet-Codec [URL]](https://huggingface.co/ftshijt/espnet_codec_dac_large_v1.4_360epoch)
and [XEUS [URL]](https://huggingface.co/espnet/xeus) are adopted for codec and SSL tokenizers respectively.

Note: XEUS K-Means tokenizer trained on its last layer of representation using 5k clusters.
...
We balance the weights for text, SSL, and codec tokens as 1: 0.5: 0.0625

Note: Each audio frame is represented by 1 SSL token and 8 codec tokens.
...
Following **VoxtLM**[^11], [^34], we test word error rate (WER) for ASR; ASR WER, Speaker Similarity and Proxy MOS for TTS; perplexity for AudioLM.
We measure the TextLM ability using popular metrics like MMLU [^40], ARC-Challenge (ARC-C) [^41], HellaSwag (HS)[^42], and OpenBookQA (OBQA) [^43].
...
We evaluate our ASR system on multiple benchmarks and compare it with the popular open-sourced ASR models: whisper-v3-large [^44] and OWSM v3.1-medium [^45].
...
In Tab.\ref{tab:tts}, we compare the ***ESPnet-SpeechLM*** TTS system with other discrete-based TTS systems.
For VALL-E X and VALL-E 2, we use the third-party implementations: [VALL-E X [URL]](https://huggingface.co/Plachta/VALL-E-X/resolve/main/vallex-checkpoint.pt), [VALL-E 2 [URL]](https://huggingface.co/amphion/valle).
...
Compared with other SpeechLMs (**VoxtLM**[^11], **Mini-Omni**[^15], [^23], [^46]) and multimodal LMs [^05], our pre-trained model still preserves decent ASR, TTS and AudioLM performance even with limited parameter budget.
In terms of text capability, the pre-trained model preserves close performance compared with the text-only LLM **LLaMA-3.2-1B**[^02].
...
We will continue the development of the ***ESPnet-SpeechLM*** toolkit, such as supporting more tokenization methods, more task templates, more modeling options, and LLM inference engines [^47].
...
本文展示了 ***ESPnet-SpeechLM*** 工具包, 它涵盖了语音语言模型开发的整个流程, 并在多个设计选择方面提供了广泛的支持.
我们还为单任务训练和多任务训练提供了用户案例, 展示了市场上其他模型的竞争性能.
工具包承诺在数据, 代码, 配方, 和预训练模型方面保持全面透明.
