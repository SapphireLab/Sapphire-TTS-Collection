...
A subjective human evaluation (**Mean Opinion Score, MOS**) of a single speaker dataset indicates that our proposed method demonstrates similarity to human quality while generating 22.05 kHz high-fidelity audio 167.9 times faster than real-time on a single V100 GPU.
...
语音合成领域的几项近期工作都采用了**生成对抗网络**来生成原始波形.
...
Most neural speech synthesis models use a two-stage pipeline: 1) predicting a low resolution intermediate representation such as mel-spectrograms[^01], [^02], [^03] or linguistic features[^04] from text, and 2) synthesizing raw waveform audio from the intermediate representation[^04], [^05], [^06], [^07].
...
WaveNet[^04] is an autoregressive (AR) convolutional neural network that demonstrates the ability of neural network based methods to surpass conventional methods in quality.
...
Parallel WaveNet[^05] is an inverse autoregressive flow (IAF) that is trained to minimize its Kullback-Leibler divergence from a pre-trained WaveNet called a teacher.
Compared to the teacher model, it improves the synthesis speed to 1,000 times or more, without quality degradation.
WaveGlow[^06] eliminates the need for distilling a teacher model, and simplifies the learning process through maximum likelihood estimation by employing efficient bijective flows based on Glow[^08].
...
Generative adversarial networks (GANs)[^09], which are one of the most dominant deep generative models, have also been applied to speech synthesis. [^07] proposed a multi-scale architecture for discriminators operating on multiple scales of raw waveforms.
With sophisticated architectural consideration, the MelGAN generator is compact enough to enable real-time synthesis on CPU. [^10] proposed multi-resolution STFT loss function to improve and stabilize GAN training and achieved better parameter efficiency and less training time than an IAF model, ClariNet[^11].
Instead of mel-spectrograms, GAN-TTS[^12] successfully generates high quality raw audio waveforms from linguistic features through multiple discriminators operating on different window sizes.
...
As we extract different parts of audio for the discriminator,
...
Figure~\ref{arch_gen_overall} shows the architecture of the generator.
As in previous work[^13], [^14], [^07], noise is not given to the generator as an additional input.
...
**Multi-Receptive Field Fusion (MRF) Fusion**
...
The architectures of MRF module and a residual block are shown in Figure.01.
...
This problem has been addressed in the previous work[^15] by increasing receptive fields of the generator and discriminator.
...
Additionally, to capture consecutive patterns and long-term dependencies, we use the multi-scale discriminator (MSD) proposed in MelGAN [^07], which consecutively evaluates audio samples at different levels.
...
As shown in Figure~\ref{arch_mpd_overall}, we first reshape 1D raw audio of length $T$ into 2D data of height $T/p$ and width $p$ and then apply 2D convolutions to the reshaped data.
...
Subsequently, weight normalization[^16] is applied to MPD.
...
The architecture of MSD is drawn from that of MelGAN[^07].
MSD is a mixture of three sub-discriminators operating on different input scales: raw audio, $\times$2 average-pooled audio, and $\times$4 average-pooled audio, as shown in Figure~\ref{arch_msd_overall}.
...
Instead, spectral normalization[^17] is applied and stabilizes training as it reported.

Note that MPD operates on disjoint samples of raw waveforms, whereas MSD operates on smoothed waveforms.

For previous work using multi-discriminator architecture such as MPD and MSD, [^12]'s work can also be referred.
...
Also, the resemblance between MPD and RWD[^12] can be considered in the part of  reshaping input audio, but MPD uses periods set to prime numbers to discriminate data of as many periods as possible, whereas RWD uses reshape factors of overlapped periods and does not handle each channel of the reshaped data separately,
...
More details of the architectural difference can be found in Appendix \hyperref[app-c1]{C}.
...
### Training Loss Terms: 训练损失项

<table><tr><td width="50%">

**GAN Loss**

For brevity, we describe our discriminators, MSD and MPD, as one discriminator throughout Section~\ref{training_loss_terms}.
For the generator and discriminator, the training objectives follow LSGAN[^18], which replace the binary cross-entropy terms of the original GAN objectives[^09] with least squares loss functions for non-vanishing gradient flows.
...
Referring to previous work[^14], applying a reconstruction loss to GAN model helps to generate realistic results, and in [^10]'s work, time-frequency distribution is effectively captured by jointly optimizing multi-resolution spectrogram and adversarial loss functions.
...
The feature matching loss is a learned similarity metric measured by the difference in features of the discriminator between a ground truth sample and a generated sample[^19], [^07].
As it was successfully adopted to speech synthesis[^07], we use it as an additional loss to train the generator.
...
For fair and reproducible comparison with other models, we used the LJSpeech dataset[^20] in which many speech synthesis models are trained.
...
HiFi-GAN was compared against the best publicly available models: the popular mixture of logistics (MoL) WaveNet[^05] implementation[^21]
\footnote{In the original paper[^05], an input condition called linguistic features was used, but this implementation uses mel-spectrogram as the input condition as in the later paper[^01].} and the official implementation of WaveGlow[^22] and MelGAN[^23].
We used the provided pretrained weights for all the models.

To evaluate the generality of HiFi-GAN to the mel-spectrogram inversion of unseen speakers, we used the VCTK multi-speaker dataset[^24], which consists of approximately 44,200 short audio clips uttered by 109 native English speakers with various accents.
...
The synthesis speed was measured on GPU and CPU environments according to the recent research trends regarding the efficiency of neural networks[^07], [^25], [^26].
...
For $V1$, we set $h_u=512$, $k_u=[16, 16, 4, 4]$, $k_r=[3,7,11]$, and $D_r=[[1,1],[3,1],[5,1]] \times 3]$. $V2$ is simply a smaller version of $V1$, which has a smaller hidden dimension $h_u=128$ but with exactly the same receptive fields.
...
The detailed configurations of the models are listed in Appendix \hyperref[app-a1]{A.1}.
...
The networks were trained using the AdamW optimizer[^27] with $\beta_1=0.8$, $\beta_2=0.99$, and weight decay $\lambda=0.01$.
The learning rate decay was scheduled by a $0.999$ factor in every epoch with an initial learning rate of $2\times 10^{-4}$.
...
Table~\ref{tab:speaker-generalization} shows the experimental results for the mel-spectrogram inversion of the unseen speakers.
...
Additionally, the tendency of difference in MOS scores of the proposed models is similar with the result shown in Section~\ref{sec_fidelity_speed}, which exhibits generalization across different datasets.
...
We conducted an additional experiment to examine the effectiveness of the proposed models when applied to an end-to-end speech synthesis pipeline, which consists of \textit{text to mel-spectrogram} and \textit{mel-spectrogram to waveform} synthesis modules.
We herein used Tacotron2[^01] to generate mel-spectrograms from text.
Without any modification, we synthesized the mel-spectrograms using the most popular implementation of Tacotron2[^28] with the provided pre-trained weights.
We then fed them as input conditions into second stage models, including our models and WaveGlow used in Section~\ref{sec_fidelity_speed}.\footnote{MelGAN and MoL WaveNet were excluded from comparison for their differences in pre-processing such as frequency clipping.
Mismatch in the input representation led to producing low quality audio when combined with Tacotron2.}

The MOS scores are listed in Table~\ref{tab:end-to-end-speech-synthesis}.
...
However, when the pixel-wise difference in the mel-spectrogram domain between generated waveforms and a mel-spectrogram from Tacotron2 are investigated as demonstrated in Figure~\ref{e2e_comparison}, we found that the difference is insignificant, which means that the predicted mel-spectrogram from Tacotron2 was already noisy.
To improve the audio quality in the end-to-end setting, we applied fine-tuning with predicted mel-spectrograms of Tacotron2 in teacher-forcing mode[^01] to all the models up to 100k steps.
...
[^04]: oord2016wavenet
[^05]: oord2018parallel
[^06]: prenger2019waveglow
[^07]: Kumar2018melgan
[^08]: kingma2018glow
[^09]: goodfellow2014generative
[^10]: yamamoto2020parallel
[^11]: ping2018clarinet
[^12]: binkowski2019high
[^13]: mathieu2015deep
[^14]: isola2017image
[^15]: donahue2018adversarial
[^16]: salimans2016weight
[^17]: miyato2018spectral
[^18]: mao2017least
[^19]: larsen2016autoencoding
[^20]: ljspeech17
[^21]: Yamamoto18wavenet
[^22]: WaveGlowRepo
[^23]: MelGANRepo
[^24]: veaux2017cstr
[^25]: zhai2020squeezewave
[^26]: tan2020efficientdet
[^27]: loshchilov2017decoupled
[^28]: Tacotron2Repo
