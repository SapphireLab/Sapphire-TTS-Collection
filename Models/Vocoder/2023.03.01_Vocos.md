...
- 作者:
  - 01 Hubert Siuzdak (Gemelo AI)
- 链接:
  - [ArXiv](https://arxiv.org/abs/2306.00814)
  - [Publication](https://openreview.net/forum?id=vY9nzQmQBw) ICLR2024Poster
  - [Github](https://github.com/gemelo-ai/vocos)
  - [Demo](https://gemelo-ai.github.io/vocos/)
- 文件:
  - [ArXiv](_PDF/2306.00814v3__Vocos__Closing_the_Gap_between_Time-Domain_&_Fourier-Based_Neural_Vocoders_for_High-Quality_Audio_Synthesis.pdf)
  - [Publication](_PDF/2306.00814p0__Vocos__ICLR2024Poster.pdf)
...
While vocoders evolved significantly (Kawahara et al., 1999; [WORLD](../Vocoder/2015.11.11_WORLD.md)), they tended to oversimplify speech production, generating a distinctive ”buzzy” sound and thus compromising the naturalness of the speech.

A significant breakthrough in speech synthesis was achieved with the introduction of [WaveNet](../Vocoder/2016.09.12_WaveNet.md), a deep generative model for raw audio waveforms.
...
Autoregressive models, like WaveNet, generate audio samples sequentially, conditioning each new sample on all previously generated ones ([SampleRNN](2016.12.22_SampleRNN.md); [WaveRNN](2018.02.23_WaveRNN.md); [LPCNet](2018.10.28_LPCNet.md)).
On the other hand, non-autoregressive models generate all samples independently, parallelizing the process and making it more computationally efficient ([Parallel WaveNet](2017.11.28_Parallel_WaveNet.md); [WaveGlow](2018.10.31_WaveGlow.md); Donahue et al., 2018).
...
![Images/2023.03.01_Vocos_Fig.01.png](Images/2023.03.01_Vocos_Fig.01.png)
...
Progress was made with the introduction of advanced critics, such as the multi-scale discriminator (MSD) (Kumar et al., 2019) and the multi-period discriminator (MPD) ([HiFi-GAN](../Vocoder/2020.10.12_HiFi-GAN.md)).
...
This design, known as an isotropic architecture, has been found to work well in various settings, including [Transformer](../_Transformer/2017.06.12_Transformer.md).
...
##### Baseline Methods
Our proposed model, Vocos, is compared to: iSTFTNet (Kaneko et al., 2022), BigVGAN (Lee et al., 2022), and [HiFi-GAN](../Vocoder/2020.10.12_HiFi-GAN.md).
...
![Images/2023.03.01_Vocos_Fig.03.png](Images/2023.03.01_Vocos_Fig.03.png)
...
While traditionally, neural vocoders reconstruct the audio waveform from a mel-scaled spectrogram – an approach widely adopted in many speech synthesis pipelines – recent research has started to utilize learnt features (Siuzdak et al., 2022), often in a quantized form ([AudioLM](../SpeechLM/2022.09.07_AudioLM.md)).

In this section, we draw a comparison with [EnCodec](../SpeechCodec/2022.10.24_EnCodec.md), an open-source neural audio codec, which follows a typical time-domain GAN vocoder architecture and uses Residual Vector Quantization (RVQ) ([SoundStream](../SpeechCodec/2021.07.07_SoundStream.md)) to compress the latent space.
...
In [EnCodec](../SpeechCodec/2022.10.24_EnCodec.md), dedicated discriminators are trained for each bandwidth.
...
We utilize the open-source model checkpoint of [EnCodec](../SpeechCodec/2022.10.24_EnCodec.md) operating at 24 kHz.
To align with [EnCodec](../SpeechCodec/2022.10.24_EnCodec.md), we scale down Vocos to match its parameter count (7.9M) and train it on clean speech segments sourced from the DNS Challenge (Dubey et al., 2022).
Our evaluation, conducted on the DAPS dataset (Mysore, 2014) and detailed in Table 4, reveals that despite [EnCodec](../SpeechCodec/2022.10.24_EnCodec.md)’s reconstruction artifacts not significantly impacting PESQ and Periodicity scores, they are considerably reflected in the perceptual score, as denoted by UTMOS.
In this regard, Vocos notably outperforms [EnCodec](../SpeechCodec/2022.10.24_EnCodec.md).
...
Bark (Suno AI, 2023), a widely recognized open-source model, leverages a GPT-style, decoder-only architecture, with [EnCodec](../SpeechCodec/2022.10.24_EnCodec.md)’s 6kbps audio tokens serving as its vocabulary.
Vocos trained to reconstruct [EnCodec](../SpeechCodec/2022.10.24_EnCodec.md) tokens can effectively serve as a drop-in replacement vocoder for Bark.
