...
To further improve reconstruction quality under the extremely low compression rate setting, we introduce a **prompt mechanism** into ***TaDiCodec***, similar to prior works[^Le2024Voicebox], [^Chen2024F5-TTS], [^Eskimez2024E2], [^Wang2024Maskgct].
...
We build ***TaDiCodec*** using standard Llama-style Transformer blocks[^Touvron2023Llama], with bidirectional attention instead of causal attention.
...
The ***TaDiCodec***-AR-0.5B and ***TaDiCodec***-AR-3B models are initialized from the textual LLMs \texttt{Qwen2.5-0.5B-Instruct} and \texttt{Qwen2.5-3B-Instruct} [^Yang2024Qwen2.], respectively, while ***TaDiCodec***-AR-4B is initialized from \texttt{Phi-3.5-mini-instruct} [^Abdin2024Phi-3].
...
**ARS**[^Wang2024Maskgct]
Introduced as an AR baseline by[^Wang2024Maskgct]. and referred to as "AR + SoundStorm'' in the original paper[^Wang2024Maskgct].
...
**SparkTTS**[^Wang2025Spark-TTS]
An open-source, large-scale zero-shot TTS system built upon an AR model initialized from \texttt{Qwen2.5-0.5B-Instruct}, which predicts speech codes extracted by the BiCodec[^Wang2025Spark-TTS].

**Llasa**[^Ye2025Llasa]
An open-source, large-scale zero-shot TTS system built upon an AR model initialized from \texttt{Llama3.2-1B}[^Grattafiori2024Llama], which predicts speech codes extracted by the X-codec 2[^Ye2025Llasa].
...
These sets are introduced in[^Zhang2025Advancing] and contain tongue twisters and repeated texts.
...
These sets are introduced in[^Zhang2025Advancing], consist of target texts that mix English and Chinese.
...
These sets are introduced in[^Zhang2025Advancing], two types of cross-lingual samples are constructed: *zh2en* and *en2zh*, each comprising 500 samples.
...
We adopt two automatic speech recognition (ASR) models for WER computation: [whisper-large-v3](https://huggingface.co/openai/whisper-large-v3)[^Radford2023Robust] and [paraformer-zh](https://huggingface.co/funasr/paraformer-zh)[^Gao2023Funasr].
The former is used for non-Chinese utterances, while the latter is applied to Chinese speech, following established practices in recent studies[^Wang2025Metis], [^Wang2024Maskgct], [^Anastassiou2024Seed-TTS], [^Du2024CosyVoice].
...
We use the [WavLM-TDNN model](https://github.com/microsoft/UniSpeech/tree/main/downstreams/speaker_verification)[^Chen2022Wavlm] for speaker embedding extraction, following[^Wang2023Neural], [^Ju2024NaturalSpeech], [^Le2024Voicebox], [^Wang2024Maskgct], [^Anastassiou2024Seed-TTS].
