...
They are capable of producing fine-grained token sequences capturing acoustic details essential for tasks such as automatic speech recognition (ASR), speech synthesis, and spoken language understanding [^Guo2025Recent] [^Borsos2022AudioLM].
...
HuBERT [^Hsu2021HuBERT] and wav2vec 2.0 [^Baevski2020Wav2Vec2.0] are well-known examples of these SSL models, whose token sequences have been shown to capture both phonetic and semantic information, thereby enhancing performance across a variety of speech processing tasks [^Mohamed2022Self].

Building upon these foundations, NAC models, such as **EnCodec**[^Defossez2022EnCodec] and **SoundStream**[^Zeghidour2021SoundStream], have further constructed the process with the explicit goal of speech resynthesis.
...
For example, in text-to-speech (TTS) synthesis, NAC tokens enable high-quality waveform generation from text-based inputs, contributing to improved audio fidelity and naturalness [^Defossez2022EnCodec] [^Zeghidour2021SoundStream].
NACs have also been integrated into ASR pipelines, where compressed token representations facilitate efficient and accurate transcription while reducing computational costs [^dhawan2024codec].
Furthermore, NAC models have been explored for speech separation tasks, operating directly in the compressed token space to achieve efficient source separation with lower computational requirements [^yip2024towards].
...
In linguistics and information theory, it is well established that the frequency distribution of words or character $n$-grams in natural language tends to follow Zipf's law [^zipf1949human], characterised by a few high-frequency elements and many low-frequency elements.
For example, the third most frequent word in an English document, "and," appears approximately one-third as often as the most frequent word, "the\footnote{https://www.cs.cmu.edu/˜cburch/words/top.html}."
This relationship is given by the following power-law equation between the frequency rank $r$ of a word and its frequency $f(r)$:
...
Such distributions have been often observed not only in natural languages but also in animal communication systems and large language models, making $\eta$ a key indicator of the balance between redundancy and information efficiency [^mandelbrot1953contribution] [^gelbukh2001zipf].

**Heaps' Law**:

To capture the properties of a natural language in terms of the uniqueness of vocabulary, analyses based on Heaps' law [^heaps1978information] are also employed.
...
Entropy, proposed by Shannon [^shannon1951prediction], is widely used as a measure of the diversity and unpredictability of information within a sequence and serves as a fundamental metric in evaluating linguistic systems and coding efficiency.
...
Takamichi et al. [^takamichi2024learned] analysed whether speech tokens generated by SSL models follow Zipf's law, as observed in natural language text.
...
Moreover, Sicherman et al. [^Sicherman2023AnalysingDS] explored the interpretability and redundancy of SSL-based speech tokens and found a strong correlation between tokens and phonemes.
...
With respect to NAC-based tokens, Liu et al. [^liu2024visual] identified the challenge of inconsistency—different token sequences may be produced from the same speech input due to variations in the tokenization process.
...
<a id="tab:nac_models">
Codec configuration comparison.
"A--F" represents different NAC models, where
- "A" is SpeechTokenizer [^Zhang2023SpeechTokenizer],
- "B$\sim$" is AcademiCodec [^Yang2023HiFi-Codec],
- "C" is AudioDec [^Wu2023AudioDec],
- "D" is DAC [^Kumar2023DAC],
- "E$\sim$" is EnCodec [^Defossez2022EnCodec],
- "F$\sim$" is FunCodec [^Du2023FunCodec].

$n_d$ represents the dimension size, SR represents the sample rate.
The contents of this table are partially cited from [^Wu2024Towards].
</a>
...
The analysis was performed on 10 hours of single-speaker speech data for English (LJSpeech\footnote{\url{https://keithito.com/LJ-Speech-Dataset/}}) and Mandarin (Chinese CSS10 [^Park2019CSS10AC])
...
Specifically, we evaluated the properties of the token sequences for each model on Zipf's law [^zipf1949human], Heaps' law [^heaps1978information], and calculations of entropy [^shannon1951prediction] and redundancy.
...
In this experiment, we computed the frequency distributions of token $n$-grams extracted from various NAC models and evaluated their fit to Zipf's law [^zipf1949human] by analysing their rank-frequency distributions on a log-log scale.

For real-world data, when the tail is too heavy or too light, it is difficult to properly predict the shape of the data using general linear regression in Zipf's model [^chan2024analyzinglanguagevisualtokens].
Thus, to robustly estimate the parameters of Zipf's law under such conditions, we employed the \texttt{powerlaw} library [^clauset2009powerlaw], which fits the rank-frequency data using the maximum likelihood estimate:
$$
...
Also, the goodness-of-fit was evaluated using the Kolmogorov–Smirnov (KS) distance, where larger suggest deviation from linearity [^KSdistance].
...
To evaluate the novelty of speech tokens, we conducted analyses based on Heaps' law [^heaps1978information].
...
To evaluate this relationship, we used the Codec-SUPERB [^codecsuperb] implementation to decode the NAC tokens back into speech waveforms, generating resynthesised speech outputs for each codec model under study.
For the intelligibility task, we then applied \texttt{whisper-medium} [^Radford2022Whisper] ASR model, to transcribe the synthetic speech.
...
Also, we assessed the naturalness of the synthetic speech using UTMOS [^Saeki2022UTMOS], speech quality assessment metric that estimates the perceived naturalness of speech based on neural network architectures trained on large-scale human-annotated datasets.
...
Interestingly, we observed that this trend was most clearly observed with 3-gram token sequences, while analyses with 2-gram and 4-gram configurations did not show the same consistent relationship between $\alpha$ and benchmark performances.
\label{note1}The exact values and graphs could not be included in the paper due to page limitations.
...
[^Guo2025Recent]: [**Survey20250210**: Recent Advances in Discrete Speech Tokens: A Review](../../Surveys/2025.02.10_Guo2025Recent.md). ArXiv:2502.06490.
...
[^Hsu2021HuBERT]: [**HuBERT**: Self-Supervised Speech Representation Learning by Masked Prediction of Hidden Units](2021.06.14_HuBERT.md). ArXiv:2106.07447/TASLP2021.
[^Baevski2020Wav2Vec2.0]: [**Wav2Vec 2.0**: A Framework for Self-Supervised Learning of Speech Representations](2020.06.20_Wav2Vec2.0.md). ArXiv:2006.11477/NeurIPS2020.
[^Mohamed2022Self]: [**Survey20220521**: Self-Supervised Speech Representation Learning: A Review](../../Surveys/S20220521.md). ArXiv:2205.10643/JSTSP2022.
[^Defossez2022EnCodec]: [**EnCodec**: High Fidelity Neural Audio Compression](2022.10.24_EnCodec.md). ArXiv:2210.13438/TMLR2023.
[^Zeghidour2021SoundStream]: [**SoundStream**: An End-to-End Neural Audio Codec](2021.07.07_SoundStream.md). ArXiv:2107.03312/TASLP2021.
[^dhawan2024codec]: Codec-ASR: Training performant automatic speech recognition systems with discrete speech representations.
[^yip2024towards]: Towards audio codec-based speech separation.
[^zipf1949human]: Human behavior and the principle of least effort.
[^mandelbrot1953contribution]: Contribution `a la th´eorie math´ ematique des jeux de communication
[^gelbukh2001zipf]: Zipf and heaps laws’ coefficients depend on language
[^heaps1978information]: Information retrieval: Computational and theoretical aspects
[^shannon1951prediction]: Prediction and entropy of printed english
[^takamichi2024learned]: Do learned speech symbols follow Zipf’s law?
[^Sicherman2023AnalysingDS]: Analysing discrete self supervised speech representation for spoken language modeling,
[^liu2024visual]: Visual instruction tuning
[^Zhang2023SpeechTokenizer]: [**Speechtokenizer**: Unified Speech Tokenizer for Speech Large Language Models](2023.08.31_SpeechTokenizer.md). ArXiv:2308.16692/ICLR2024.
[^Yang2023HiFi-Codec]: [**HiFi-Codec**: Group-Residual Vector Quantization for High Fidelity Audio Codec](2023.05.04_HiFi-Codec.md). ArXiv:2305.02765.
[^Wu2023AudioDec]: [**AudioDec**: An Open-Source Streaming High-Fidelity Neural Audio Codec](2023.05.26_AudioDec.md). ArXiv:2305.16608/ICASSP2023.
[^Kumar2023DAC]: [**DAC**: High-Fidelity Audio Compression with Improved RVQGAN](2023.06.11_Descript-Audio-Codec.md). ArXiv:2306.06546/NeurIPS2023.
[^Du2023FunCodec]: [**FunCodec**: A Fundamental, Reproducible and Integrable Open-Source Toolkit for Neural Speech Codec](2023.09.14_FunCodec.md). ArXiv:2309.07405/ICASSP2024.
[^Wu2024Towards]: [**Survey20240220**: Towards Audio Language Modeling: An Overview](../../Surveys/S20240220.md). ArXiv:2402.13236.
[^codecsuperb]: Codec-SUPERB: An in-depth analysis of sound codec models
[^Park2019CSS10AC]: CSS10: A collection of single speaker speech datasets for 10 languages,
[^chan2024analyzinglanguagevisualtokens]: Analyzing the language of visual tokens
[^clauset2009powerlaw]: Power-law distributions in empirical data
[^KSdistance]: The kolmogorov-smirnov test for goodness of fi
