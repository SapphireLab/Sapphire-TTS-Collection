...
- 作者:
  ...
  - 02 [Yakun Song](../../Authors/Yakun_Song_(宋雅昆).md)
  - 03 [Chenpeng Du](../../Authors/Chenpeng_Du_(杜晨鹏).md)
...
- 机构:
  - [上海交通大学 X-LANCE](../../Institutions/CHN-SJTU_上海交通大学.md)
...
- 链接:
  ...
  - [Scholar](https://scholar.google.com/scholar?cluster=10227820571254254402)
- 标签:
  - [全双工建模](../../Tags/FullDuplexModel.md)
...
- 数据:
  ...
  - [MUSAN](../../Datasets/2015.10.28_MUSAN.md)
  - [The Speech Commands Dataset](../../Datasets/SpeechCommands.md)
- 对比:
  - Vanilla TTS without Listening Channel;
  - [VQ-Wav2Vec [02]](../Speech_Representaion/2019.10.12_VQ-Wav2Vec.md) + LSLM (Listening + Speaking) + [CTX-vec2wav](../Diffusion/2023.06.13_UniCATS.md);
  - Fusion: Early, Middle, Late;
  - Training: From Scratch, Loading&Fix, Loading&Training;
- 复现:
...
Other research employs this paradigm to task-specific applications, such as decoder-only high-fidelity TTS ([VALL-E [40]](2023.01.05_VALL-E.md); [AudioLM [03]](2022.09.07_AudioLM.md); [Seed-TTS [31]](2024.06.04_Seed-TTS.md); [CosyVoice [13]](2024.07.07_CosyVoice.md)) and decoder-only streaming ASR ([Speech ReaLLM [33]](2024.06.13_Speech_ReaLLM.md); [Tsunoo et al.[38]](../ASR/2024.06.23_Decoder-only_Architecture_for_Streaming_E2E_Speech_Recognition.md); [Chen et al.[04]](../ASR/2024.06.27_Streaming_Decoder-Only_ASR_with_Discrete_Speech_Units.md); [BESTOW [08]](2024.06.28_BESTOW.md))
...
- 其他研究则采用这一范式进行特定任务的应用, 如
  - 基于仅解码器架构的高保真文本转语音 ([VALL-E [40]](2023.01.05_VALL-E.md); [AudioLM [03]](2022.09.07_AudioLM.md)；[Seed-TTS [31]](2024.06.04_Seed-TTS.md); [CosyVoice [13]](2024.07.07_CosyVoice.md));
...
Some research leverages the powerful in-context learning capabilities of LLMs to extend task-specific abilities to more universal applications, such as speech understanding ([Wav2Prompt [11]](2024.06.01_Wav2Prompt.md)), audio understanding ([LTU [14]](2023.05.18_LTU.md)), or both ([SALMONN [35]](2023.10.20_SALMONN.md); [Qwen-Audio [09]](2023.11.14_Qwen-Audio.md); [Qwen2-Audio [10]](2024.07.15_Qwen2-Audio.md)).
...
- 基于 LLM 的语音翻译: [COSMIC [28]](2023.11.03_COSMIC.md); [SALM [07]](2023.10.13_SALM.md); [Huang et al. [16]](Investigating_Decoder-Only_LLMs_for_STT_Translation.md), [LLaST [06]](2024.07.22_LLaST.md);
...
一些研究利用 LLMs 强大的上下文学习能力以扩展具体任务的能力到更通用的应用, 例如语音理解 ([Wav2Prompt [11]](2024.06.01_Wav2Prompt.md)), 音频理解 ([LTU [14]](2023.05.18_LTU.md)) 或两者兼有  ([SALMONN [35]](2023.10.20_SALMONN.md); [Qwen-Audio [09]](2023.11.14_Qwen-Audio.md); [Qwen2-Audio [10]](2024.07.15_Qwen2-Audio.md)).
...
不像 [VALL-E](2023.01.05_VALL-E.md) 类型的模型用多层残差矢量量化 (RVQ) Token 将自回归方法和非自回归方法结合, 我们的模型采用单层的离散音频 Token.
...
2. 为了基于离散 Token 训练一个自回归文本转语音模型, 我们将语音嵌入 $R$ 进行量化:
$$
    R^q=Qnt(R)
$$
其中 $Qnt$ 是离散操作, $R^q$ 是离散 Token.

3. 给定上下文信息 $C$, 在这个场景中是对于需要进行合成的文本内容, 模型自回归地合成对应的语音离散 Token.
...
4. 在推理时, 模型从基于已经生成的 Token $\hat{R}^q_{1:t-1}$ 和上下文 $C$ 的条件分布中采样 $\hat{r}^q_t$.
这个过程用方程描述如下:
$$
    \hat{r}^q_t\sim P(r^q_t | \hat{R}^q_{1:t-1}, C; \theta_{S}).
$$

5. 声码器 $Dec$ 用于从离散 Token $\hat{R}^q$ 恢复语音信号 $\hat{X}^R$, 写为:
$$
    \hat{X}^R=Dec(\hat{R}^q, A)
$$
其中 $A$ 是提供合成语音音色的声学提示.
...
![](Images/2024.08.05_LSLM_Fig.02.png)
...
![](Images/2024.08.05_LSLM_Fig.03.png)
...
![](Images/2024.08.05_LSLM_Tab.01.png)
...
The word error rate (WER) comparing the generated speech to the original text is considered as the TTS capability evaluation metrics using [Whisper [30]](2022.12.06_Whisper.md) [large v3](https://github.com/openai/whisper).
...
使用 [Whisper [30]](2022.12.06_Whisper.md) [large v3](https://github.com/openai/whisper) 计算生成语音与原始文本之间的词错误率 (WER) 作为 TTS 能力评估指标.
...
![](Images/2024.08.05_LSLM_Tab.02.png)
...
![](Images/2024.08.05_LSLM_Tab.03.png)
...
![](Images/2024.08.05_LSLM_Fig.04.png)
...
![](Images/2024.08.05_LSLM_Tab.04.png)
