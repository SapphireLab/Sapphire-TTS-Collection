...
Similarly, OZSpeech [^Hieu2025OZSpeech] achieves single-pass decoding by incorporating a prior as the initial stage and introducing an anchor loss during training to facilitate one-step sampling.
...
In contrast, flow-matching models have predominantly followed a single strategy: embedding discrete data into a continuous space followed by continuous flow matching [^Du2024Cosyvoice2], [^Hieu2025OZS}peech], [^Wang2025Discl-Vc], [^Zuo2025Enhancing].
...
<--
A growing trend in speech synthesis focuses on converting raw waveforms into discrete token representations using **vector-quantized variational autoencoders (VQ-VAE)**, which was first introduced by **VQ-VAE**[^Oord2017Neural] in the field of computer vision and later adapted to speech synthesis.
...
This limitation has prompted a shift toward non-autoregressive (NAR) paradigms (**NaturalSpeech2**[^Shen2024NaturalSpeech2], **NaturalSpeech3**[^Ju2024NaturalSpeech3], [^Du2024UniCATS], **DiTTo-TTS**[^Lee2024DiTTo-TTS], [^Jia2025DiTAR]).
For example, NaturalSpeech 2 [^Shen2024NaturalSpeech2] uses diffusion [^Ho2020Denoising], [^Song2021Score-Based] to generate discrete acoustic tokens as continuous features, and **NaturalSpeech3**[^Ju2024NaturalSpeech3] further factorizes speech into subspaces of content, prosody, and acoustic details, employing multiple diffusion models to independently capture various acoustic characteristics.
...
However, most existing speech-related flow matching applications operate in a continuous space (**Matcha-TTS**[^Mehta2023Matcha-TTS], [^Guan2024Reflow-TTS], [^Yao2025Stablevc], [^Zuo2025Rhythm], [^Zuo2025Enhancing], [^Hieu2025OZSpeech]), requiring either a pure mel-spectrogram or discrete tokens to be embedded into continuous representations prior to generation.
...
<--
**Source Distribution**:
Following [^Gat2024Discrete], we instantiate the source distribution $p$ to assign all probability mass to sequences in which every token is the mask token \verb|[MASK]|, that is, $p(x) = \delta_{\verb|[MASK]|}(x)$.
...
<--
Inspired by the Multi-head Attention mechanism [^Vaswani2017Transformer], which enables the model to jointly attend to information from different representation subspaces, we propose a factorized flow prediction mechanism based on multi-head prediction.
...
[^Hieu2025OZSpeech]: OZSpeech: One-Step Zero-Shot Speech Synthesis With Learned-Prior-Conditioned Flow Matching. Proceedings of the 63rd Annual Meeting of the Association for Computational Linguistics (Volume 1: Long Papers) 2025.
...
[^Du2024Cosyvoice2]: Cosyvoice 2: Scalable Streaming Speech Synthesis With Large Language Models. arXiv:2412.10117.
