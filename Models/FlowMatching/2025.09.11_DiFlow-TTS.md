...
- 链接:
  ...
  - [Publication]() submitted to AAAI2026
  - [Github](https://github.com/DiFlow-TTS/DiFlow-TTS)
  - [Demo](https://diflow-tts.github.io)
- 文件:
  - [ArXiv:2509.09631v1](PDF/2025.09.11_2509.09631v1__DiFlow-TTS__Discrete_Flow_Matching_With_Factorized_Speech_Tokens_for_Low-Latency_Zero-Shot_Text-to-Speech.pdf)
  - [ArXiv:2509.09631v2](PDF/2025.09.12_2509.09631v2__DiFlow-TTS__Discrete_Flow_Matching_With_Factorized_Speech_Tokens_for_Low-Latency_Zero-Shot_Text-to-Speech.pdf)
  - [Publication] #TODO
...
Code and audio samples are available on our [demo page](https://diflow-tts.github.io).
-->
零样本文本转语音旨在使用很短的参考样本模仿未见说话人的声音以合成高质量语音, 不需要说话人适应和对韵律属性的精确建模.
近期基于语言模型, 扩散模型和流匹配的方法在零样本文本转语音方面展示了良好的结果, 但仍然受限于推理速度慢和重复伪影.

离散编解码器表示被广泛用于语音合成, 且近期工作开始探索扩散模型在纯离散设置下的应用, 表明了离散生成建模在语音合成方面的潜力.
但是, 现有的流匹配方法通常将这些离散 Token 嵌入到连续空间中并使用连续流匹配, 这可能无法完全利用离散表示的优势.

为了解决这些挑战, 我们提出了 ***DiFlow-TTS***, 这是我们所知的第一个探索用于语音合成的纯**离散流匹配 (Discrete Flow Matching)** 模型, 它在一个紧凑且统一的架构中显式地建模了分解的语音属性.
它通过在文本内容上条件化以实现上下文学习, 并从参考语音中提取韵律和声学属性, 从而在零样本设置下有效地克隆属性.
此外, 模型采用了分解流预测机制 (Factorized Flow Prediction) 使用不同的头分支处理韵律和声学细节, 允许它学习具体方面的分布.

实验结果表明 ***DiFlow-TTS*** 在多个关键指标上都取得了良好的性能, 包括自然度, 韵律, 保存说话人的风格, 和能量控制.
它还保持了模型的紧凑大小, 并实现了低延迟推理, 相比最新基线生成语音速度提高了 25.8 倍.
代码和音频示例可在我们的[演示页面](https://diflow-tts.github.io)上获得.
...
-->
零样本 TTS 近年来取得了显著进展, 目标是仅使用数秒的参考语音来精确地模仿先前未见过的说话人声音并生成高质量语音, 应用于个性化虚拟助手, 低资源语言的可访问性工具, 和内容创作.
虽然大规模生成模型在这个领域取得了重大进展, 但仍有一些挑战.

<!--
Recent studies have explored the application of language modeling (**VALL-E X**[^Zhang2023VALL-EX], **VALL-E R**[^Han2024VALL-ER], **MELLE**[^Meng2024MELLE], **ELLA-V**[^Song2024ELLA-V], **VALL-E 2**[^Chen2024VALL-E2], **VoiceCraft**[^Peng2024VoiceCraft], **MobileSpeech**[^Ji2024MobileSpeech], **Spark-TTS**[^Wang2025Spark-TTS], **VALL-E**[^Wang2023VALL-E]), particularly through autoregressive (AR) approaches.
A pioneering work of this is **VALL-E**[^Wang2023VALL-E], which represents speech as discrete codec tokens and treats these tokens analogously to text, thus reformulating TTS as a conditional codec language modeling task.
...
-->
近期研究探索了语言建模的应用 (**VALL-E X**[^Zhang2023VALL-EX], **VALL-E R**[^Han2024VALL-ER], **MELLE**[^Meng2024MELLE], **ELLA-V**[^Song2024ELLA-V], **VALL-E 2**[^Chen2024VALL-E2], **VoiceCraft**[^Peng2024VoiceCraft], **MobileSpeech**[^Ji2024MobileSpeech], **Spark-TTS**[^Wang2025Spark-TTS], **VALL-E**[^Wang2023VALL-E]), 特别是通过自回归 (AR) 方法.
一个开创性的工作是 **VALL-E**[^Wang2023VALL-E], 它将语音表示为离散编解码器 Token, 并将这些标记类似于文本处理, 从而将 TTS 任务重新形式化为条件编解码器语言建模任务.
虽然这样的模型在语音自然度和说话人相似度方面表现出了强大性能, 但它们通常需要在大型数据集上进行训练才能有效.
此外, 它们的自回归性质导致了推理速度慢且引入了常见的伪影, 例如来自参考音频的非预期重复内容或缺少输入文本中的初始词汇.
...
Among these, diffusion-based (**ZET-Speech**[^Kang2023ZET-Speech], **NaturalSpeech2**[^Shen2024NaturalSpeech2], **NaturalSpeech3**[^Ju2024NaturalSpeech3], **DiTTo-TTS**[^Lee2024DiTTo-TTS]) and flow-matching-based (**P-Flow**[^Kim2023P-Flow], **VoiceBox**[^Le2023VoiceBox], **Matcha-TTS**[^Mehta2023Matcha-TTS], **E2-TTS**[^Eskimez2024E2-TTS], **F5-TTS**[^Chen2024F5-TTS]) models have emerged as effective generative frameworks for TTS, striking a better balance between synthesis quality and inference efficiency.
...
For example, **R-VC**[^Zuo2025R-VC] introduces a shortcut flow matching mechanism that allows generation in a few steps and even one step, significantly reducing inference latency.
Similarly, **OZSpeech**[^Huynh2025OZSpeech] achieves single-pass decoding by incorporating a prior as the initial stage and introducing an anchor loss during training to facilitate one-step sampling.
...
This lack of explicit supervision results in entangled representations, which hinders the model's ability to learn disentangled and controllable factors.
...
***Q1: How can we explicitly model factorized speech tokens in a compact and unified framework?***
-->
为了克服这些局限性, 非自回归 (NAR) 方法被开发出来, 通过并行解码实现更快的生成.
在这类方法中, 基于扩散模型的 (**ZET-Speech**[^Kang2023ZET-Speech], **NaturalSpeech2**[^Shen2024NaturalSpeech2], **NaturalSpeech3**[^Ju2024NaturalSpeech3], **DiTTo-TTS**[^Lee2024DiTTo-TTS]) 和基于流匹配的 (**P-Flow**[^Kim2023P-Flow], **VoiceBox**[^Le2023VoiceBox], **Matcha-TTS**[^Mehta2023Matcha-TTS], **E2-TTS**[^Eskimez2024E2-TTS], **F5-TTS**[^Chen2024F5-TTS]) 模型已经成为 TTS 的有效生成框架, 在语音合成质量和推理效率之间取得了更好的平衡.
这些模型通常在梅尔频谱域中操作, 保留了丰富的声学细节并通过目标语音参考实现上下文学习, 进而提高了说话人相似度.

为了进一步减少推理复杂度, 数项工作采用离散编解码器 Token 作为中间表示.
- **R-VC**[^Zuo2025R-VC] 引入了捷径流匹配机制以允许在数步甚至一步内生成, 显著降低了推理延迟.
- 类似地, **OZSpeech**[^Huynh2025OZSpeech] 通过引入先验作为初始阶段, 并在训练期间引入锚点损失以支持一步采样, 实现单步解码.

但是零样本文本转语音不仅仅需要捕捉说话人身份, 还需要精确建模参考语音中的韵律特征.
一个自然的解决方案是将参考语音分解为各种属性: 韵律, 内容, 和声学细节, 并显式地建模每个属性.
尽管 **OZSpeech** 试图将提示语音分解为属性表示并通过流匹配建模, 但它采取的是隐式的方式: 视所有属性等同且依赖于流匹配过程隐式学习分离.
这种缺乏显式监督的机制导致表示紊乱, 这妨碍了模型学习分离和可控因素的能力.
因此, 合成语音的表达性和自然性显著受限.
基于这些观察, 我们提出以下问题:
***问题 1: 我们如何在紧凑且统一的框架中显式地建模分解的语音 Token?***

<!--
In particular, recent efforts to adapt discrete codec tokens to generative paradigms have sparked a growing interest in applying diffusion models within fully discrete settings (**DEmoFace**[^Ye2025DEmoFace], **ImaginTalk**[^Ye2025ImaginTalk]).
In contrast, flow-matching models have predominantly followed a single strategy: embedding discrete data into a continuous space followed by continuous flow matching (**CosyVoice2**[^Du2024CosyVoice2], **OZSpeech**[^Huynh2025OZSpeech], **Discl-VC**[^Wang2025Discl-VC], **PFlow-VC**[^Zuo2025PFlow-VC]).
Although **Discrete Flow Matching (DFM)**[^Gat2024DFM] has emerged as a promising generative paradigm in domains such as language, vision, and bioinformatics ([^Shaul2025Flow], **RSF**[^Yadav2025RSF], **MaskFlow**[^Fuest2025MaskFlow]), its application to speech synthesis is still limited.
...
-->
特别地, 最近将离散编解码器 Token 应用于生成范式的努力引起了在完全离散设置下应用扩散模型的增长热潮 (**DEmoFace**[^Ye2025DEmoFace], **ImaginTalk**[^Ye2025ImaginTalk]).
流匹配模型主要遵循单个策略: 将离散数据嵌入到连续空间中, 然后通过连续流匹配 (**CosyVoice2**[^Du2024CosyVoice2], **OZSpeech**[^Huynh2025OZSpeech], **Discl-VC**[^Wang2025Discl-VC], **PFlow-VC**[^Zuo2025PFlow-VC]) 进行学习.

尽管**离散流匹配 (Discrete Flow Matching, DFM)**[^Gat2024DFM] 在语言, 视觉, 生物信息等领域取得了令人振奋的生成范式, 但其在语音合成中的应用仍有限.
这促使我们提出第二个问题:
***问题 2: 纯粹的离散流匹配是否能够在合成语音中实现最先进的零样本 TTS 性能, 同时保持自然度和保真度?***

<--
In this study, we propose ***Discrete Flow Matching with Factorized Speech Tokens for Zero-Shot Text-To-Speech (DiFlow-TTS)***, as illustrated in [Fig.01](#fig:overview), to directly address the aforementioned challenges and research questions.
-->
在本研究中, 我们提出了 ***离散流匹配结合分解语音 Token 的零样本文本转语音 (DiFlow-TTS)*** 模型, 如[图 1](#fig:overview) 所示, 以直接解决上述挑战和研究问题.

> ![](Images/2025.09.11_DiFlow-TTS_Fig.01.png)
>
> <a id="fig:overview">
> <!--
> Figure.01: Overview of DiLow-TTS.
...
> A codec decoder reconstructs the final waveform.
> -->
> 图 01: DiFlow-TTS 概览.
> 模型使用编解码器的编码器将语音参考分解为音色, 韵律, 声学 Token.
> 输入文本经过 PCM 处理以生成内容 Token 和嵌入.
> 分解离散流去噪器 (Factorized Discrete Flow Denoiser, FDFD) 以内容嵌入, 说话人嵌入和由语音参考导出的离散韵律和声学 Token 为条件来生成韵律和声学 Token.
> 编解码器的解码器负责重构出最终的语音波形.
> </a>
...
-->
为了在紧凑且统一的框架下显式建模分解后的语音属性, 我们提出了 **音素-内容映射器 (Phoneme-Content Mapper, PCM)**, 它将音素序列映射到离散语音 Token, 代表着发言内容.
这一模块生成内容嵌入, 和语音的语义结构近似对齐.
这些嵌入, 以及从参考语音提示中提取出的听觉属性, 之后被用于条件化 **分解离散流去噪器 (Factorized Discrete Flow Denoiser, FDFD)**, 使其能够有效克隆参考的说话风格.
至关重要的是, 我们设计模型时, 对不同的语音方面 (具体是韵律和声学细节) 的概率速度采用了单独的预测头, 使其能够显式学习具体方面的分布.
因此, 模型的合成语音的自然度, 表现力和真实度都得到了改善.

我们的主要贡献如下:
- 提出了新式离散流匹配模型, 据我们所知是首个探索纯离散流匹配进行语音合成的模型.
  我们的方法通过以 PCM 导出的目标文本内容表示, 参考语音提示的听觉属性为条件进行上下文学习, 以完全非自回归的方式实现零样本设置下的有效属性克隆和高质量语音生成.
- 我们引入了分解流预测机制, DFM 通过不同预测头联合建模多个语音方面, 包括韵律和细粒度声学细节.
  这一架构使得模型在统一框架下显式学习特定方面的分布.
- 实验结果表明 ***DiFlow-TTS*** 在模型尺寸方面紧凑且高效, 在自然度, 韵律, 能量控制和说话人风格保持等方面超越了数种强力的零样本 TTS 基线模型.
  它还保持了紧凑的模型尺寸, 并实现了低延迟推理, 能以 25.8 倍于基线的速度生成语音.

## 2·背景

<--
A growing trend in speech synthesis focuses on converting raw waveforms into discrete token representations using **vector-quantized variational autoencoders (VQ-VAE)**, which was first introduced by **VQ-VAE**[^Oord2017VQ-VAE] in the field of computer vision and later adapted to speech synthesis.
...
To effectively model sequences of discrete speech tokens, recent efforts have adapted large language models (LLMs) from the natural language processing (NLP) domain (**VALL-E X**[^Zhang2023VALL-EX], **VALL-E 2**[^Chen2024VALL-E2], **VALL-E R**[^Han2024VALL-ER], **CosyVoice2**[^Du2024CosyVoice2], **VoiceCraft**[^Peng2024VoiceCraft], **MELLE**[^Meng2024MELLE], **VALL-E**[^Wang2023VALL-E], **Spark-TTS**[^Wang2025Spark-TTS]).
A notable example is **VALL-E**[^Wang2023VALL-E], which leverages a pre-trained neural codec to encode speech into discrete codec tokens and reformulates zero-shot TTS as a conditional codec language modeling task.
...
This limitation has prompted a shift toward non-autoregressive (NAR) paradigms (**NaturalSpeech2**[^Shen2024NaturalSpeech2], **NaturalSpeech3**[^Ju2024NaturalSpeech3], **UniCATS**[^Du2024UniCATS], **DiTTo-TTS**[^Lee2024DiTTo-TTS], **DiTAR**[^Jia2025DiTAR]).
For example, **NaturalSpeech2**[^Shen2024NaturalSpeech2] uses diffusion ([^Ho2020Denoising], [^Song2021Score-Based]) to generate discrete acoustic tokens as continuous features, and **NaturalSpeech3**[^Ju2024NaturalSpeech3] further factorizes speech into subspaces of content, prosody, and acoustic details, employing multiple diffusion models to independently capture various acoustic characteristics.
In parallel, flow matching (**FM**[^Lipman2022FM], **RectifiedFlow**[^Liu2022RectifiedFlow]) has gained attention as a promising generative technique, producing strong results in various domains.
However, most existing speech-related flow matching applications operate in a continuous space (**Matcha-TTS**[^Mehta2023Matcha-TTS], **ReFlow-TTS**[^Guan2024Reflow-TTS], [^Yao2025Stablevc], **R-VC**[^Zuo2025R-VC], [^Zuo2025PFlow-VC], **OZSpeech**[^Huynh2025OZSpeech]), requiring either a pure mel-spectrogram or discrete tokens to be embedded into continuous representations prior to generation.
...
Discrete-space generative models have already proven effective in domains such as natural language ([^Lou2024Discrete], [^Shi2024Simplified], [^Sahoo2024Simple]), proteins ([^Campbell2024Generative], [^Yi2025All-Atom]), vision ([^Austin2021Structured], [^Chang2022MaskGIT], [^Shi2024Simplified], [^Fuest2025MaskFlow]), code ([^Gat2024DFM]), and even graphs ([^Qin2025DeFoG]).
Although discrete diffusion models have recently been applied to speech synthesis (**DEmoFace**[^Ye2025DEmoFace], **ImaginTalk**[^Ye2025ImaginTalk]), the use of **discrete flow matching**[^Gat2024DFM] to model speech tokens remains largely unexplored, particularly in zero-shot TTS scenarios.
...
<--
[Fig.02](#fig:components) illustrates the overall framework of ***DiFlow-TTS***, which comprises three main modules: (a) **Speech Tokenization**, (b) **Phoneme-Content Mapper**, and (c) **Factorized Discrete Flow Denoiser**.
In the following sections, we describe each module in detail.
-->

> ![](Images/2025.09.11_DiFlow-TTS_Fig.02.png)
> <a id="fig:components">
> The detailed components of DiFlow TTS.
> The architecture consists of three main components:
> (a) **Speech Tokenization**, which extracts discrete tokens and a speaker embedding from a raw speech;
> (b) **Phoneme-Content Mapper**, which maps input phonemes to discrete content tokens and generates the corresponding content embeddings;
> (c) **Factorized Discrete Flow Denoiser**, which performs discrete flow matching conditioned on the content embeddings, speaker embedding, and the discrete prosody and acoustic tokens derived from the reference speech prompt.
</a>
...
<--
**Source Distribution**:
Following **Discrete Flow Matching (DFM)**[^Gat2024DFM], we instantiate the source distribution $p$ to assign all probability mass to sequences in which every token is the mask token $\verb|[MASK]|$, that is, $p(x) = \delta_{\verb|[MASK]|}(x)$.
...
We define a scheduler $\kappa_t \in [0,1]$, which is a monotonically increasing function with boundary conditions $\kappa_0 = 0$ and $\kappa_1 = 1$, where $t \in [0, 1]$ denotes the continuous time variable.
Following **DFM**[^Gat2024DFM], we consider a conditional probability path known as the *mixture path*, which linearly interpolates between the source and target distributions:
...
In our framework, we employ **FACodec**[^Ju2024NaturalSpeech3] as the speech tokenizer and speaker embedding extractor.
...
Given a transcript text, we first use an [open-source grapheme-to-phoneme converter](https://github.com/Kyubyong/g2p/tree/master) to obtain the textual phoneme sequence $\mathbf{P} = (\mathbf{P}_1, \mathbf{P}_2, \dots, \mathbf{P}_N)$ with $N$ phoneme tokens.
...
<--
Inspired by the Multi-head Attention mechanism (**Transformer**[^Vaswani2017Transformer]), which enables the model to jointly attend to information from different representation subspaces, we propose a factorized flow prediction mechanism based on multi-head prediction.
...
<--
To assess the impact of each component in ***DiFlow-TTS***, we perform an ablation study by systematically removing or modifying key elements:
(1) removing the attribute-type embeddings used to distinguish prosody, content, and acoustic streams;
(2) excluding the speaker embedding from the conditioning process, i.e., not injecting it into the DiT blocks;
(3) disabling the use of content embeddings in the FDFD module;
(4) replacing the multi-head prediction architecture with a single-head prediction.
-->
...
-->
在本工作中, 我们提出了 ***DiFlow-TTS***, 一种新式零样本文本转语音系统, 利用了纯离散流匹配技术和分解语音 Token 建模.
通过在一个统一的非自回归框架中使用不同的头来显式解耦并预测韵律和声学细节, ***DiFlow-TTS*** 实现了在自然度, 说话人相似性, 韵律准确性, 和语音合成速度等方面的强大性能.
我们的设计通过轻量级架构有效地集成了上下文学习和内容条件化, 提供了质量和延迟之间的良好平衡.
实验结果表明 ***DiFlow-TTS*** 超过了许多强大的基线, 尽管它仅使用了显著小得多的数据集进行训练.
未来工作将包括使多语言和情感合成成为可能, 并将视觉唇部信息集成到 ***DiFlow-TTS*** 以实现配音.

## 参考文献

[^Zhang2023VALL-EX]: [**VALL-E X**: Speak Foreign Languages with Your Own Voice: Cross-Lingual Neural Codec Language Modeling](../SpeechLM/ST2S/2023.03.07_VALL-E_X.md). ArXiv:2303.03926.
...
[^Wang2023VALL-E]: [**VALL-E**: Neural Codec Language Models are Zero-Shot Text to Speech Synthesizers](../SpeechLM/ST2S/2023.01.05_VALL-E.md). ArXiv:2301.02111/TASLP2025.
...
[^Shen2024NaturalSpeech2]: [**NaturalSpeech2**: Latent Diffusion Models are Natural and Zero-Shot Speech and Singing Synthesizers](../Diffusion/2023.04.18_NaturalSpeech2.md). ArXiv:2304.09116v3/ICLR2024.
...
[^Zuo2025R-VC]: [**R-VC**: Rhythm Controllable and Efficient Zero-Shot Voice Conversion via Shortcut Flow Matching](../-VC/2025.06.01_R-VC.md). ArXiv:2506.01014v1/ACL2025.
...
[^Ye2025DEmoFace]: [**DEmoFace**: Emotional Face-to-Speech](../_Basis/2025.02.03_DEmoFace.md). ArXiv:2502.01046v1.
[^Ye2025ImaginTalk]: [**ImaginTalk**: Shushing! Let's Imagine an Authentic Speech from the Silent Video](../_Basis/2025.03.19_ImaginTalk.md). ArXiv:2503.14928v1.
...
[^Wang2025Discl-VC]: [**Discl-VC**: Disentangled Discrete Tokens and In-Context Learning for Controllable Zero-Shot Voice Conversion](../-VC/2025.05.30_Discl-VC.md). ArXiv:2505.24291v1.
[^Zuo2025PFlow-VC]: [**PFlow-VC**: Enhancing Expressive Voice Conversion with Discrete Pitch-Conditioned Flow Matching Model](../-VC/2025.02.08_PFlow-VC.md). ArXiv:2502.05471v1/ICASSP2025.
[^Gat2024DFM]: [**DFM**: Discrete Flow Matching](_2024.07.22_Discrete_Flow_Matching.md). ArXiv:2407.15595v2.
...
[^Yadav2025RSF]: [**RETRO SYNFLOW/RSF**: Discrete Flow Matching for Accurate and Diverse Single-Step Retrosynthesis](../_Basis/2025.06.04_RSF.md). ArXiv:2506.04439v1.
[^Fuest2025MaskFlow]: [**MaskFlow**: Discrete Flows for Flexible and Efficient Long Video Generation](../_Basis/2025.02.16_MaskFlow.md). ArXiv:2502.11234v2.
...
[^Jia2025DiTAR]: DiTAR: Diffusion Transformer Autoregressive Modeling for Speech Generation. Forty-Second International Conference on Machine Learning 2025.
