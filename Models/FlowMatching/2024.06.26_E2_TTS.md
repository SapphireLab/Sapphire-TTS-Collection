...
- 文件:
  - [ArXiv](../Diffusion/_PDF/2406.18009v2__E2_TTS__Embarrassingly_Easy_Fully_Non-AutoRegressive_Zero-Shot_TTS.pdf)
  - [Publication] #TODO
...
In recent years, text-to-speech (TTS) systems have seen significant improvements (**FastSpeech**[^01], **FastSpeech2**[^02], **Glow-TTS**[^03], **HiFi-GAN**[^04]), achieving a level of naturalness that is indistinguishable from human speech (**NaturalSpeech**[^05]).
...
Early studies of zero-shot TTS used speaker embedding to condition the TTS system [^06] [^07].
More recently, **VALL-E**[^08] proposed formulating the zero-shot TTS problem as a language modeling problem in the neural codec domain, achieving significantly improved speaker similarity while maintaining a simplistic model architecture.
Various extensions were proposed to improve stability (**SpeechX**[^09], **VALL-T**[^10], **RALL-E**[^11]), and **VALL-E 2**[^12] recently achieved human-level zero-shot TTS with techniques including repetition-aware sampling and grouped code modeling.
...
Secondly, a dedicated effort to figure out the best tokenizer (both for text tokens and audio tokens) is necessary to achieve the best quality (**SpeechX**[^09]).
Thirdly, it is required to use some tricks to stably handle long sequences of audio codecs, such as the combination of AR and non-autoregressive (NAR) modeling (**VALL-E**[^08], **VALL-E 2**[^12]), multi-scale transformer (**UniAudio**[^13]), grouped code modeling (**VALL-E 2**[^12]).
...
![Images/2024.06.26.E2_TTS.Fig.01a.png](../Diffusion/Images/2024.06.26_E2_TTS_Fig.01a.png)
...
![Images/2024.06.26.E2_TTS.Fig.01b.png](../Diffusion/Images/2024.06.26_E2_TTS_Fig.01b.png)
